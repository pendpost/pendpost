...
import { COMMENT_PLATFORMS, PLATFORM_LANE, LANE_OBJECT_FIELD, metaObjectId, isOwnAuthor, COMMENT_CAPABILITIES, metaPostPermalink, laneReadAvailable, commentReadCapabilities } from './comments.mjs';
  ...
  for (const c of campaigns || []) {
    for (const post of c.posts || []) {
      ...
      for (const p of post.platforms || []) {
        ...
        laneSeen.add(lane);
        targets.push({ campaign: c.id, postId: post.id, platform: p, lane, objectId });
      }
    }
  }
  return targets;
...
export async function commentSweep({ force = false, now = Date.now(), readComments = listComments } = {}) {
  ...
  for (const t of targets) {
    ...
    for (const it of res.items || []) {
      ...
      // Hide the owner's OWN comments/replies (first-comment, own thread replies): they
      // are not inbound to answer. Fail-open per isOwnAuthor - a stranger is never hidden.
      if (isOwnAuthor(t.lane, it)) continue;
