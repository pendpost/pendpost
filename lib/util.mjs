...
export const ERROR_CODES = new Set([
  ...
  // Guarded retry (B3): a "retry now" reschedule was refused because the post's PUBLIC
  // media URL is probe-confirmed dead (404/non-image) - re-arming would just re-fail at
  // the platform's server-side fetch. DISTINCT from media_missing (the local render IS on
  // disk) so the UI shows the re-mirror recovery, not "add a file". A future reschedule is
  // still allowed; the message carries the actionable diagnosis (public-media.mjs).
  'media_url_dead',
  // The MCP job-client pin (audit 2026-08-31): while a radar agent job is in flight, the
  // child-facing radar write tools are pinned to the job's client - a call naming a client
  // with no running job is refused with this code (lib/mcp.mjs callTool). A spawned child
  // should re-read the clientId from its brief; an operator retries after the scan settles.
  'client_mismatch',
  // Spec 50 P3 (§7.9, risk 5): engage_undo was asked to take back something the platform
  // genuinely cannot recall - a Reddit PM, a Mastodon direct-visibility status, a Bluesky chat
  // message, a Nostr reaction. Its OWN code, not invalid_input, because the GUI renders a
  // specific sentence ("Cannot be recalled on {platform}") and hides the Delete control instead
  // of showing a generic error. Nothing is changed when it is returned.
  'no_recall',
  // Spec 50 P3 (§8): a platform said "you're doing that too much" (or answered 429). DISTINCT
  // from engine_failure because the response is the opposite: cool the lane down for 24 hours
  // rather than climb the retry ladder into a ban.
  'platform_limit',
  // Spec 50 P3 (§7.2): the capability table routes this (lane, kind) to the API, but this
  // build's credential tier has no such API - nostr kind-3 follows, kind-6 reposts and NIP-17
  // DMs. DISTINCT from engine_failure ("it broke") and from a silent success: the operator is
  // told plainly that the capability does not exist here, and what to do instead.
  'not_available',
]);
