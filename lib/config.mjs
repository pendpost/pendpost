...
const POSTING_DEFAULTS = { defaultLink: '', utm: '', hashtagPresets: [], defaultTimezone: 'UTC', locale: 'en', contentLanguage: '', platforms: {}, skippedPlatforms: [], publicMediaBaseUrl: '', approvalExpiryHours: null, slotSlipMinutes: null, overdueGraceSeconds: 90, carouselViaCloud: false, autoApprove: { ...AUTO_APPROVE_DEFAULTS }, radar: { ...RADAR_DEFAULTS }, digest: { ...DIGEST_DEFAULTS }, review: { ...REVIEW_DEFAULTS }, relationshipMemory: { ...RELATIONSHIP_MEMORY_DEFAULTS }, commentWatch: { ...COMMENT_WATCH_DEFAULTS }, insights: { ...INSIGHTS_DEFAULTS } };
...
function isRadarConfig(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['enabled', 'competitorsDefault', 'replyVoiceDefault', 'queries', 'sources', 'dailyAt', 'lookbackDays', 'xEnterprise', 'geo', 'brand', 'autoReply', 'agent', 'drafting'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  if ('enabled' in v && typeof v.enabled !== 'boolean') return false;
