...
//    + expiry). NEVER written by pendpost - rotation stays the interactive CLI
//    ceremony (node scripts/<engine>.mjs auth). They are not in ENV_KEY, so they
//    can never reach writeEnvVars.
...
import { ENV_PATH, readEnv, tokenTail, writeEnvVars, atomicWriteJson, errorBody } from './util.mjs';
...
export function setConfig({ ifRev, actor, set } = {}) {
  ...
  if (set.identifiers) {
    ...
    for (const [k, v] of Object.entries(set.identifiers)) {
      if (!(k in ENV_KEY)) return errorBody('invalid_input', `not an editable identifier: ${k}`);
      const err = validateIdentifier(k, v);
      if (err) return errorBody('invalid_input', err);
      envUpdates[ENV_KEY[k]] = v;
    }
  }
