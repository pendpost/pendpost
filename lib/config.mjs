...
import { AUTO_APPROVE_DEFAULTS, AUTO_APPROVE_RADAR_REPLY_DEFAULTS } from './auto-approve.mjs';
import { RADAR_SOURCES, RADAR_CAPABILITIES, ENGAGE_KINDS } from './radar.mjs';
...
export const IDENTIFIER_ENV_KEYS = {
  ...
  linkedinPersonUrn: 'LINKEDIN_PERSON_URN',
...
export const RADAR_OWNER_ONLY_KEYS = ['agent', 'xEnterprise', 'drafting', 'engage'];
...
// owner round 3: daily arming is now the global posting.radar.dailyEnabled switch (was: a
// per-query cadence:'daily'), so the toggle whose off-state contradicted a query's own
// "Täglich" is gone.)
// agent (spec 41) is owner-only because `provider` chooses which binary pendpost spawns and
// `dailyBudget` caps unattended spend. dailyEnabled itself is agent-writable (like the
// per-query flag it replaced) - the owner-only dailyBudget (default 1) is the standing spend
// fence that keeps unattended autonomy owner-authorized. There is deliberately NO `mode` field: scanning is
...
// engage (spec 50, "Respond for me") is owner-only for the sharpest version of the autoReply
// reason: it is the widest autonomy in the app. autoReply could post a REPLY on four lanes;
// engage can reply, like, upvote, follow, repost, message and post ORIGINAL content on every
// lane, including through the operator's own logged-in Chrome. Caps, gaps, waking hours and
// the warm-up ramp are the whole safety story, so an agent must never be able to raise a cap
// or flip the mode to live. It replaces autoReply, which stays valid for one release and is
// migrated on read (migrateAutoReplyToEngage below).
//
// Every default here is closed or conservative: mode off, nothing paused, medium-and-up
// signals only (30 = TIER_MED), no lane enabled, caps that a human could plausibly hit by
// hand, 2-15 minute gaps, waking hours only, a 14-day half-cap warm-up and a 15-minute grace
// window on the high-reach kinds. A kind with cap 0 is disabled (that is how "replies only"
// is expressed - there is no separate mode for it).
const ENGAGE_DEFAULTS = {
  mode: 'off',                          // 'off' | 'dry_run' | 'live'
  paused: false,                        // D18: a resumable pause, independent of mode
  minScore: 30,                         // act threshold; 30 = medium and up
  model: 'claude-opus-4-8',             // D9: triage, drafting and execution alike
  maxDecisionsPerRun: 40,               // the rest stay undecided for the next run
  lanes: {},                            // lane -> { enabled, handle, warmupStartedAt }
  caps: { reply: 10, like: 20, upvote: 20, follow: 5, repost: 3, dm: 2, post: 1 }, // per lane per day
  gapMinutes: { min: 2, max: 15 },
  wakingHours: { start: '08:00', end: '22:00' },   // client timezone
  warmup: { days: 14, factor: 0.5 },
  grace: { minutes: 15, followerThreshold: 10000 },
  ask: { sensitiveConfirm: true },      // D11: the agent may escalate an answered ask to confirm
  disclosure: { respectCommunityRules: true, line: 'Written with AI help, reviewed by a human at {brand}.' },
  originalPosts: { enabled: true, minSignals: 3, windowDays: 7 },
  chrome: { probeTtlHours: 24 },
};
// The engage keys whose VALUE is an object. Both the read-side re-merge and the setConfig
// write-side merge recurse into exactly these, so a partial { caps:{ dm:0 } } write never
// wipes the sibling caps and a partially persisted subtree never presents a missing key.
const ENGAGE_OBJECT_KEYS = Object.freeze(Object.keys(ENGAGE_DEFAULTS).filter((k) => ENGAGE_DEFAULTS[k] && typeof ENGAGE_DEFAULTS[k] === 'object'));
// The full-shape view of a stored (possibly partial, possibly absent) engage subtree.
function mergeEngage(stored) {
  const sub = stored && typeof stored === 'object' && !Array.isArray(stored) ? stored : {};
  const out = { ...ENGAGE_DEFAULTS, ...sub };
  for (const k of ENGAGE_OBJECT_KEYS) {
    const v = sub[k];
    out[k] = { ...ENGAGE_DEFAULTS[k], ...(v && typeof v === 'object' && !Array.isArray(v) ? v : {}) };
  }
  return out;
}
// MIGRATION ON READ (spec 50 §7.1). autoReply (spec 40 6.7) is the narrow ancestor of engage:
// one kind (reply), four lanes, one threshold. It stays valid for one release, and a client
// that had it ON must not silently lose its autonomy the day engage lands - nor gain any.
// So: an UNTOUCHED engage subtree plus an ENABLED autoReply with lanes reads as engage in
// live mode, on exactly those lanes, with every non-reply kind capped at 0. That is the same
// behaviour the owner already authorized, expressed in the new shape - never wider.
//
// Pure (no I/O, no state): readPosting applies it (on the STORED subtree, not the
// default-merged one), and it is exported so a test and any future one-shot rewrite can call
// it on a plain object. It stops the moment the owner has CHOSEN A MODE: a stored `mode` is
// the policy, whatever it says, and autoReply no longer speaks for it. That is the one
// reading that cannot fight the owner - "engage still equals the defaults" would flip an
// explicit `mode:'off'` back to live for as long as the old autoReply switch sat there.
export function migrateAutoReplyToEngage(radar) {
  const storedEngage = radar && typeof radar.engage === 'object' && !Array.isArray(radar.engage) ? radar.engage : null;
  const merged = mergeEngage(storedEngage);
  if (storedEngage && 'mode' in storedEngage) return merged;
  const ar = radar && radar.autoReply;
  const lanes = ar && ar.enabled === true && Array.isArray(ar.lanes) ? ar.lanes.filter((l) => l in RADAR_CAPABILITIES) : [];
  if (!lanes.length) return merged;
  const caps = {};
  for (const k of ENGAGE_KINDS) caps[k] = k === 'reply' ? ENGAGE_DEFAULTS.caps.reply : 0;
  return {
    ...merged,
    mode: 'live',
    lanes: Object.fromEntries(lanes.map((l) => [l, { enabled: true, handle: '', warmupStartedAt: null }])),
    caps,
  };
}

// The radar-reply TRUST migration (owner Q2): the retired posting.radar.autoReply door moves into
// the auto-approve object the owner already knows, as autoApprove.radarReplies. Precedence:
//   1. An explicit stored radarReplies WINS (idempotent - once the owner has saved it here, a
//      lingering legacy autoReply never re-speaks for it).
//   2. Else the legacy posting.radar.autoReply seeds it (same shape, lanes clamped).
//   3. Else an engage-live client's reply-capable lanes seed it: that client WAS auto-posting
//      replies through the engage force-approve door (now removed), so carry the exact grant
//      across, clamped to RADAR_AUTO_REPLY_LANES - nobody silently loses auto-reply on ship day.
//   4. Else fail-closed to off.
// Pure (no I/O): readPosting applies it on the STORED subtrees. Exported for a test.
export function deriveRadarReplies(rrStored, legacyAutoReply, engage) {
  const hasStored = rrStored && typeof rrStored === 'object' && !Array.isArray(rrStored);
  // An OWNER-ARMED radarReplies (enabled, or carrying lanes) is the source of truth and wins - once
  // set here, a lingering legacy autoReply never re-speaks for it. A default-off radarReplies (what
  // setConfig persists before anyone arms it) must NOT block the one-time legacy/engage migration.
  const armed = hasStored && (rrStored.enabled === true || (Array.isArray(rrStored.lanes) && rrStored.lanes.length > 0));
  if (armed) {
    return { ...AUTO_APPROVE_RADAR_REPLY_DEFAULTS, ...rrStored };
  }
  if (legacyAutoReply && typeof legacyAutoReply === 'object' && !Array.isArray(legacyAutoReply)) {
    return {
      ...AUTO_APPROVE_RADAR_REPLY_DEFAULTS,
      enabled: legacyAutoReply.enabled === true,
      lanes: Array.isArray(legacyAutoReply.lanes) ? legacyAutoReply.lanes.filter((l) => RADAR_AUTO_REPLY_LANES.includes(l)) : [],
      requireLintClean: legacyAutoReply.requireLintClean !== false,
      ...(Number.isFinite(legacyAutoReply.minScore) ? { minScore: legacyAutoReply.minScore } : {}),
    };
  }
  // ONLY a LIVE engage client was actually auto-posting replies (dry_run posts nothing, so it never
  // authorized unread replies - it must not seed a trust grant).
  if (engage && engage.mode === 'live' && engage.lanes && typeof engage.lanes === 'object'
      && Number(engage.caps?.reply) > 0) {
    const lanes = Object.entries(engage.lanes)
      .filter(([lane, cfg]) => cfg && cfg.enabled === true && RADAR_AUTO_REPLY_LANES.includes(lane))
      .map(([lane]) => lane);
    if (lanes.length) return { ...AUTO_APPROVE_RADAR_REPLY_DEFAULTS, enabled: true, lanes };
  }
  // No legacy/engage source: keep a stored-but-disarmed radarReplies (its requireLintClean/minScore
  // choices round-trip), else fail-closed to the defaults.
  return { ...AUTO_APPROVE_RADAR_REPLY_DEFAULTS, ...(hasStored ? rrStored : {}) };
}
export const RADAR_DEFAULTS = { enabled: false, competitorsDefault: [], replyVoiceDefault: '', queries: [], sources: {}, dailyEnabled: false, dailyAt: '09:00', lookbackDays: 90, xEnterprise: false, geo: { buyingQuestions: [], provider: '' }, brand: { facts: '', isSupplyOnly: false, audience: '', notForClaims: '' }, agent: { provider: '', dailyBudget: 1, maxPerRun: 20 }, drafting: { minScore: 30, maxPerRun: 20 }, engage: { ...ENGAGE_DEFAULTS } };
...
// notify: WHERE a push goes, as opposed to whether one is sent (digest.notify above).
// telegramChatId is the owner's own chat with the brand's existing Telegram bot, stored by
// the `owner-chat` ceremony (spec 50 S7) - the bot token already exists from the Telegram
// lane, so no secret is typed and nothing here is a credential. Empty ('') = no Telegram
// push; the macOS notification still fires and the digest carries the setup line, so a
// missing chat id degrades, never throws. A notification PREFERENCE, not autonomy: like
// digest.notify it is deliberately NOT owner-gated - it can only change where a message the
// owner already asked for is delivered, never what the agent may do.
const NOTIFY_DEFAULTS = { telegramChatId: '' };
...
const POSTING_DEFAULTS = { defaultLink: '', utm: '', hashtagPresets: [], defaultTimezone: 'UTC', locale: 'en', contentLanguage: '', platforms: {}, skippedPlatforms: [], publicMediaBaseUrl: '', approvalExpiryHours: null, slotSlipMinutes: null, overdueGraceSeconds: 90, carouselViaCloud: false, autoApprove: { ...AUTO_APPROVE_DEFAULTS }, radar: { ...RADAR_DEFAULTS }, digest: { ...DIGEST_DEFAULTS }, notify: { ...NOTIFY_DEFAULTS }, review: { ...REVIEW_DEFAULTS }, relationshipMemory: { ...RELATIONSHIP_MEMORY_DEFAULTS }, commentWatch: { ...COMMENT_WATCH_DEFAULTS }, insights: { ...INSIGHTS_DEFAULTS } };
...
export const POSTING_OWNER_ONLY_KEYS = ['autoApprove', 'approvalExpiryHours', 'slotSlipMinutes', 'overdueGraceSeconds', 'carouselViaCloud', 'relationshipMemory', 'insights'];
...
function readPosting() {
  try {
    ...
    // notify (spec 50 S7) gets the same full-shape re-merge: a caller reading
    // notify.telegramChatId must always see a real string (default '' = not set up),
    // never undefined, so the push path can branch on it without a guard.
    const notifyStored = data && typeof data.notify === 'object' && !Array.isArray(data.notify) ? data.notify : {};
    merged.notify = { ...NOTIFY_DEFAULTS, ...notifyStored };
    ...
    // Cadence collapsed to ONE global flag (owner: "decide the cadence once"). Derive dailyEnabled
    // from any query still carrying the retired per-query daily cadence - UNLESS the global flag was
    // already persisted, in which case storedDaily wins, so once it is saved the derivation stops
    // re-reading stale cadence and stays idempotent for headless installs that never open the
    // Studio. Unlike the autoScan strip this READS the old value before dropping it, so a client
    // with daily queries keeps scanning daily. Derive BEFORE the query map below (it still sees
    // cadence), then strip per-query `cadence` in the same pass that filters unknown sources.
    if (typeof radarStored.dailyEnabled !== 'boolean') {
      merged.radar.dailyEnabled = Array.isArray(merged.radar.queries)
        && merged.radar.queries.some((q) => q && q.enabled !== false && q.cadence === 'daily');
    }
    if (Array.isArray(merged.radar.queries)) {
      merged.radar.queries = merged.radar.queries.map((q) => {
        if (!q || typeof q !== 'object') return q;
        const rest = { ...q };
        delete rest.cadence; // retired: per-query cadence is now the global dailyEnabled flag
        if (Array.isArray(rest.sources)) rest.sources = rest.sources.filter((x) => RADAR_SOURCES.includes(x));
        return rest;
      });
    }
    ...
    // The owner-only object subtrees (agent, drafting) get the same full-shape re-merge for the
    // same reason: a partially persisted subtree would otherwise present without some keys, and a
    // caller reading them would silently get undefined. Fail-closed shapes throughout. (engage is
    // handled one level deeper just below; xEnterprise is a scalar and rides the base merge.)
    ...
    // engage (spec 50) needs the re-merge ONE LEVEL DEEPER than its owner-only siblings: its
    // value keys (caps, gapMinutes, warmup, grace, ...) are themselves objects, so the shallow
    // merge above would present a stored partial `caps` with its sibling kinds missing - and a
    // missing cap reads as "no limit" at exactly the wrong place. migrateAutoReplyToEngage does
    // that full-shape merge and, for a client still on the old autoReply switch, expresses the
    // autonomy the owner already authorized in the new shape.
    merged.radar.engage = migrateAutoReplyToEngage({ ...merged.radar, engage: radarStored.engage });
    // MERGE the radar-reply trust into the auto-approve object (owner Q2). Runs AFTER the engage
    // migration so the engage-live seed sees the folded lanes. Explicit autoApprove.radarReplies
    // wins; else the legacy radar.autoReply; else engage-live reply lanes; else off. Then strip
    // the retired radar.autoReply (reading it first) - the autoScan precedent, so an old install
    // still saves through the Studio's read-modify-write.
    merged.autoApprove.radarReplies = deriveRadarReplies(stored.radarReplies, radarStored.autoReply, merged.radar.engage);
    delete merged.radar.autoReply;
    ...
    return { ...POSTING_DEFAULTS, autoApprove: { ...AUTO_APPROVE_DEFAULTS }, radar: { ...RADAR_DEFAULTS, engage: mergeEngage(null) }, digest: { ...DIGEST_DEFAULTS }, notify: { ...NOTIFY_DEFAULTS }, review: { ...REVIEW_DEFAULTS }, relationshipMemory: { ...RELATIONSHIP_MEMORY_DEFAULTS }, commentWatch: { ...COMMENT_WATCH_DEFAULTS }, insights: { ...INSIGHTS_DEFAULTS } };
  }
}
...
function identifiers() {
  return {
    ...
    linkedinPersonUrn: readEnv('LINKEDIN_PERSON_URN') || '',
...
export const PLATFORM_ENV_KEYS = {
  ...
  linkedin: ['LINKEDIN_ACCESS_TOKEN', 'LINKEDIN_REFRESH_TOKEN', 'LINKEDIN_TOKEN_EXPIRES_AT', 'LINKEDIN_ORG_URN', 'LINKEDIN_PERSON_URN', 'LINKEDIN_API_VERSION', 'LINKEDIN_CLIENT_ID', 'LINKEDIN_CLIENT_SECRET'],
...
function validateIdentifier(key, v) {
  ...
  if (key === 'linkedinPersonUrn') return (v === '' || /^urn:li:person:[A-Za-z0-9_-]+$/.test(v)) ? null : 'linkedinPersonUrn must be urn:li:person:<id>';
...
function isAutoApprovePolicy(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['enabled', 'platforms', 'campaigns', 'types', 'requireLintClean', 'radarReplies'];
  ...
  // radarReplies is the relocated radar auto-reply trust scope; same closed shape as the retired
  // posting.radar.autoReply (isRadarAutoReply), still clamped to RADAR_AUTO_REPLY_LANES so a
  // copy-only / non-reply lane can never be armed here.
  if ('radarReplies' in v && !isRadarAutoReply(v.radarReplies)) return false;
...
// The posting.notify subtree (spec 50 S7): where a push is delivered. telegramChatId is the
// owner's chat id with the brand's own Telegram bot - a routing address, not a credential
// (the bot token lives in .env and is never reachable from here). A bounded string; '' is
// the "not set up" default the digest nags about.
function isNotifyConfig(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['telegramChatId'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  if ('telegramChatId' in v && !(typeof v.telegramChatId === 'string' && v.telegramChatId.length <= 64)) return false;
  return true;
}
...
function isRadarQuery(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['id', 'label', 'brief', 'enabled', 'sources', 'keywords', 'excludeKeywords', 'subreddits', 'instances', 'hashtags', 'competitors', 'intentPatterns', 'minScore', 'actionsWanted', 'replyVoice', 'cadence', 'warmup', 'mention', 'lang'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  ...
  // `lang` is the query's search language (spec: DE/FR queries must SEARCH in their own language,
  // not return English). A BCP-47 tag validated exactly like posting.locale/contentLanguage. It is
  // an OPTIONAL explicit override: absent, the agent infers the language from the query's own
  // keywords (radar-prompt.mjs WHERE-TO-LOOK). Agent-writable - a per-query field, not a
  // radar-subtree key, so never gated by RADAR_OWNER_ONLY_KEYS.
  if ('lang' in v && !isLocaleTag(v.lang)) return false;
...
// The posting.radar.engage subtree (spec 50 §7.1, "Respond for me"): the widest autonomy
// policy in the app, so the validator is the strictest one here. Every key is a closed set
// and every number is range-checked, because these numbers ARE the safety story: caps, gaps,
// waking hours and the warm-up ramp are what keeps a platform from reading the brand as a
// bot. A partial subtree is accepted (setConfig merges it, one level deeper than its
// siblings), an unknown key is refused at the door, and lanes are restricted to the Radar
// capability table so a typo can never enable a platform that does not exist.
const ENGAGE_MODES = ['off', 'dry_run', 'live'];
const ENGAGE_LANE_KEYS = ['enabled', 'handle', 'warmupStartedAt'];
const HHMM = /^([01]\d|2[0-3]):[0-5]\d$/;
const isInt = (v, lo, hi) => Number.isInteger(v) && v >= lo && v <= hi;
// A closed-key object whose every present key passes `check`.
function isClosedObject(v, allowed, check) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  return Object.entries(v).every(([k, val]) => check(k, val));
}
function isEngageLanes(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  return Object.entries(v).every(([lane, l]) => (
    Object.keys(RADAR_CAPABILITIES).includes(lane)
    && isClosedObject(l, ENGAGE_LANE_KEYS, (k, val) => (
      k === 'enabled' ? typeof val === 'boolean'
        : k === 'handle' ? typeof val === 'string' && val.length <= 100
          : val === null || (typeof val === 'string' && val.length <= 40)
    ))
  ));
}
function isRadarEngage(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['mode', 'paused', 'minScore', 'model', 'maxDecisionsPerRun', 'lanes', 'caps', 'gapMinutes', 'wakingHours', 'warmup', 'grace', 'ask', 'disclosure', 'originalPosts', 'chrome'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  if ('mode' in v && !(typeof v.mode === 'string' && ENGAGE_MODES.includes(v.mode))) return false;
  if ('paused' in v && typeof v.paused !== 'boolean') return false;
  // The act threshold, 0-100 to match intentScore's range (the drafting threshold is separate).
  if ('minScore' in v && !(Number.isFinite(v.minScore) && v.minScore >= 0 && v.minScore <= 100)) return false;
  // A free string: the operator's own CLI validates model ids (the agent.draftModel precedent).
  if ('model' in v && !(typeof v.model === 'string' && v.model.length <= 100)) return false;
  // The per-run triage batch. Bounded by the signal cap - asking to decide more signals than a
  // scan can ever hold would be a knob that lies (the agent.maxPerRun precedent).
  if ('maxDecisionsPerRun' in v && !isInt(v.maxDecisionsPerRun, 1, 200)) return false;
  if ('lanes' in v && !isEngageLanes(v.lanes)) return false;
  // Per lane per day, per kind. 0 disables the kind (that is how "replies only" is expressed).
  if ('caps' in v && !isClosedObject(v.caps, ENGAGE_KINDS, (k, val) => isInt(val, 0, 200))) return false;
  // The random pause between two actions on one lane, in minutes. A zero gap would burst.
  if ('gapMinutes' in v) {
    if (!isClosedObject(v.gapMinutes, ['min', 'max'], (k, val) => isInt(val, 1, 120))) return false;
    if ('min' in v.gapMinutes && 'max' in v.gapMinutes && v.gapMinutes.min > v.gapMinutes.max) return false;
  }
  if ('wakingHours' in v && !isClosedObject(v.wakingHours, ['start', 'end'], (k, val) => typeof val === 'string' && HHMM.test(val))) return false;
  // The ramp: `days` at half (`factor`) of every cap. days 0 = no warm-up; factor is a share
  // of the cap, so above 1 it would RAISE the cap during warm-up, which the word cannot mean.
  if ('warmup' in v && !isClosedObject(v.warmup, ['days', 'factor'], (k, val) => (
    k === 'days' ? isInt(val, 0, 90) : Number.isFinite(val) && val >= 0 && val <= 1
  ))) return false;
  // The "posting soon" undo window on high-reach rows, and what counts as high reach.
  if ('grace' in v && !isClosedObject(v.grace, ['minutes', 'followerThreshold'], (k, val) => (
    k === 'minutes' ? isInt(val, 0, 120) : Number.isInteger(val) && val >= 0
  ))) return false;
  if ('ask' in v && !isClosedObject(v.ask, ['sensitiveConfirm'], (k, val) => typeof val === 'boolean')) return false;
  if ('disclosure' in v && !isClosedObject(v.disclosure, ['respectCommunityRules', 'line'], (k, val) => (
    k === 'respectCommunityRules' ? typeof val === 'boolean' : typeof val === 'string' && val.length <= 200
  ))) return false;
  if ('originalPosts' in v && !isClosedObject(v.originalPosts, ['enabled', 'minSignals', 'windowDays'], (k, val) => (
    k === 'enabled' ? typeof val === 'boolean' : k === 'minSignals' ? isInt(val, 1, 100) : isInt(val, 1, 90)
  ))) return false;
  if ('chrome' in v && !isClosedObject(v.chrome, ['probeTtlHours'], (k, val) => isInt(val, 1, 168))) return false;
  return true;
}
...
function isRadarAgent(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  // `daily` is deliberately absent (owner round 3): daily arming is a connected provider + the
  // global posting.radar.dailyEnabled switch, so the validator refuses the retired key at the
  // door (readPosting strips it from configs persisted by older builds - the autoScan precedent).
...
function isRadarConfig(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['enabled', 'competitorsDefault', 'replyVoiceDefault', 'queries', 'sources', 'dailyEnabled', 'dailyAt', 'lookbackDays', 'xEnterprise', 'geo', 'brand', 'agent', 'drafting', 'engage'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  if ('enabled' in v && typeof v.enabled !== 'boolean') return false;
  // The ONE global cadence switch (replaces per-query cadence): on ⇒ every enabled query is
  // swept once a day at dailyAt; off ⇒ Radar is on-demand only ("Scan now"). Agent-writable
  // like the per-query flag it replaces (the paid agent budget stays the owner-only spend fence).
  if ('dailyEnabled' in v && typeof v.dailyEnabled !== 'boolean') return false;
  ...
  // How far back a scan considers posts (2026-08-26). A whole number of days 1-3650, or
  // null/0 = no limit. Bounded so a typo can never yield a zero-day (nothing) or absurd window.
  if ('lookbackDays' in v && !(v.lookbackDays === null || v.lookbackDays === 0 || (Number.isInteger(v.lookbackDays) && v.lookbackDays >= 1 && v.lookbackDays <= 3650))) return false;
  ...
  if ('engage' in v && !isRadarEngage(v.engage)) return false;
  return true;
}
...
function validatePosting(key, v) {
  ...
  // The grace, in seconds, before a past-due-but-unpublished post reads the red
  // "overdue" alarm - so a post does NOT flash overdue during the normal
  // publish+confirm window (the scheduler ticks every 60s). Display-only; never
  // gates publishing (the scheduler fires on due<=now, not derivedState). 0 = off
  // (overdue the instant a slot passes, the pre-grace behavior).
  if (key === 'overdueGraceSeconds') return (Number.isInteger(v) && v >= 0 && v <= 3600) ? null : 'overdueGraceSeconds must be a whole number of seconds between 0 and 3600 (0 turns the overdue grace off)';
  // Rollout gate for cloud-served IG carousels / feed images (spec 39 companion). OFF by
  // default so a carousel/IG-image still fires LOCALLY until the owner has DEPLOYED the
  // pendpost-cloud presigned-media companion (presignGet + N-slide staging), then flips this
  // to route those posts through the cloud. Owner-only: it changes where a post publishes from.
  if (key === 'carouselViaCloud') return typeof v === 'boolean' ? null : 'carouselViaCloud must be a boolean (route cloud-served IG carousels/images through pendpost-cloud once its companion is deployed)';
  if (key === 'radar') return isRadarConfig(v) ? null : 'radar must be an object { enabled?, competitorsDefault?[], replyVoiceDefault?, queries?[], sources?{ [source]:{ scan? } }, dailyEnabled?boolean (global: sweep every enabled query daily at dailyAt; off = on-demand only), dailyAt?"HH:MM", lookbackDays?1-3650|null (how far back a scan considers posts; null/0 = no limit; default 90), xEnterprise?, geo?{ buyingQuestions?[], provider? }, brand?{ facts?, isSupplyOnly?, audience?, notForClaims? }, agent?{ provider?, dailyBudget?, maxPerRun?, draftModel? }, drafting?{ minScore?0-100, maxPerRun?1-50 }, engage?{ mode?off|dry_run|live, paused?, minScore?0-100, model?, maxDecisionsPerRun?1-200, lanes?{ [lane]:{ enabled?, handle?, warmupStartedAt? } }, caps?{ [kind]:0-200 }, gapMinutes?{ min?1-120, max?1-120 }, wakingHours?{ start?"HH:MM", end?"HH:MM" }, warmup?{ days?0-90, factor?0-1 }, grace?{ minutes?0-120, followerThreshold?>=0 }, ask?{ sensitiveConfirm? }, disclosure?{ respectCommunityRules?, line? }, originalPosts?{ enabled?, minSignals?1-100, windowDays?1-90 }, chrome?{ probeTtlHours?1-168 } } } where each query is { id?, label?, sources?[], keywords?[], competitors?[], minScore?0-100, cadence?manual|daily, ... }';
  if (key === 'notify') return isNotifyConfig(v) ? null : 'notify must be an object { telegramChatId?: string } - the owner chat the Telegram push goes to (stored by `node scripts/telegram-social.mjs owner-chat`); empty means no Telegram push';
  ...
  if (set.posting) {
    ...
    for (const [k, v] of Object.entries(set.posting)) {
      ...
      if (k === 'radar') {
        ...
        // engage (spec 50) recurses ONE LEVEL FURTHER than its owner-only siblings, because its
        // value keys are themselves objects. Without this, turning one kind off
        // ({ engage:{ caps:{ dm:0 } } }) would replace the whole caps object and, via the
        // read-side re-merge, silently reset every other kind to its default - a partial write
        // RAISING a cap the owner had lowered. The pattern is the geo/brand recursion, one
        // level down, over the engage keys whose default is an object.
        if (v && typeof v.engage === 'object' && !Array.isArray(v.engage)) {
          const curEngage = (posting.radar && posting.radar.engage) || {};
          for (const k3 of ENGAGE_OBJECT_KEYS) {
            if (typeof v.engage[k3] === 'object' && v.engage[k3] !== null && !Array.isArray(v.engage[k3])) {
              nextPosting.radar.engage[k3] = { ...(curEngage[k3] || {}), ...v.engage[k3] };
            }
          }
        }
      } else if (k === 'notify') {
        // notify SHALLOW-MERGES onto the current subtree, mirroring digest/review: a partial
        // write must not wipe a sibling delivery address.
        nextPosting.notify = { ...(posting.notify || {}), ...v };
      } else if (k === 'review') {
      ...
      } else if (k === 'autoApprove') {
        // autoApprove SHALLOW-MERGES onto the current subtree so a partial write - e.g. the Radar
        // automation control sending only { radarReplies:{ enabled, lanes } } - never wipes the
        // normal-post axes (platforms/campaigns/types), and vice-versa. radarReplies then recurses
        // ONE LEVEL: a partial { radarReplies:{ lanes } } must not drop its sibling enabled/minScore.
        // The whole subtree is owner-only (POSTING_OWNER_ONLY_KEYS), so a non-owner write was already
        // refused above - arming radar auto-reply stays owner-authorized, exactly like the retired
        // posting.radar.autoReply door it replaces.
        nextPosting.autoApprove = { ...(posting.autoApprove || {}), ...v };
        if (v && typeof v.radarReplies === 'object' && !Array.isArray(v.radarReplies)) {
          nextPosting.autoApprove.radarReplies = { ...((posting.autoApprove && posting.autoApprove.radarReplies) || {}), ...v.radarReplies };
        }
