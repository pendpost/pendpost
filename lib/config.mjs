...
const POSTING_DEFAULTS = { defaultLink: '', utm: '', hashtagPresets: [], defaultTimezone: 'UTC', locale: 'en', contentLanguage: '', platforms: {}, skippedPlatforms: [], publicMediaBaseUrl: '', approvalExpiryHours: null, slotSlipMinutes: null, autoApprove: { ...AUTO_APPROVE_DEFAULTS }, radar: { ...RADAR_DEFAULTS }, digest: { ...DIGEST_DEFAULTS }, review: { ...REVIEW_DEFAULTS }, relationshipMemory: { ...RELATIONSHIP_MEMORY_DEFAULTS }, commentWatch: { ...COMMENT_WATCH_DEFAULTS } };
...
export const POSTING_OWNER_ONLY_KEYS = ['autoApprove', 'approvalExpiryHours', 'slotSlipMinutes', 'relationshipMemory'];
...
function readPosting() {
  try {
    ...
    return { ...POSTING_DEFAULTS, autoApprove: { ...AUTO_APPROVE_DEFAULTS }, radar: { ...RADAR_DEFAULTS }, digest: { ...DIGEST_DEFAULTS }, review: { ...REVIEW_DEFAULTS }, relationshipMemory: { ...RELATIONSHIP_MEMORY_DEFAULTS }, commentWatch: { ...COMMENT_WATCH_DEFAULTS } };
  }
}
...
function isRadarConfig(v) {
  if (!v || typeof v !== 'object' || Array.isArray(v)) return false;
  const allowed = ['enabled', 'competitorsDefault', 'replyVoiceDefault', 'queries', 'sources', 'dailyAt', 'xEnterprise', 'geo', 'brand', 'autoReply', 'agent'];
  if (Object.keys(v).some((k) => !allowed.includes(k))) return false;
  if ('enabled' in v && typeof v.enabled !== 'boolean') return false;
...
function validatePosting(key, v) {
  ...
  if (key === 'radar') return isRadarConfig(v) ? null : 'radar must be an object { enabled?, competitorsDefault?[], replyVoiceDefault?, queries?[], dailyAt?"HH:MM", brand?{ facts?, isSupplyOnly?, audience?, notForClaims? } } where each query is { id?, label?, sources?[], keywords?[], competitors?[], minScore?0-100, cadence?manual|daily, ... }';
