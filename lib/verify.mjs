...
// Spec 51 §4.6 / D2: project a verifyReceipt per-platform verdict onto the two additive
// fields verify_post exposes. No receipt => { signed:false, provenance:'none' }. A broken
// signature OR a broken chain => 'invalid'; intact statement but drifted content =>
// 'content-changed'; all-good => 'verified'.
function receiptProvenance(verdict) {
  if (!verdict) return { signed: false, provenance: 'none' };
  if (!verdict.signatureValid || !verdict.chainValid) return { signed: true, provenance: 'invalid' };
  if (!verdict.contentMatchesPlan) return { signed: true, provenance: 'content-changed' };
  return { signed: true, provenance: 'verified' };
}
...
export async function verifyPost({ campaign, postId, actor, skipMeta = false } = {}) {
  ...
  // Spec 51 (D2): fold per-platform signed-receipt provenance into the RESULT only
  // (additive, result-shape - NOT persisted to post.verify, NO schema change, NO new
  // tool or route). Computed locally via the verifyReceipt lib function; never contacts a
  // platform and never blocks a verify read.
  let verdicts = {};
  try { verdicts = (await verifyReceipt({ campaign, postId, platform: null, actor: who }))?.receipts || {}; }
  catch { verdicts = {}; }
  const platformsOut = {};
  for (const [plat, entry] of Object.entries(block.platforms)) {
    platformsOut[plat] = { ...entry, ...receiptProvenance(verdicts[plat]) };
  }
  // A platform that carries a receipt but had no live read-back still surfaces provenance.
  for (const plat of Object.keys(verdicts)) {
    if (!platformsOut[plat]) platformsOut[plat] = { live: false, state: null, permalink: null, account: null, served: null, ...receiptProvenance(verdicts[plat]) };
  }
  return { ok: true, campaign, postId, verify: { ...block, platforms: platformsOut }, liveCount, checked: Object.keys(byPlatform).length };
}
