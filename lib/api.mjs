...
import {
  ...
  tokenRefresh, xUpdateProfile, validateMedia, platformValidate, pendpostHealth, publishPreview, uploadAsset, deleteAsset, renameAsset, setMetaLane, resumeLane,
  ...
  runRadarScan, listRadar, triageSignal, triageSignalsBulk, restoreSignals, markCopyPosted, queueRadarReply, logRadarFootprint, radarIngest, radarFollowupCheck,
  radarAgentScan, radarAgentStop, radarGeoReset, radarDraftComparison, radarAgentComparison,
  archiveClientSweep, autonomyStatus, revokeAutoApprovals,
} from './writes.mjs';
// Spec 50 P1: the auto-engage triage report (the hard-rule gate lives in that module, not here).
...
import { fetchInsights, getInsights, generateDigest, includeMeteredForScope } from './insights.mjs';
...
import { listClients, setActiveClient, createClient, updateClient } from './clients.mjs';
import { createReviewer, listReviewers, revokeReviewer } from './reviewers.mjs';
import { readEngagers, forgetEngagerVerb, unforgetEngagerVerb, linkEngagersVerb, unlinkEngagersVerb, dismissLinkVerb } from './engager-verbs.mjs';
// Spec 50 P2 ("Respond for me"): the auto-engage verbs, shared with the MCP face.
import { engageStatus, engageQueueList, engageCancel, engageUndo, engagePause, engageProbe, engageConfirmHandle, engageCommunityRecheck, engageAsksList, engageAnswer, engageConfirmAsk, engageDismiss } from './engage-verbs.mjs';
...
// /api/clients/<id>/reviewers - the client review link reviewer identities (spec 48
// R10): GET lists them (tail-only), POST mints one (owner-gated, returns the link
// once). The bare /api/clients + /api/clients/<id> routes never match (they lack the
// trailing /reviewers segment). Matched under the isClientAdmin prefix, so the handler
// runs UNSCOPED and takes the client from the path, never from a scoping override.
const REVIEWERS_RE = /^\/api\/clients\/([a-z0-9][a-z0-9-]*)\/reviewers$/;
// /api/clients/<id>/reviewers/<reviewerId>/revoke - revoke one reviewer (owner-gated).
const REVIEWER_REVOKE_RE = /^\/api\/clients\/([a-z0-9][a-z0-9-]*)\/reviewers\/([a-z0-9][a-z0-9-]*)\/revoke$/;
...
const ERROR_STATUS = {
  ...
  // Enforced pre-flight gate: the post carries a content blocker on a targeted lane.
  // 409 (conflict with the not-yet-fit state), same family as not_approved.
  not_ready: 409,
  ...
  // Inbound-reply round-trip (spec 23): a reaction/follow event has no thread to answer
  // (422 unprocessable), an unknown eventId is a 404, a missing write scope is a 403,
  // depleted X API credits are a 402 (surfacing the top-up path), and a gone tweet/DM
  // conversation is a 410.
  not_repliable: 422, not_found: 404, needs_scope: 403, credits: 402, target_gone: 410,
  // Spec 50 P3: an undo the platform cannot perform (409 - a conflict with reality, not a bad
  // request), a rate limit (429, with the lane already cooling down), and a capability this
  // build does not have (501 - not implemented HERE, which is what the code means).
  no_recall: 409, platform_limit: 429, not_available: 501,
};
...
const ROUTES = [
  ...
  // lookup absorbs the cover path.
  {
    ...
    handler: async (req, res, url) => {
      ...
      if (/^image\//i.test(contentType)) {
        ...
        args = { campaign, postId, frameSec: body.frameSec, filePath: body.filePath, base64: body.base64, coverUrl: body.coverUrl };
      }
  ...
  },
  {
    ...
    handler: jsonRoute(POST_ACTION_RE, (body, m) => approvePost({ campaign: m[1], postId: m[2], actor: body.actor, note: body.note, force: body.force === true })),
  },
  {
  ...
  },
  {
    ...
    handler: jsonRoute(POST_ACTION_RE, (body, m) => markPosted({ campaign: m[1], postId: m[2], actor: body.actor, externalUrl: body.externalUrl, platform: body.platform })),
  },
  {
  ...
  {
    // The autonomy ledger read (ux-audit R7): the ONE derived read the ledger needs
    // beyond config_get + pendpost_health - the auto-approve dry-run (AU5, "this scope
    // would match N of your last M drafts") and the count of live policy-approvals a
    // revoke would unwind (AU4). Pure derivation over plans, no new engine state.
    // UI-facing read (mcpTool: null): a preview computed for the operator's own toggle.
    method: 'GET', path: '/api/autonomy', mcpTool: null,
    handler: (req, res, url) => {
      const raw = Number(url.searchParams.get('limit'));
      const limit = Number.isInteger(raw) && raw > 0 ? raw : 20;
      sendResult(res, autonomyStatus({ limit }));
    },
  },
  {
    // AU4 revoke-that-unwinds: the write twin of the read above. Owner-only sweep that
    // returns not-yet-published policy approvals to review. Its MCP face is autonomy_revoke.
    method: 'POST', path: '/api/autonomy/revoke', mcpTool: 'autonomy_revoke',
    handler: jsonRoute(/^.*$/, (body) => revokeAutoApprovals({ actor: body.actor })),
  },
  {
  ...
  },
  {
    ...
    // A4 archive safety sweep: archiveClientSweep (lib/writes.mjs) computes the
    // client's in-flight work first, refuses a blind archive over native-scheduled
    // platform objects (needs_confirm + counts), and fans the optional
    // unscheduleInFlight sweep over the existing unschedule verb before archiving.
    handler: jsonRoute(CLIENT_ARCHIVE_RE, (body, m) => archiveClientSweep({ id: m[1], actor: body.actor, acknowledgeInFlight: body.acknowledgeInFlight === true, unscheduleInFlight: body.unscheduleInFlight === true })),
  },
  // --- client review link reviewers (spec 48 R10) ---
  // The reviewer identities live per-brand in clients/<id>/reviewers.json (a 128-bit
  // token IS the whole identity). These three routes are the operator-face twins of
  // reviewer_list / reviewer_create / reviewer_revoke; the reviewer's own decision
  // face is the FOURTH face (POST /review/<token>/decision on the separate review
  // listener), NOT an /api route, so it has no twin here (its identity is the token,
  // which a loopback caller cannot carry). Matched by the isClientAdmin prefix in
  // handleApi and therefore run UNSCOPED: the clientId comes from the PATH, never a
  // scoping override. reviewer_list is a plain read (any operator actor); create and
  // revoke are OWNER-gated (actor must be "owner") right here, mirroring the MCP twin,
  // so both faces reject a non-owner identically. No credential VALUE is ever touched.
  {
    method: 'GET', prefix: '/api/clients/', mcpTool: 'reviewer_list',
    test: (route) => REVIEWERS_RE.test(route),
    handler: (req, res, url) => {
      const m = url.pathname.replace(/\/+$/, '').match(REVIEWERS_RE);
      return sendResult(res, listReviewers({ clientId: m[1] }));
    },
  },
  {
    method: 'POST', prefix: '/api/clients/', mcpTool: 'reviewer_create',
    test: (route) => REVIEWERS_RE.test(route),
    // Body { name, expiresAt? (nullable, default null) } -> { ok, reviewer, token,
    // actorString }. The token (the whole link) is returned EXACTLY ONCE and is never
    // retrievable again. Owner-gated identically to the MCP reviewer_create twin.
    handler: jsonRoute(REVIEWERS_RE, (body, m) => {
      if (body.actor !== 'owner') {
        return errorBody('invalid_input', 'inviting a reviewer is owner-only: pass actor: "owner". Minting a client review link is never delegated to an agent except on the owner\'s explicit instruction.');
      }
      return createReviewer({ clientId: m[1], name: body.name, expiresAt: body.expiresAt ?? null, actor: body.actor });
    }),
  },
  {
    method: 'POST', prefix: '/api/clients/', mcpTool: 'reviewer_revoke',
    test: (route) => REVIEWER_REVOKE_RE.test(route),
    // -> { ok, reviewer } with revoked:true. Idempotent (revoking twice is a no-op
    // success). Owner-gated identically to the MCP reviewer_revoke twin.
    handler: jsonRoute(REVIEWER_REVOKE_RE, (body, m) => {
      if (body.actor !== 'owner') {
        return errorBody('invalid_input', 'revoking a reviewer is owner-only: pass actor: "owner". Revoking a client review link is never delegated to an agent except on the owner\'s explicit instruction.');
      }
      return revokeReviewer({ clientId: m[1], reviewerId: m[2], actor: body.actor });
    }),
  },
  // --- Phase E insights ---
  {
    method: 'GET', path: '/api/insights', mcpTool: 'read_insights',
    handler: (req, res) => sendResult(res, getInsights()),
  },
  {
    method: 'POST', path: '/api/insights/fetch', mcpTool: 'fetch_insights',
    handler: jsonRoute(/^.*$/, (body) => fetchInsights({ campaign: typeof body.campaign === 'string' ? body.campaign : null, includeMetered: includeMeteredForScope(body.scope) })),
  },
  {
  ...
  },
  {
    ...
    // B2 confirm gate lives inside replyToComment, so this twin inherits it: a bare
    // non-owner call (curl, an agent hitting REST directly) returns needs_confirm ->
    // 428 here (via ERROR_STATUS); the Studio panel posts actor:'owner' and is untouched.
    method: 'POST', path: '/api/comments/reply', mcpTool: 'reply_to_comment',
    handler: jsonRoute(/^.*$/, (body) => replyToComment({ campaign: body.campaign, postId: body.postId, commentId: body.commentId, text: body.text, platform: body.platform, actor: body.actor, confirm: body.confirm, author: body.author })),
  },
  {
    // Inbound-reply round-trip WRITE (spec 23): answer one inbound X Activity event
    // (paired twin of reply_to_inbound_event, twin of GET /api/cloud/events). Routes by
    // the stored event's type - mention/comment -> a tweet reply, message -> a DM,
    // reaction/follow -> not_repliable. A real platform write, but NOT a scheduled
    // publish - operator-triggered, required actor, logged as an 'inbound-reply'/
    // 'inbound-dm' Activity entry; never touches approval. The B2 confirm gate lives
    // inside replyToInboundEvent, so this twin inherits it - a bare non-owner call
    // returns needs_confirm -> 428 here (via ERROR_STATUS); the Studio inbox posts
    // actor:'owner' and is untouched.
    method: 'POST', path: '/api/inbound/reply', mcpTool: 'reply_to_inbound_event',
    handler: jsonRoute(/^.*$/, (body) => replyToInboundEvent({ actor: body.actor, eventId: body.eventId, text: body.text, confirm: body.confirm, clientId: body.clientId })),
  },
  {
  ...
  {
    // Own-post comment monitor READ (own-post comment inbox): the aggregated unanswered
    // comments across THIS client's recently-published posts, grouped by post, newest
    // comment first. GET twin of the comment_inbox MCP tool - the third face the sweep
    // (state.comments) feeds, so the Studio + an agent see the SAME unanswered set without
    // opening every post. Distinct from GET /api/comments (one post's live thread): this is
    // the cross-post inbox the sweep maintains. Client-scoped by the handleApi binding.
    method: 'GET', path: '/api/comments/inbox', mcpTool: 'comment_inbox',
    handler: async (req, res) => sendJson(res, 200, commentInbox()),
  },
  {
    // Own-post comment monitor REFRESH (check now): force one sweep of this client's
    // recently-published posts, then return the fresh inbox. Twin of comment_inbox_refresh.
    // The on-demand path the "Check now" control drives, bypassing the interval cadence
    // clock (not the enabled gate - OFF stays inert). READ-only against the platforms.
    method: 'POST', path: '/api/comments/inbox/refresh', mcpTool: 'comment_inbox_refresh',
    handler: jsonRoute(/^.*$/, async () => { await commentSweep({ force: true }); return commentInbox(); }),
  },
  {
    // Own-post comment monitor RESOLVE: mark one comment handled (the owner replied or
    // dismissed it) so it leaves the unanswered set and the next sweep never re-surfaces it.
    // Twin of comment_resolve. Not a platform write - it only stamps the local seen-ledger;
    // the actual reply goes through POST /api/comments/reply (unchanged). Idempotent.
    method: 'POST', path: '/api/comments/inbox/resolve', mcpTool: 'comment_resolve',
    handler: jsonRoute(/^.*$/, (body) => resolveComment({ key: body.key, reason: body.reason, actor: body.actor })),
  },
  {
  ...
  },
  {
    ...
    // entry; never touches approval. Reversible, but still public text posted
    // immediately: the B2 confirm gate lives inside replyToReview, so this twin
    // inherits it - a bare non-owner call returns needs_confirm -> 428 here; the
    // Studio panel posts actor:'owner' and is untouched.
    method: 'POST', path: '/api/reviews/reply', mcpTool: 'reply_to_review',
    handler: jsonRoute(/^.*$/, (body) => replyToReview({ reviewId: body.reviewId, text: body.text, actor: body.actor, confirm: body.confirm })),
  },
  {
    // Relationship memory READ (spec 49 R12, Pattern P4-read). GET twin of the list_engagers
    // MCP tool - named directly (like list_reviews) so no API-CONTRACT.md exemption is needed.
    // With lane+handle, one person's accreted history (the popover's source of truth); without,
    // the full brand list. NEVER gated: the operator's own popover always reads (the agentRead
    // opt-in gates ONLY the MCP tool, not this REST read). Local-only, never a network call.
    // Client-scoped by handleApi. Always ok:true (a corrupt store degrades to empty).
    method: 'GET', path: '/api/engagers', mcpTool: 'list_engagers',
    handler: (req, res, url) => {
      const lane = url.searchParams.get('lane');
      const handle = url.searchParams.get('handle');
      const limitRaw = Number(url.searchParams.get('limit'));
      return sendJson(res, 200, readEngagers({ lane: lane || undefined, handle: handle || undefined, limit: Number.isInteger(limitRaw) && limitRaw > 0 ? limitRaw : undefined }));
    },
  },
  {
    // Relationship memory WRITE (spec 49 S6): forget a person (paired twin of forget_engager).
    // DESTRUCTIVE + confirm-gated INSIDE forgetEngagerVerb, so a bare curl without confirm:true
    // returns needs_confirm -> 428 here (via ERROR_STATUS); the GUI posts confirm:true after its
    // inline ForgetConfirm. Local-only; never touches the two records of any linked pair.
    method: 'POST', path: '/api/engagers/forget', mcpTool: 'forget_engager',
    handler: jsonRoute(/^.*$/, (body) => forgetEngagerVerb({ lane: body.lane, handle: body.handle, confirm: body.confirm })),
  },
  {
    // Relationship memory WRITE (spec 49 S6u): un-forget a person (paired twin of
    // unforget_engager). Restorative, NOT confirm-gated - it only clears the tombstone so
    // accretion resumes; it never resurrects the erased history. Local-only.
    method: 'POST', path: '/api/engagers/unforget', mcpTool: 'unforget_engager',
    handler: jsonRoute(/^.*$/, (body) => unforgetEngagerVerb({ lane: body.lane, handle: body.handle })),
  },
  {
    // Relationship memory WRITE (spec 49 S4c): link two people (paired twin of link_engagers).
    // A stored ASSOCIATION, NOT a merge - both records stay separate; additive, NOT confirm-
    // gated, idempotent. Body shape { a:{lane,handle}, b:{lane,handle} }. Local-only.
    method: 'POST', path: '/api/engagers/link', mcpTool: 'link_engagers',
    handler: jsonRoute(/^.*$/, (body) => linkEngagersVerb({ a: body.a, b: body.b })),
  },
  {
    // Relationship memory WRITE (spec 49 S4u): un-link two people (paired twin of
    // unlink_engagers). CONFIRM-gated inside unlinkEngagersVerb (removes an operator decision;
    // still lossless since nothing was merged) - a bare call returns needs_confirm -> 428 here.
    // Body shape { a:{lane,handle}, b:{lane,handle}, confirm }. Local-only.
    method: 'POST', path: '/api/engagers/unlink', mcpTool: 'unlink_engagers',
    handler: jsonRoute(/^.*$/, (body) => unlinkEngagersVerb({ a: body.a, b: body.b, confirm: body.confirm })),
  },
  {
    // Relationship memory WRITE (spec 49 S4): dismiss a cross-lane GUESS (record it so the
    // suggestion stops re-surfacing). GUI/REST-ONLY by design - there is deliberately NO MCP
    // twin (the dismissible hint is an operator affordance on the popover, not an agent verb),
    // so this route carries mcpTool:null and is a documented uiOnly parity exemption in
    // API-CONTRACT.md. Idempotent, no confirm. Body shape { a:{lane,handle}, b:{lane,handle} }.
    method: 'POST', path: '/api/engagers/dismiss-link', mcpTool: null,
    handler: jsonRoute(/^.*$/, (body) => dismissLinkVerb({ a: body.a, b: body.b })),
  },
  {
  ...
  },
  {
    ...
    handler: jsonRoute(/^.*$/, (body) => triageSignal({ source: body.source, externalId: body.externalId, backlogKey: body.backlogKey, action: body.action, actor: body.actor })),
  },
  {
    // Radar (beta) BULK triage WRITE (granular filters): dismiss a LIST of cached signals in
    // one call (paired twin of radar_triage_bulk). Same LOCAL, idempotent, non-destructive
    // contract as /api/radar/triage - it just fans the single dismiss over an array. A WATCHED
    // signal is NEVER dismissed (engine-side guard); unknown sources are skipped, not fatal.
    method: 'POST', path: '/api/radar/triage-bulk', mcpTool: 'radar_triage_bulk',
    handler: jsonRoute(/^.*$/, (body) => triageSignalsBulk({ ids: body.ids, action: body.action, actor: body.actor })),
  },
  {
    // Radar (beta) UNDO WRITE (granular filters): restore signals a dismiss removed - single or
    // bulk. A LOCAL state write, no MCP twin BY DESIGN (GUI-only undo; an agent composes
    // dismiss + rescan/ingest instead, so it is listed in the `routes` parity exemption). It
    // re-inserts the passed signal objects the GUI held and clears their seen-ledger entries so
    // a re-scan does not re-suppress them. Idempotent, never touches approval/publish.
    method: 'POST', path: '/api/radar/restore', mcpTool: null,
    handler: jsonRoute(/^.*$/, (body) => restoreSignals({ signals: body.signals, actor: body.actor })),
  },
  {
    // R5 piece 2 (Pattern P4): record a copy-draft signal posted by hand (paired twin of
    // radar_mark_copy_posted). A LOCAL state write - no platform reach, idempotent, never
    // touches approval. The durable proof a copy draft went out (there is no repliedUrl).
    method: 'POST', path: '/api/radar/mark-copy-posted', mcpTool: 'radar_mark_copy_posted',
    handler: jsonRoute(/^.*$/, (body) => markCopyPosted({ source: body.source, externalId: body.externalId, postedUrl: body.postedUrl, actor: body.actor })),
  },
  {
    ...
    handler: jsonRoute(/^.*$/, (body) => queueRadarReply({ campaign: body.campaign, signalUrl: body.signalUrl, source: body.source, externalId: body.externalId, parentExternalId: body.parentExternalId, text: body.text, executionMode: body.executionMode, actor: body.actor, confirm: body.confirm })),
  },
  {
    // Auto-engage TRIAGE report WRITE (spec 50 P1, §7.4): the paired twin of
    // radar_engage_report. A LOCAL state write - it records one decision per signal and queues
    // Action rows; it never posts, likes, follows, messages or approves anything, so it is
    // neither confirm-gated nor destructive. Every hard rule lives INSIDE the verb, so this
    // route is exactly as safe as the MCP face and cannot be the softer of the two. Inert while
    // "Respond for me" is off. Client-scoped by handleApi.
    method: 'POST', path: '/api/radar/engage-report', mcpTool: 'radar_engage_report',
    // P4: the SAME verb also carries a browser batch's results ({ code, results, handleSeen,
    // community }). Which branch runs is decided by the engage fence inside the verb, never by
    // this route, so the two faces cannot diverge on it.
    handler: jsonRoute(/^.*$/, (body) => radarEngageReport({
      clientId: body.clientId, actor: body.actor, decisions: body.decisions, themes: body.themes,
      batchId: body.batchId, lane: body.lane, code: body.code, handleSeen: body.handleSeen,
      community: body.community, results: body.results,
    })),
  },
  {
    ...
    handler: jsonRoute(/^.*$/, (body) => logRadarFootprint({ question: body.question, mentioned: body.mentioned, competitorsMentioned: body.competitorsMentioned, excerpt: body.excerpt, assistant: body.assistant, actor: body.actor })),
  },
  {
  ...
  },
  {
    ...
    // agent does not run. wait:false (Stage 1): the request resolves the instant the running job
    // row is saved, and the panel renders + polls that row from GET /api/radar - so the browser
    // never holds the multi-minute POST open (which died at the network layer as a raw "Load
    // failed"). The MCP twin calls radarAgentScan directly (wait defaults true) and still awaits.
    method: 'POST', path: '/api/radar/agent-scan', mcpTool: 'radar_agent_scan',
    handler: jsonRoute(/^.*$/, (body) => radarAgentScan({ clientId: body.clientId, queryId: body.queryId, actor: body.actor, scope: body.scope, target: body.target, sources: body.sources, wait: false })),
  },
  {
  ...
  {
    // Radar (beta) GEO reset WRITE (S7.3, radar-reliability 2026-08-31): drop the bound
    // client's AI-visibility STATE (footprint log + derived comparison backlog + dismissed
    // ledger) in one shot - the recovery for a tenant seeded with another brand's rows,
    // which no config edit can clear because it is state, not config. Paired twin of
    // radar_geo_reset. A LOCAL state write - no platform reach, no model call. Destructive
    // to agent-logged history, so radarGeoReset itself refuses any non-owner actor
    // (invalid_input -> 400); the Studio passes actor:'owner' like its other owner writes.
    // Client-scoped by handleApi (radarGeoReset is withClient-bound at the call site).
    method: 'POST', path: '/api/radar/geo-reset', mcpTool: 'radar_geo_reset',
    handler: jsonRoute(/^.*$/, (body) => radarGeoReset({ clientId: body.clientId, actor: body.actor })),
  },
  // ---------------------------------------------------------------------------
  // "Respond for me" (auto-engage, spec 50 7.8). Six routes, each the twin of an MCP verb
  // of the same name, plus one summary read the ledger row and the platform list render from.
  // The POLICY (mode, platforms, caps, gaps, waking hours) is written through the EXISTING
  // owner-only config save on posting.radar.engage - there is no bespoke policy route, exactly
  // as Radar itself has none.
  // ---------------------------------------------------------------------------
  {
    // The summary the ledger row S1 and the platform list S2 read: mode, the pause flag, one
    // row PER LANE in ENGAGE_CAPABILITIES (the owner's intent and the platform's reality side
    // by side), today's tallies and the "Waiting for Chrome" count. A pure local read that
    // always answers - including with the mode off, because the ledger has to be able to say
    // "Off" as truthfully as it says "Live". No MCP twin: an agent reads the same facts
    // through config_get (the policy) and engage_queue_list (the work), and inventing a
    // seventh verb to restate them would be the parity rule obeyed to the letter and broken
    // in spirit. Client-scoped by handleApi.
    method: 'GET', path: '/api/engage', mcpTool: null,
    handler: async (req, res) => sendJson(res, 200, engageStatus({})),
  },
  {
    // The action list READ (Pattern P4-read). GET twin of engage_queue_list, with the same
    // status / lane filters. Pure read of state.engage.queue - it never posts, never probes
    // and never moves a row.
    method: 'GET', path: '/api/engage/queue', mcpTool: 'engage_queue_list',
    handler: async (req, res, url) => sendJson(res, 200, engageQueueList({
      status: url.searchParams.get('status') || null,
      lane: url.searchParams.get('lane') || null,
    })),
  },
  {
    // Pause / resume everything (row 12). OWNER-ONLY and mode-gated inside the verb, so this
    // route and its MCP twin inherit the identical gates. A resumable hold, never Off.
    method: 'POST', path: '/api/engage/pause', mcpTool: 'engage_pause',
    handler: jsonRoute(/^.*$/, (body) => engagePause({ paused: body.paused, actor: body.actor || 'owner' })),
  },
  {
    // "Check again" on one platform (rows 2e / 2e2). Reaches the platform READ-only for an
    // API lane (the same liveness probe health_recheck runs); a browser lane records the
    // honest not_logged_in until the engage child (P4) can look at Chrome for itself.
    method: 'POST', path: '/api/engage/probe', mcpTool: 'engage_probe',
    handler: jsonRoute(/^.*$/, (body) => engageProbe({ lane: body.lane, actor: body.actor || 'owner' })),
  },
  {
    // The inline [Yes] / [No] on the platform row (row 2e2). OWNER-ONLY: it writes the
    // owner-only handle into config. There is no free-text handle field anywhere - the handle
    // is recognised from what the probe reported, never typed.
    method: 'POST', path: '/api/engage/confirm-handle', mcpTool: 'engage_confirm_handle',
    handler: jsonRoute(/^.*$/, (body) => engageConfirmHandle({ lane: body.lane, ok: body.ok, actor: body.actor || 'owner' })),
  },
  {
    // Cancel one queued or posting-soon action (row 5, from the row overflow). OWNER-ONLY.
    // A row already handed to an executor is refused - that would be an undo, not a cancel.
    method: 'POST', path: '/api/engage/cancel', mcpTool: 'engage_cancel',
    handler: jsonRoute(/^.*$/, (body) => engageCancel({ actionId: body.actionId, actor: body.actor || 'owner' })),
  },
  {
    // P3 (spec 50 §7.9): take ONE performed action back. The Studio's row-11 inline
    // "Delete / Keep" confirm calls this. OWNER-ONLY, and reachable while the mode is off -
    // the owner who just switched the feature off is the likeliest person to want this.
    method: 'POST', path: '/api/engage/undo', mcpTool: 'engage_undo',
    handler: jsonRoute(/^.*$/, (body) => engageUndo({ actionId: body.actionId, actor: body.actor || 'owner' })),
  },
  {
    // "[Check rules again]" on a signal skipped by a community rule (row 7e5): clears the
    // cached rule and re-opens the signals it skipped, so a changed sidebar is not a dead end.
    method: 'POST', path: '/api/engage/community-recheck', mcpTool: 'engage_community_recheck',
    handler: jsonRoute(/^.*$/, (body) => engageCommunityRecheck({ lane: body.lane, community: body.community, actor: body.actor || 'owner' })),
  },
  // ---------------------------------------------------------------------------
  // "Needs you" (spec 50 P5a, §7.7). The strip above the Radar feed reads the first route
  // and writes the other three; each is the twin of the MCP verb of the same name, and both
  // faces inherit the owner-only + mode-off gates that live inside lib/engage-verbs.mjs.
  // ---------------------------------------------------------------------------
  {
    // Everything that genuinely needs a human, joined onto the signal it is about (S3/S4
    // render exactly these fields). A pure local read that answers in every mode, including
    // off: a backlog filed while the feature was live must not vanish when it is switched off.
    method: 'GET', path: '/api/engage/asks', mcpTool: 'engage_asks_list',
    handler: async (req, res, url) => sendJson(res, 200, engageAsksList({ status: url.searchParams.get('status') || 'open' })),
  },
  {
    // Row 8: the owner's one-line answer. SPAWNS the operator's own agent to write the reply
    // from it, so this route is slower than its neighbours by design - the Studio shows the
    // sending state on the row's own primary while it runs. OWNER-ONLY.
    method: 'POST', path: '/api/engage/answer', mcpTool: 'engage_answer',
    handler: jsonRoute(/^.*$/, (body) => engageAnswer({ askId: body.askId, text: body.text, actor: body.actor || 'owner' })),
  },
  {
    // Row 8e: post the held reply, optionally edited. Skips the grace window - the owner just
    // read it, and charging them a second wait for that would be the opposite of forgiveness.
    method: 'POST', path: '/api/engage/confirm', mcpTool: 'engage_confirm',
    handler: jsonRoute(/^.*$/, (body) => engageConfirmAsk({ askId: body.askId, text: body.text ?? null, actor: body.actor || 'owner' })),
  },
  {
    // Row 8e2: skip it. The signal records `skip / owner`, so the feed says "Skipped by you"
    // rather than attributing a human decision to the machine.
    method: 'POST', path: '/api/engage/dismiss', mcpTool: 'engage_dismiss',
    handler: jsonRoute(/^.*$/, (body) => engageDismiss({ askId: body.askId, actor: body.actor || 'owner' })),
  },
  {
  ...
  {
    // Resume a lane the generic lane-block breaker halted (lib/state.mjs
    // recordLaneBlock - first case: X HTTP 402, API credits depleted). Clears the
    // block, releases the publishHolds its failure streak parked, then re-fires the
    // released posts NOW as the credit recheck (writes.mjs#resumeLane). Agent-operable
    // (mcpTool: resume_lane) so the owner can resolve a halted lane from chat/CLI, not
    // only the dashboard: it re-enables publishing the owner already approved (like
    // meta_lane_set paused:false), so actor is required but no confirm gate. Meta's 368
    // breaker keeps its own clear (POST /api/state/meta-block; resumeLane refuses meta).
    method: 'POST', path: '/api/state/lane-resume', mcpTool: 'resume_lane',
    handler: async (req, res) => {
      let body;
      try {
        body = await readJsonBody(req);
      ...
      return sendResult(res, await resumeLane(body));
    },
  },
  {
    ...
    // dot. sync.state is FOUR-valued (green: all confirmed; yellow: push pending;
    // amber: degraded, the local backstop covers it; red: broken, a post is at risk
    // now) - see cloudSyncStatus for the reason ladder. Pure local read - never returns
    // a key or token, never blocks on the network; sync is null when not cloud-managed.
  ...
  },
  {
    ...
    // worker fires; turning ON first seals that client's tokens into the vault
    // (fail-closed: a failed seal aborts the toggle) and pushes its approved jobs.
    // Operator-only.
