 ...
 * @returns {{ text: string, changed: boolean, fixes: string[], counts: object, findings: object[] }}
 *   findings are the WARN-severity lint hits on the cleaned text (advisory, not applied).
 *   counts maps each applied fix kind to its OCCURRENCE count in this string (R6b receipt),
 *   so callers can report "2 em dashes" instead of just "something changed".
 */
export function humanize(text, { locale = 'en' } = {}) {
  if (typeof text !== 'string' || text === '') {
    return { text: typeof text === 'string' ? text : '', changed: false, fixes: [], counts: {}, findings: [] };
  }
  let out = text;
  const fixes = [];
  const counts = {};
  ...
  if (DASH.test(out)) {
    counts['em-dash'] = (out.match(/[–—]/g) || []).length;
    out = out
      .replace(/\s*[–—]\s*/g, ', ')
      .replace(/,\s*([.!?;:])/g, '$1')
      .replace(/^\s*,\s+/, '')
  ...
  const curlyHits = (out.match(CURLY_APOS) || []).length + (out.match(CURLY_QUOT) || []).length;
  if (curlyHits) counts['curly-quote'] = curlyHits;
  ...
  if (locale === 'de-CH' && out.includes('ß')) {
    counts.eszett = (out.match(/ß/g) || []).length;
    out = out.replace(/ß/g, 'ss');
    fixes.push('eszett');
  }

  const findings = lintText(out).findings.filter((f) => f.severity === 'warn');
  return { text: out, changed: out !== text, fixes: [...new Set(fixes)], counts, findings };
}
 ...
 * @returns {{ changed: boolean, fixes: {kind: string, count: number}[], findings: object[] }}
 *   fixes aggregates each fix kind's occurrence count ACROSS the fields (R6b receipt -
 *   "2 em dashes" over the whole post, not per field); findings carry the originating `field`.
 */
export function humanizeFields(obj, fieldNames, locale = 'en') {
  if (!obj || typeof obj !== 'object') return { changed: false, fixes: [], findings: [] };
  let changed = false;
  const findings = [];
  const counts = {};
  ...
  const fixes = Object.entries(counts).map(([kind, count]) => ({ kind, count }));
  return { changed, fixes, findings };
}
