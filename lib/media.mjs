...
export function serveMedia(req, res, url) {
  ...
  // Optional per-request client scope. The dashboard mints cover/media URLs with the
  // OWNING client's id (lib/plans.mjs mediaUrl) so the all-clients List resolves each
  // row against its OWN root, not just the globally-active client. Absent (a bare or
  // legacy URL) => today's activeRoot() behavior, byte-for-byte. The within-root guard
  // below re-runs under whichever root we bind, so scoping can only NARROW, never widen,
  // what is servable - each client stays sealed to its own data/ subtree.
  const clientId = url.searchParams.get('clientId');
  if (clientId) {
    let root;
    try {
      root = clientRoot(clientId);
    } catch {
      return sendJson(res, 403, errorBody('invalid_input', 'invalid clientId'));
    }
    return withClient(root, () => streamFromRoot(req, res, p));
  }
  return streamFromRoot(req, res, p);
}

function streamFromRoot(req, res, p) {
