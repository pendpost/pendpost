...
// THE PER-LANE BUDGET FOLLOWS THE WORK (incident 2026-09-04, a second client). A manual scan isolates
...
// Spec 50 P1 (scope 'triage'): the child that DECIDES act / skip / ask for the signals pendpost
// itself picked. Modelled on AGENT_DRAFT_TOOLS deliberately, and for the same reasons: no web
// tools (it is handed the thread text as data and has nothing left to look up; a child that can
// fetch is a child a thread can TELL what to fetch), one write tool, and the two read-only
// lookups the draft child demonstrably needed - radar_list to re-read a cached signal's full
// fields, config_get for the project's own posting context.
//
// radar_queue_reply is deliberately ABSENT: a triage decision is not a reply-post, and a child
// holding both could route around the engage gate by queueing the reply directly. Its one write
// is radar_engage_report, which lands behind every hard rule in lib/engage-triage.mjs.
export const AGENT_TRIAGE_TOOLS = Object.freeze(['mcp__pendpost__radar_engage_report', 'mcp__pendpost__radar_list', 'mcp__pendpost__config_get']);

// --- spec 50 P4: the ENGAGE scopes (the child that drives a real browser) ----------------
//
// The browser tools come from a SECOND MCP server, `browser` (@playwright/mcp), which
// lib/engage-browser.mjs describes and runAgentJob writes into the per-job --mcp-config
// alongside pendpost's own. Their MCP names are therefore mcp__browser__<tool>.
//
// ENUMERATED, never `mcp__browser` (the whole-server form). @playwright/mcp ships
// browser_evaluate and browser_run_code_unsafe - arbitrary JavaScript in the page context -
// and a child reading an untrusted thread must not hold either: that is the one place where a
// page's own words could become code. browser_fill_form is out for the same reason in
// miniature (a form filler is exactly the tool that types into a password field); a composer
// needs browser_type and nothing more. browser_file_upload, browser_drop, browser_drag and the
// network/console readers have no part in "read the thread, post a reply", so they are absent.
//
// This is a genuine tool-layer narrowing, and it is NOT the credential defence: browser_type
// stays allowed because the composer needs it, so §9's read-only-first rule, the auth_wall
// code and the transcript audit remain the whole story for "never types a password".
export const AGENT_ENGAGE_TOOLS = Object.freeze([
  ...
  'mcp__pendpost__radar_engage_report',
  'mcp__pendpost__radar_list',
]);

// The IDENTITY probe (§7.2 identity column). Strictly read-only at the TOOL layer: there is
// no way for this child to click, type or press a key, so "the probe cannot log anything in"
// is a property of the allow-list rather than of the prompt. Its one write is the report.
export const AGENT_ENGAGE_PROBE_TOOLS = Object.freeze([
  'mcp__browser__browser_navigate',
  'mcp__browser__browser_snapshot',
  'mcp__browser__browser_find',
  'mcp__browser__browser_wait_for',
...
// The tool calls that put CHARACTERS on a page. The §9 transcript audit is defined against
// this set: any of these AFTER the child reported auth_wall fails the batch and cools the lane
// down. browser_fill_form is listed even though the allow-list above withholds it - an audit
// that only knows about the tools we happen to allow today rots the moment the list grows.
export const BROWSER_TYPING_TOOLS = Object.freeze(['browser_type', 'browser_fill_form', 'browser_press_key']);

// Where a spawned child's tool-call transcript lands (§7.6). One JSONL file per runId, holding
// the tool NAME and a short target string per call - never page content, never a tool result.
// It is the only evidence that survives a child, and row 7e2's acceptance is written against it.
export const AGENT_LOG_DIR = path.join(os.homedir(), '.pendpost', 'agent-logs');

export const transcriptPath = (runId) => path.join(AGENT_LOG_DIR, `${String(runId).replace(/[^A-Za-z0-9._-]/g, '_')}.jsonl`);

// The short, content-free target for one tool call: a url, an element description, a ref, an
// actionId. Capped hard, because a snapshot's own input can carry a page's words and a
// transcript is not a place to accumulate untrusted text.
export function transcriptTarget(input) {
  if (!input || typeof input !== 'object') return '';
  for (const k of ['url', 'element', 'ref', 'actionId', 'name', 'selector', 'text']) {
    const v = input[k];
...
// PURE: one stream-json event -> the tool calls it carries. Exported so the audit test can
// build a transcript without spawning anything.
export function transcriptEventsFrom(evt) {
  ...
  return blocks
    ...
    .map((b) => ({ name: b.name, target: transcriptTarget(b.input) }));
}

// Append one transcript line. Fail-soft: a transcript that cannot be written must never sink
// a job - the audit that reads it back treats a missing file as "nothing observed", and the
// engine's own report handling is what decides the row's fate either way.
export function appendTranscript(runId, entry) {
  if (!runId) return false;
  try {
    fs.mkdirSync(AGENT_LOG_DIR, { recursive: true });
    fs.appendFileSync(transcriptPath(runId), `${JSON.stringify({ ts: new Date().toISOString(), ...entry })}\n`, { mode: 0o600 });
    return true;
  } catch { return false; }
}

// Read one child's transcript back. [] when the file is absent or unreadable - an absent
// transcript is "we did not observe", never "nothing happened", and the callers say so.
export function readTranscript(runId) {
  let raw = '';
  try { raw = fs.readFileSync(transcriptPath(runId), 'utf8'); } catch { return []; }
  const out = [];
  for (const line of raw.split('\n')) {
    const s = line.trim();
    if (!s) continue;
    try { const o = JSON.parse(s); if (o && typeof o === 'object') out.push(o); } catch { /* skip */ }
  }
  return out;
}
...
// open.
//
// `extraServers` (spec 50 P4) merges ADDITIONAL stdio servers in beside pendpost's own - today
// only `browser` (@playwright/mcp), described by lib/engage-browser.mjs. It is a plain object
// of server definitions the CALLER built from frozen constants, never anything read from
// config: --strict-mcp-config means this file is the child's ENTIRE server surface, so what
// lands here is exactly what the child can reach. The written config carries no secret (the
// browser server's whole state is the on-disk Chrome profile), and the file is 0600 regardless.
function writeMcpConfig(extraServers = null) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pendpost-agent-'));
  const file = path.join(dir, 'mcp.json');
  const mcpServers = { pendpost: { type: 'http', url: mcpUrl() } };
  if (extraServers && typeof extraServers === 'object' && !Array.isArray(extraServers)) {
    for (const [name, def] of Object.entries(extraServers)) {
      // pendpost's own server is never overridable from a caller's map: that is the seam a
      // future bug would use to point the child's radar_engage_report somewhere else.
      if (name === 'pendpost' || !def || typeof def !== 'object') continue;
      mcpServers[name] = def;
    }
  }
 ...
 */
export async function runAgentJob({ providerId, prompt, allowedTools, model = null, timeoutMs = AGENT_TIMEOUT_MS, jobId = null, root = activeRoot(), stream = false, onEvent = null, meterNow = Date.now, mcpServers = null, transcriptRunId = null } = {}) {
  const def = AGENT_PROVIDERS[providerId];
  ...
  const cfg = writeMcpConfig(mcpServers);
  try {
    // A transcript needs the per-turn NDJSON: the plain json envelope carries no record of
    // which tools were called (PROVEN #3), so asking for one implies stream mode. The caller's
    // own onEvent still fires - observing is additive, never a replacement.
    const wantStream = stream === true || Boolean(transcriptRunId);
    const observe = transcriptRunId
      ? (evt) => {
        for (const call of transcriptEventsFrom(evt)) appendTranscript(transcriptRunId, call);
        if (onEvent) onEvent(evt);
      }
      : onEvent;
    const argv = def.argv({ prompt, mcpConfigPath: cfg.file, allowedTools, model, stream: wantStream });
    let child;
    try {
