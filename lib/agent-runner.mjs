...
// the whole 10-minute budget and the job dies "ran too long and was stopped", with the hook
...
export async function runAgentJob({ providerId, prompt, allowedTools, model = null, timeoutMs = AGENT_TIMEOUT_MS, jobId = null, root = activeRoot(), stream = false, onEvent = null } = {}) {
  ...
  try {
    ...
    const tail = normalizeTail((stderrClean || envText || stdoutClean).slice(-AGENT_TAIL_MAX));
    const detail = normalizeTail(result.detail || firstLine(envText) || envelopeDetail(stdoutClean) || firstLine(stderrClean, stdoutClean)) || null;
    ...
    const error = reclassifiable && AGENT_LIMIT_RE.test(`${tail} ${detail || ''}`) ? 'limit' : result.error;
    return {
      ...result,
      error,
      stdout: stdoutClean,
