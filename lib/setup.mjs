...
import { connectFor, boundClientId } from './context.mjs';
...
// THE OWNER PUSH ENTRY (spec 50 S7, row 9e). A SIBLING of platforms[] for the same reason
// `agent` is one: `ready` below is every(platform => live || skipped) and pendpostHealth turns
// every non-live platform into a global BLOCKER. A phone push is a courtesy on top of the
// Studio strip and the daily digest, so an instance with every lane live is READY whether or
// not the owner ever wants their phone buzzed. Folding it into `ready` would paint a working
// instance red over an opt-in convenience, which is how a Setup page teaches people to ignore
// it. It is a HINT, and it is honest about which of the two halves is missing:
//   - the Telegram LANE carries the bot token, so without it there is nothing to push through;
//   - the owner CHAT id is what the `owner-chat` ceremony stores, and it is a preference, not
//     a credential, which is why it lives in config.json.
// Naming the nearer of the two in `fix` is the difference between an owner who runs one
// command and an owner who reads "Telegram is not working" and opens a portal.
function notifySetup(accounts) {
  const chatId = String(((getPosting() || {}).notify || {}).telegramChatId || '').trim();
  const laneConnected = Boolean(accounts && accounts.telegram && accounts.telegram.authenticated);
  const connected = laneConnected && Boolean(chatId);
  let fix = null;
  // `auth` follows the platform cards (connectFor scopes it only where there is more than one
  // client). `owner-chat` always names the client, because the command itself refuses to guess
  // which brand's asks should reach this phone - and the digest's row 9e line says it the same
  // way, so the owner never sees two different spellings of one command.
  const ownerChatCmd = `node scripts/telegram-social.mjs owner-chat --client ${boundClientId() || '<id>'}`;
  if (!laneConnected) fix = connectFor('node scripts/telegram-social.mjs auth');
  else if (!chatId) fix = ownerChatCmd;
  return {
    telegram: {
      label: 'Urgent push to your phone',
      // connected | incomplete only: there is no "skipped" here, because not wanting a push
      // is the DEFAULT, and an owner who never runs the ceremony has not left anything undone.
      status: connected ? 'connected' : 'incomplete',
      connected,
      // Presence only. The chat id is not a secret, but there is no reason for a health
      // payload to carry a number that identifies the owner's own Telegram account.
...
export function setupStatus() {
  ...
  const platforms = PLATFORMS.map((p) => {
    ...
    // The ceremony command, scoped to this client on a multi-client workspace (bare
    // otherwise). One resolution per lane, reused by every surface below.
    const connect = connectFor(def.connect);
    ...
    if (!connected) {
      ...
      missing.push({ kind: 'secret', label: def.secret, how: 'cli', action: connect });
    }
    ...
    else if (state === 'unproven' && !connected) fix = connect;
    else if (state === 'failed') fix = `token invalid or expired - re-run: ${connect}`;
    ...
    return {
      ...
      // Spec 18 (P9): the nostr lane carries whether a NIP-96 media server is
      // configured so the connected card can tell the honest media-state line
      // (images publish) from the "set NOSTR_MEDIA_SERVER" hint (text-only).
      ...(p === 'nostr' ? { mediaServer: Boolean(acct.mediaServer) } : {}),
  ...
  return {
    ...
    // Spec 50 S7 / row 9e: where an URGENT ask reaches the owner when they are not looking at
    // the screen. A sibling of `agent` for the same reason and with the same posture - a hint
    // the Setup card can render, never a term in `ready` or `summary`.
    notify: notifySetup(accounts),
    config,
  };
}
