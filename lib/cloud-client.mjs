...
export async function reconcileCloudResults({ since = null } = {}) {
  ...
  if (failed.length || credentialRefusals.length || nowLive.length) {
    try {
      const s = loadState();
      ...
      if (s.cloudAccepted && typeof s.cloudAccepted === 'object') {
        for (const p of patched) {
          ...
          for (const k of Object.keys(s.cloudAccepted)) if (k.startsWith(prefix)) { delete s.cloudAccepted[k]; mutated = true; }
        }
      }
      ...
      if (s.cloudRetriggered && typeof s.cloudRetriggered === 'object') {
        for (const p of patched) {
          const prefix = `${p.campaign}:${p.postId}:`;
          for (const k of Object.keys(s.cloudRetriggered)) if (k.startsWith(prefix)) { delete s.cloudRetriggered[k]; mutated = true; }
        }
      }
...
const INBOUND_EVENT_TYPES = new Set(['comment', 'mention', 'message', 'reaction']);
