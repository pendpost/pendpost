...
import { isCarouselPost, carouselItems } from './carousel.mjs';
...
//
// CAROUSEL + IG FEED IMAGE companion (2026-08): the pendpost-cloud presigned-media seam
// (presignGet + N-slide staging + injection) now EXISTS. The rollout gate
// `posting.carouselViaCloud` (OFF by default) lets those two shapes cloud-fire ONLY once the
// owner has DEPLOYED that cloud companion - until then they still fire locally (unchanged), so
// landing this on develop before the cloud deploy is safe. nostr-longform stays local-only (a
// separate companion). The flag is read via an optional param so tests can pin either state.
export function cloudFiresPost(post, carouselViaCloud = getPosting().carouselViaCloud === true) {
  if (post.type === 'nostr-longform') return false;
  const isCarousel = isCarouselPost(post);
  const isIgImage = post.type === 'image' && (post.platforms || []).includes('instagram');
  if ((isCarousel || isIgImage) && !carouselViaCloud) return false;
  return true;
}
...
export async function pushApprovedJobs() {
  ...
  for (const { campaign: c, post } of eligibleDuePosts(campaigns, {})) {
    ...
    try {
      ...
      // A CAROUSEL has no single post.media - upload EVERY slide's bytes, keyed by the same
      // RELATIVE plan path the envelope's mediaItems carry and the cloud resolves against
      // (sync.ts mediaByPath.get(path)). carouselItems is the ONE ordered source shared with
      // buildPublishJob, so the manifest keys and the envelope's slide paths can never disagree.
      for (const it of carouselItems(post)) {
        const rel = it.path || it.file;
        if (!rel || mediaByPath.has(rel)) continue;
        const slideAbs = path.isAbsolute(rel) ? rel : path.resolve(activeRoot(), rel);
        const { sha256, bytes } = await ensureUploaded(slideAbs, 'media');
        mediaByPath.set(rel, { mediaPath: rel, sha256, bytes });
      }
    } catch (e) {
...
export async function remediateCloudFailures(failed = [], { now = Date.now() } = {}) {
  ...
  // Meta vault row every time, which is what kept handing a second client's Instagram lane to
