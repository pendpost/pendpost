...
function requireActor(actor) {
  ...
  // Spec 48 §3.2: the `reviewer:` actor namespace is reserved (minted server-side
  // from a review token), so it is refused on the operator client-admin faces too.
...
// B5 + US-MC-10: attach a booleans-ONLY health roll-up to each public client
// entry, shared verbatim by client_list (MCP) and GET /api/clients (REST) so the
// two faces return identical fields. schedulerRunning is now the PER-CLIENT
// scheduler flag (state.scheduler.enabled, default-ON == enabled !== false), read
// INSIDE that client's own withClient(clientRoot(id), ...) scope - NOT the single
// process-global timer stamped identically on every row (the old lie: a client the
// operator stopped still showed the global "running" flag, and a never-toggled
// default-on client showed off). actionBlocked is the per-client Meta-368 breaker,
// read in the same scope so no read crosses into another client's subtree. A
// corrupt/unreadable per-client state.json degrades to schedulerRunning:true
// (default-ON) + actionBlocked:false without aborting the rest of the list. NEVER
// leak blockedUntil/reason/fbTraceId or any secret - booleans only.
function withHealthRollup(clients) {
  ...
  return clients.map((c) => {
    ...
    // US-MC-10: default-ON, so a corrupt/unreadable state degrades to true (the
    // client keeps scheduling unless it explicitly opted out) - matches how
    // schedulerEnabledHere treats a missing flag.
    ...
    try {
      ...
      const summary = withClient(clientRoot(c.id), () => ({
        ...
        // Per-client scheduler flag, read in THIS client's scope (default-ON).
        schedulerRunning: schedulerEnabledHere(loadState()),
      ...
      schedulerRunning = true; // default-ON degrade (see above)
    }
