...
// (this client's own, never a hardcoded 'pendpost' - the leak that emailed the bondigoo
...
export async function radarAgentScan({ clientId, queryId, actor, cadence, scope, target, sources, wait = true } = {}) {
  ...
  try {
    for (let i = 0; i < lanes.length; i++) {
      ...
      // STARVES signal research (it did: a single-lane bondigoo scan spent its whole budget on GEO
...
// pollution under bondigoo) has no config edit that clears it, because this is STATE not config.
