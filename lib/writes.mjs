...
import { autoApproveDecision, AUTO_APPROVE_ACTOR, AUTO_APPROVE_DEFAULTS } from './auto-approve.mjs';
...
export async function revokeAutoApprovals({ actor, clientId } = {}) { // eslint-disable-line no-unused-vars
  ...
  try {
    ...
    return { ok: true, reverted: reverted.length, posts: reverted };
  } catch (err) {
    return errorBody(err.code || 'engine_failure', err.message);
  }
}
...
export async function resumeLane({ platform, actor } = {}) {
  ...
  return { ok: true, platform: p, cleared, released, published, stillDepleted, pending };
}
...
export async function runRadarScan({ clientId, queryId, cadence } = {}) {
  ...
  state.radar.geo.comparisonBacklog = comparisonBacklog(state.radar.signals, state.radar.geo.dismissedBacklog, state.radar.geo.footprint);
  saveState();
...
export async function radarAgentScan({ clientId, queryId, actor, cadence, scope, target, sources, wait = true } = {}) {
  ...
  // The GEO buying-question check folds into the FIRST lane only (near-zero extra cost, no N-fold
  // duplicate checks). The SCHEDULER packs exactly like a manual scan (Wave 1 R1, 2026-09-04): the
  ...
  try {
    for (let i = 0; i < lanes.length; i++) {
      ...
      const laneGeo = i === 0 ? geoContext : null; // GEO folds into the first lane only
      ...
      const laneRun = await researchWithRetry(job, {
        ...
        prompt: radarScanPrompt(queries, maxPerRun, boundClientId, laneSources, laneGeo, getPosting().locale || null, radar.brand || null, radar.lookbackDays, perLaneMs),
...
export async function listRadar({ clientId, source, action, minScore, queryId, view } = {}) {
  ...
  const geo = {
    ...
    shareOfVoice: shareOfVoice(state.radar.signals || [], state.radar.geo?.footprint || []),
  ...
  try {
    ...
    // The SAME derived arming dailyAgentScan/dailyRadarScan gate on: at least one enabled
    // cadence:'daily' query (plus, for the agent, a connected provider).
...
async function queueRadarReplyCore({ clientId, campaign, signalUrl, source, externalId, parentExternalId, text, executionMode, actor, confirm } = {}) {
  ...
  try {
    ...
    const lintOk = lintGateOk(draft, src, policy);
    // The draft-for-review hold (the operator's "Wird automatisch gepostet" tap): the spawn
    // window armed the fence WITH holdApproval, so the policy stands down entirely - the
    // human asked to READ this draft before it goes out, and auto-approving it under them
    // would be the exact surprise the tap exists to prevent. Not a refusal row either: the
    // policy was never in play.
    const policyArmed = !draftFenceHoldsApproval() && radar.enabled === true && policy.enabled === true && laneAllowed && scoreGateOk;
    if (policyArmed && lintOk && !foreign.length) {
      ...
      // The policy WOULD have auto-posted this reply, but a CONTENT gate held it back. The
      // digest's autonomy report (R2, dim-5 AU3) reports these by reason class: a foreign
      // link (spec 42 §4.3, the fence that removes the payoff) or a brand-lint error. It
      // stays pending for a human either way - this row just makes the refusal legible.
      const reason = foreign.length ? 'foreign_link' : (!lintOk ? 'lint' : null);
