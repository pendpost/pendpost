...
import { RADAR_SOURCES, RADAR_REPLY_SOURCES, RADAR_COPY_DRAFT_SOURCES, RADAR_CAPABILITIES, RADAR_SOURCE_SCOPE, RADAR_EXCERPT_MAX, RADAR_AGENT_FOLLOWUP_SOURCES, effectiveRadarCapabilities, radarReplySources, radarCopyDraftSources, effectiveRadarSources, agentResearchSources, runLaneRadar, scoreInto, normalizeSignal, sortByIntent, mergeSignals, pruneSeen, signalKey, isExcluded, replyContextFrom, resolveReplyPermalink, stampFollowup, stampFollowupEngager, normAuthor, needsAgentFollowupCheck, comparisonBacklog, footprintMentionRate, shareOfVoice } from './radar.mjs';
...
// timeout (a wall-clock fact, promoted to partial instead), stopped (the operator's own act),
...
export async function radarAgentScan({ clientId, queryId, actor, cadence, scope, target, wait = true } = {}) {
  ...
  if (scope === 'geo') {
    ...
    const geoJob = newJob(null, providerId, [], 'geo');
  ...
  if (scope === 'draft-one') {
    ...
    const doJob = newJob(null, providerId, [src], 'draft-one');
  ...
  if (scope === 'followup') {
    ...
    const fuJob = newJob(null, providerId, [...new Set(targets.map((t) => t.source))], 'followup');
  ...
  if (actor.trim() === 'scheduler') {
    ...
    const spent = (state.radar.jobs || []).filter((j) => j.reason !== 'limit' && (Date.parse(j.startedAt) || 0) > Date.now() - 24 * 3600 * 1000).length;
  ...
  // is even blocked for the agent) and, because lanes run sequentially under the 10-min job cap,
  ...
  const scanLanes = researchSources.length ? researchSources : scanSources;
  ...
  // duplicate checks). GATED: ON for a manual scan (the operator chose to spend and wants the
  // resilience), OFF on the SCHEDULER path, because the daily budget counts JOBS not
  // spawns - N (up to 2N with retry) unattended spawns behind a "1 job/day" label would be a silent
  // nightly multiplier. A single-source scan => today's one combined spawn, unchanged.
  // (agent.perSourceIsolation was a DEAD read here: never in the validator or the defaults, so no
  // config could ever set it and the scheduler was ALWAYS single-spawn - the code now says so.)
  const perSource = actor.trim() !== 'scheduler';
  const lanes = perSource && scanLanes.length > 1 ? scanLanes.map((s) => [s]) : [scanLanes];
  // Divide the hard 10-min budget across the lanes (floored so no lane is uselessly short, capped at
  // the safety const). One lane => the whole budget, exactly as before.
  const perLaneMs = lanes.length > 1
    ? Math.min(AGENT_TIMEOUT_MS, Math.max(120_000, Math.floor(AGENT_TIMEOUT_MS / lanes.length)))
    : AGENT_TIMEOUT_MS;
  const sourceResults = {};
  ...
  try {
    for (let i = 0; i < lanes.length; i++) {
      ...
      const laneRun = await researchWithRetry(job, {
        ...
        prompt: radarScanPrompt(queries, maxPerRun, boundClientId, laneSources, laneGeo, getPosting().locale || null, radar.brand || null),
  ...
  //  - no lane ok but signals in -> PARTIAL ('partial_timeout' when a lane timed out): promote what
  ...
  const ingested = runningRadarJob(radarState())?.accepted || 0;
  ...
  const partialReason = researchOk ? 'partial' : ((laneErrors.includes('timeout') || (run && run.error === 'timeout')) ? 'partial_timeout' : 'partial');
  ...
  const finished = finishJob(job.id, {
    ...
    reason: stoppedMid ? 'stopped' : ((researchOk && !anyFailed) ? null : partial ? partialReason : (run?.error || laneErrors[0] || 'failed')),
    exitCode: run?.exitCode ?? null,
...
export async function listRadar({ clientId, source, action, minScore, queryId, view } = {}) {
  ...
  try {
    const { nextDueDailyAt, agentJobsToday } = await import('./radar-sweep.mjs');
    const tz = posting.defaultTimezone || 'UTC';
...
export async function queueRadarReply({ clientId, campaign, signalUrl, source, externalId, parentExternalId, text, executionMode, actor, confirm } = {}) {
  void clientId; // per-call client scoping is bound by withClient at the call site
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  if (confirm !== true) {
