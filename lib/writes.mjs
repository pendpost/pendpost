...
import { activeRoot, withClient, boundClientId as alsClientId } from './context.mjs';
import { clientRoot, readRegistry } from './multi-client.mjs';
import { listClients, archiveClient } from './clients.mjs';
import { loadManifest, loadPlanStore, postRev, postContentHash, POST_CONTENT_FIELDS, ALL_PLATFORM_ID_FIELDS, PLATFORM_ID_FIELDS, resolveMediaPath, resolveMediaItems, probedChecks, postMediaPaths, postNeedsMedia, findCampaign, awaitingApproval } from './plans.mjs';
import { POLL_LANE_LIMITS } from './poll.mjs';
import { CAROUSEL_LANE_LIMITS, CAROUSEL_MIN_ITEMS, carouselItems, carouselItemKind, carouselUnsupported, carouselUnsupportedCode } from './carousel.mjs';
import { CAPTION_LIMITS as GENERIC_CAPTION_LIMITS } from './caption.mjs';
import { effectivePublicUrl, effectiveSlideUrl, deadMediaUrlDiagnosis, MEDIA_URL_DEAD_MARK } from './public-media.mjs';
import { mutatePlan, withPlanLock, resolvePlanPath } from './planWrite.mjs';
...
import {
  runAgentJob, isJobRunning, killJob, AGENT_SCAN_TOOLS, AGENT_DRAFT_TOOLS, AGENT_TIMEOUT_MS, planAgentScanLanes,
  beginDraftFence, endDraftFence, draftTargetAllowed, draftFenceHoldsApproval, foreignLinksIn, AGENT_COMPARISON_TOOLS,
  AGENT_GEO_TOOLS, scrubCredential, normalizeTail,
  followupTargetAllowed, beginFollowupFence, endFollowupFence, AGENT_FOLLOWUP_TOOLS,
} from './agent-runner.mjs';
import { radarScanPrompt, radarDraftPrompt, radarComparisonPrompt, radarGeoPrompt, radarFollowupPrompt, AGENT_MAX_PER_RUN_DEFAULT } from './radar-prompt.mjs';
import { notifyRadarScanDone } from './notify.mjs';
import { inFlight, appendActivity, runDueExclusive } from './scheduler.mjs';
import { probeMedia, specChecks, rendersDir, sanitizeAssetName, AV_SYNC_REASON } from './assets.mjs';
import { extractDefaultCoverGuarded } from './covers.mjs';
import { accountStatus } from './accounts.mjs';
import { loadState, saveState, isMetaBlocked, persistRedditWarmth, getLaneBlock, clearLaneBlock } from './state.mjs';
import { stampEngager } from './engagers.mjs';
import { allPostPlatforms, allLanes } from './drivers/interface.mjs';
import { resolveMode, platformEnabled, resolveEnginePath, isMockPlatformId } from './mode.mjs';
...
import { IMAGE_LANES, effectiveLaneText } from './capabilities.mjs';
import { RADAR_SOURCES, RADAR_REPLY_SOURCES, RADAR_COPY_DRAFT_SOURCES, RADAR_CAPABILITIES, RADAR_SOURCE_SCOPE, RADAR_EXCERPT_MAX, RADAR_AGENT_FOLLOWUP_SOURCES, effectiveRadarCapabilities, radarReplySources, radarCopyDraftSources, effectiveRadarSources, radarSourceSkipped, radarQueryLanes, agentResearchSources, runLaneRadar, mergeRadarSourceRows, scoreInto, normalizeSignal, sortByIntent, mergeSignals, pruneSeen, signalKey, isExcluded, replyContextFrom, resolveReplyPermalink, stampFollowup, stampFollowupEngager, normAuthor, needsAgentFollowupCheck, comparisonBacklog, footprintMentionRate, shareOfVoice, withinLookback } from './radar.mjs';
...
import { refuseReviewerActor, REVIEWER_TRUST } from './reviewers.mjs';
import { autoApproveDecision, radarReplyAutoApproveDecision, AUTO_APPROVE_ACTOR, AUTO_APPROVE_DEFAULTS } from './auto-approve.mjs';
...
// The lanes whose engines publish the single-image `image` TYPE now live in one place -
// IMAGE_LANES in capabilities.mjs, the single source of truth the composer format select
// is twin-guarded against - so the two can never drift (the class of bug that stranded 7
// live X image posts). Every OTHER lane blocks it in platformValidate below, so the
// composer can never strand the type on a lane whose engine cannot publish it.
...
// Spec 50 §7.10: where a post CAME FROM, for any later audit. A closed set, because
// an open provenance string is a claim nobody can verify - absence is the default
// ("authored in the calendar") and 'radar-theme' is the one machine-set marker
// (engageApiPost, the "Respond for me" policy). Create-only: it is not in
// POST_CONTENT_FIELDS, so no edit path can rewrite a post's own history.
const POST_ORIGINS = ['radar-theme'];
...
function requireActor(actor, trusted = false) {
  ...
  // Spoof guard (spec 48 §3.2): the `reviewer:` actor namespace is reserved and
  // minted server-side from a review token. A body-supplied `reviewer:...` actor
  // is refused on EVERY operator write face; only the review ingest, which passes
  // the process-private REVIEWER_TRUST marker through to setApproval, is trusted
  // to carry a reviewer actor.
  if (!trusted) {
    const reserved = refuseReviewerActor(actor);
...
export function validateFieldValues(fields) {
  ...
  if (fields.radarReplyTo !== undefined && fields.radarReplyTo !== null) {
    ...
    // parentExternalId (R11/N2): a DEEPER reply target than externalId - the author's
    // follow-up comment id (reddit t1_ fullname, mastodon status id, bluesky at:// uri). When
    // present the source engine threads the reply UNDER it instead of the thread root, so a
    // round-2 answer continues the conversation. Same bounded opaque charset as externalId; the
    // engine falls back to externalId when it is absent (an honest root reply, never a broken
    // target). queueRadarReply vets it against the captured author-reply id before it lands here.
    if (r.parentExternalId !== undefined && r.parentExternalId !== null && (typeof r.parentExternalId !== 'string' || !/^[A-Za-z0-9_:/.@%-]{1,300}$/.test(r.parentExternalId))) {
      return errorBody('invalid_input', 'radarReplyTo.parentExternalId must be a bounded opaque id ([A-Za-z0-9_:/.@%-], 1-300 chars, or null)');
    }
    ...
    for (const [k, max] of [['author', 120], ['community', 120], ['excerpt', RADAR_EXCERPT_MAX], ['reason', RADAR_EXCERPT_MAX]]) {
    ...
    if (r.intentScore !== undefined && r.intentScore !== null && !(typeof r.intentScore === 'number' && r.intentScore >= 0 && r.intentScore <= 100)) {
      return errorBody('invalid_input', 'radarReplyTo.intentScore must be a number 0-100 (display-only context)');
    }
  ...
  // liAuthor: LinkedIn per-post author target (organization Company Page | member personal
  // profile). Value-validated only, like every other lane-specific field; readiness (member
  // needs a captured person URN) is a platform_validate concern, not an input error.
  if (fields.liAuthor !== undefined && fields.liAuthor !== null && fields.liAuthor !== '' && !['organization', 'member'].includes(fields.liAuthor)) {
    return errorBody('invalid_input', 'liAuthor must be one of organization|member');
  }
  ...
  // origin: the spec 50 §7.10 provenance marker. Absent (or empty) is the default -
  // the post was authored in the calendar; 'radar-theme' says the "Respond for me"
  // policy wrote it out of a Radar theme. A closed set, same shape as the enum above.
  if (fields.origin !== undefined && fields.origin !== null && fields.origin !== '' && !POST_ORIGINS.includes(fields.origin)) {
    return errorBody('invalid_input', `origin must be one of ${POST_ORIGINS.join('|')}`);
  }
...
export async function createPost({ campaign, post, actor } = {}) {
  ...
  // R6b receipt: the gate's own change report ({fixes, findings}) is threaded into the
  // response below instead of discarded, so the operator (Composer line) and the agent
  // (MCP envelope) see WHAT was auto-fixed. The mutation itself is byte-identical.
  const humanized = humanizeFields(post, POST_PROSE_FIELDS, getContentLocale());
  ...
  try {
    const created = await mutatePlan(found.absPlan, (plan) => {
      ...
      const fresh = {
        ...
        // Spec 50 §7.10: the provenance marker ('radar-theme') the auto-engage policy
        // stamps on a post it wrote out of a Radar theme. Absent on an owner-authored
        // post. Dropped on create if NOT whitelisted here - the same create-drop trap
        // that hid it in the first place (spec 50 §5.5 P6-3).
        origin: post.origin || undefined,
        ...
        // LinkedIn per-post author target: persist only the non-default 'member' so
        // absence == Company Page (keeps the approved-copy hash stable, mirroring
        // publishAsDraft - an explicit 'organization' normalizes back to absence).
        liAuthor: post.liAuthor === 'member' ? 'member' : undefined,
    ...
    try {
      ...
      if (policy && policy.enabled && created.createdBy !== AUTO_APPROVE_ACTOR) {
        const decision = autoApproveDecision(created, policy, campaign);
        if (decision.approve) {
        ...
        } else if (String(decision.reason || '').startsWith('brand_lint')) {
          // The digest's autonomy report (R2, dim-5 AU3) reports refusals BY REASON CLASS.
          // Only a post that MATCHED scope but failed the lint gate is a real "held back"
          // event worth surfacing - an out-of-scope post is simply not covered, never nagged.
          appendActivity({ campaign, postId: post.id, platform: null, action: 'auto-approve-refused', ok: true, errorCode: null, errorMessage: null, reason: 'lint', lateMin: null, actor: AUTO_APPROVE_ACTOR });
        }
      }
    } catch { /* leave the post a draft - never block creation on the policy */ }
    // Additive humanizer receipt: only present when the gate touched something or has
    // advisory findings, so a clean save's envelope is byte-identical to before.
    const receipt = humanized.fixes.length || humanized.findings.length
      ? { humanizer: { fixes: humanized.fixes, findings: humanized.findings } }
      : {};
    return { ok: true, post: created, autoApproved, rev: finalRev, ...receipt };
  } catch (err) {
...
export async function updatePost({ campaign, postId, ifRev, fields, actor } = {}) {
  ...
  if (fieldErr) return fieldErr;
  // Always-on humanizer gate (Layer A) on the edited prose fields, same curated subset as
  // create. R6b receipt: the change report rides the response (see createPost).
  const humanized = humanizeFields(fields, POST_PROSE_FIELDS, getContentLocale());
  const found = findPlanEntry(campaign);
  if (found.error) return found.error;
  ...
  try {
    const updated = await mutatePlan(found.absPlan, (plan) => {
      ...
      // An operator edit is remediation: clear the local failure-cap stamp
      // (lib/publish-hold.mjs) so the fixed post re-enters the fire loop. Whole-file
      // mutatePlan write, so the engines' field-merge cannot resurrect it.
      delete post.publishHold;
      delete post.publishRetry;
    ...
    const receipt = humanized.fixes.length || humanized.findings.length
      ? { humanizer: { fixes: humanized.fixes, findings: humanized.findings } }
      : {};
    return { ok: true, post: updated, rev: postRev(updated), ...receipt };
  } catch (err) {
    return errorBody(err.code || 'engine_failure', err.message);
  }
}
...
export async function deletePost({ campaign, postId, force, actor } = {}) {
  ...
  try {
    ...
    // Publish-evidence gate FIRST: EVERY lane's minted platform id counts, derived
    // from the shared PLATFORM_ID_FIELDS registry (lib/plans.mjs) - never a
    // hardcoded subset a newer lane can fall through. force means "I accept that
    // an already published copy stays live on the platforms".
    const evidence = ALL_PLATFORM_ID_FIELDS.filter((k) => post[k]);
    if ((post.status === 'posted' || evidence.length) && force !== true) {
      return errorBody('invalid_input', `post has publish evidence (${post.status === 'posted' ? 'posted' : evidence.join(', ')}) - anything still scheduled platform-side is cancelled by the delete, but an already published copy stays live; pass force: true if you really mean it`);
    }
    // "Delete always works" (owner directive 2026-08-18): a natively scheduled
    // platform object (FB scheduled post, YouTube publishAt, Mastodon queue
    // entry, WP future, Ghost scheduled, Discord guild event) no longer refuses
    // the delete with an "unschedule first" ceremony - pendpost itself cancels
    // the platform object(s) BEFORE removing the row, exactly the order
    // unschedulePost proved. A cancel failure returns verbatim with the post
    // intact: the one remaining refusal, an honest engine_failure.
    const handoffs = deleteSideHandoffs(post);
    if (handoffs.length) {
      const cancelErr = await cancelAllNative(handoffs, found.absPlan, postId, actor.trim());
      if (cancelErr) return cancelErr;
    }
    const removed = await mutatePlan(found.absPlan, (freshPlan) => {
      const idx = (freshPlan.posts || []).findIndex((p) => p.id === postId);
      if (idx < 0) throw Object.assign(new Error(`unknown post ${postId} in ${campaign}`), { code: 'unknown_post' });
      return freshPlan.posts.splice(idx, 1)[0];
    });
    appendActivity({ campaign, postId, platform: handoffs.map((h) => h.lane).join('+') || null, action: 'post-delete', ok: true, errorCode: null, errorMessage: null, lateMin: null, actor: actor.trim() });
    ...
    return { ok: true, deleted: { id: removed.id }, nativeCancelled: handoffs.length ? handoffs.map((h) => h.lane).join('+') : null };
  } catch (err) {
    return errorBody(err.code || 'engine_failure', err.message);
  }
}
...
async function setApproval({ campaign, postId, actor, note, verdict, force = false, [REVIEWER_TRUST]: reviewerTrusted = false }) {
  ...
  const idErr = requireIds(campaign, postId) || requireActor(actor, reviewerTrusted === true);
  if (idErr) return idErr;
  const found = findPlanEntry(campaign);
  if (found.error) return found.error;
  // Enforced pre-flight gate: an approve that would queue a post carrying a
  // content-integrity blocker on any targeted lane (a caption over the cap, an
  // A/V-desynced reel, a malformed poll/carousel, ...) is refused with the blockers so
  // the owner fixes them at approve time - not at the silent scheduled fire. Run on the
  // READ-MODEL post (loadPlanStore attaches media/derived fields the raw plan row lacks)
  // and BEFORE the lock (a read needs none). needsSetup lanes are excluded (laneBlockers):
  // a not-yet-connected lane is environment state, not a content fault, and unauth already
  // fails closed downstream. force:true overrides (the publish fence is the fail-closed
  // backstop); an override is recorded on the activity row below. Reject is never gated.
  // When force overrides real blockers, the list is kept for the activity note.
  let forcedOver = null;
  if (verdict === 'approved') {
    const view = (loadPlanStore().campaigns.find((x) => x.id === campaign)?.posts || []).find((p) => p.id === postId);
    if (view) {
      const blocked = laneBlockers(view, preflightContext(view));
      if (blocked.length) {
        if (force !== true) {
          const summary = `post ${postId} is not ready: ${blocked.map((b) => `${b.platform} - ${b.problems[0]}`).join('; ')}`;
          return errorBody('not_ready', summary, { blocked });
        }
        forcedOver = blocked;
      }
    }
    // Approve-time dead-URL gate (2026-08-26, third mirror-404 incident): a post whose
    // public media URL(s) are ALREADY deterministically dead (404/410, or a 2xx HTML
    // catch-all) must not be approvable into a guaranteed publish failure - the mirror
    // (web/public/media/ + prod deploy) is a manual step and this is the human moment to
    // catch a missed one. Same probe seams as the pre-fire fail-safe + guarded retry
    // (postPublicMediaUrls -> deadMediaUrlDiagnosis), so approve, dry-run and live fire
    // can never disagree. Fail-open on transient/inconclusive probes (403/5xx/timeout
    // proceed); zero cost for every post without an instagram/pinterest public URL.
    // Probed in PARALLEL so a slow host costs one timeout window, not one per slide.
    // force:true overrides like the blocker gate above, recorded on the activity row.
    // Auto-approve (createPost -> setApproval under the policy actor) flows through here
    // too: a dead URL simply leaves the post a draft instead of scheduling a doomed fire.
    const rawApprovee = rawPostOf(campaign, postId);
    const mediaUrls = rawApprovee ? postPublicMediaUrls(rawApprovee) : [];
    if (mediaUrls.length) {
      const diags = await Promise.all(mediaUrls.map((u) => deadMediaUrlDiagnosis(u)));
      const deadAt = diags.findIndex((d) => d);
      if (deadAt !== -1) {
        const diag = `${diags[deadAt]} (fix: node ops/mirror-media.mjs, then approve again)`;
        if (force !== true) return errorBody('media_url_dead', diag, { url: mediaUrls[deadAt] });
        forcedOver = [...(forcedOver || []), { platform: (rawApprovee.platforms || []).filter((p) => p === 'instagram' || p === 'pinterest').join('+') || 'media', problems: [diag] }];
      }
    }
  }
  try {
    const result = await mutatePlan(found.absPlan, (plan) => {
      ...
      if (verdict === 'approved') {
        ...
        delete post.rejectedContentHash;
        // Restore the "approved => fireable" invariant. createPost births a post
        // status:'planned', but some plan writers (e.g. the reel builders) seed an entry
        // status:'draft' directly, and the native engines only schedule a plannable status.
        // An approved-but-draft post would then be dispatched by the scheduler yet silently
        // skipped by the engine (the reason-less "engine returned no result"). Promote it
        // here so approval alone makes it fireable; a live/terminal status is left untouched.
        if (post.status === 'draft') post.status = 'planned';
      } else {
    ...
    const overrideNote = forcedOver
      ? `force-approved over ${forcedOver.length} blocker(s): ${forcedOver.map((b) => `${b.platform} - ${b.problems[0]}`).join('; ')}`
      : null;
    appendActivity({ campaign, postId, platform: null, action: verdict === 'approved' ? 'approve' : 'reject', ok: true, errorCode: null, errorMessage: overrideNote, lateMin: null, actor: actor.trim() });
    ...
    return errorBody(err.code || 'engine_failure', err.message, err.extra || {});
  }
}
...
// ---------- the autonomy ledger reads + revoke sweep (ux-audit R7) ----------

// The candidate set for the auto-approve dry-run + a shared honest predicate for
// "a draft YOU authored" - the posts the owner would review by hand. Excludes the
// auto-approve machinery's own output (createdBy is the policy actor) and Radar
// replies (a separate autonomy door that autoApproveDecision always refuses), so a
// preview reads over the planner drafts the policy is actually about, never inflated
// or deflated by posts it could never legitimately cover.
function isOwnerAuthoredDraft(post) {
  return post && post.createdBy !== AUTO_APPROVE_ACTOR && !post.radarReplyTo;
}

// AU5 dry-run: replay the deterministic, side-effect-free autoApproveDecision
// (lib/auto-approve.mjs) over the most recent authored drafts, so the owner can
// enable a rung against EVIDENCE ("this scope matched 14 of your last 14 drafts")
// instead of hope. `enabled` is forced true so the preview works BEFORE the toggle
// is on; every other axis (platforms/campaigns/types/lint) is the stored policy as
// currently edited. An empty platforms list scores 0 - the B5 fail-closed behaviour
// made legible rather than surprising. Reads only; never writes, never publishes.
export function autoApproveDryRun({ limit = 20 } = {}) {
  const n = Number.isInteger(limit) && limit > 0 && limit <= 100 ? limit : 20;
  const stored = getPosting().autoApprove || {};
  const preview = { ...AUTO_APPROVE_DEFAULTS, ...stored, enabled: true };
  const { campaigns } = loadPlanStore();
  const drafts = [];
  for (const c of campaigns) {
    for (const p of (c.posts || [])) if (isOwnerAuthoredDraft(p)) drafts.push(p);
  }
  drafts.sort((a, b) => (Date.parse(b.createdAt || b.scheduledAt || 0) || 0) - (Date.parse(a.createdAt || a.scheduledAt || 0) || 0));
  const recent = drafts.slice(0, n);
  let matched = 0;
  for (const p of recent) {
    if (autoApproveDecision(p, preview, p.campaign).approve) matched += 1;
  }
  return { ok: true, matched, total: recent.length, limit: n };
}

// The count of live policy-approvals the revoke sweep would unwind: posts blessed
// by the auto-approve policy actor (draft OR Radar reply, both stamped
// policy:auto-approve) that have NOT yet published. Drives the confirm on the
// revoke ("also return N scheduled posts to review?").
function countRevocable() {
  const { campaigns } = loadPlanStore();
  let n = 0;
  for (const c of campaigns) {
    for (const p of (c.posts || [])) {
      if (p.approvalBy === AUTO_APPROVE_ACTOR && p.approval === 'approved' && p.derivedState !== 'posted') n += 1;
    }
  }
  return n;
}

// The one read the autonomy ledger needs beyond config_get + pendpost_health: the
// dry-run count for the current scope, and how many live policy-approvals a revoke
// would unwind. Pure derivation over plans; no new engine state.
export function autonomyStatus({ limit = 20 } = {}) {
  const dry = autoApproveDryRun({ limit });
  return { ok: true, dryRun: { matched: dry.matched, total: dry.total, limit: dry.limit }, revocable: countRevocable() };
}

// AU4 revocation-that-unwinds: disabling a policy stops FUTURE approvals; this
// returns the BACKLOG - every not-yet-published post the policy already approved -
// to review. A filtered setApproval sweep over approvalBy === policy:auto-approve
// and status !== posted: it CLEARS the approval (back to a pending draft the owner
// re-decides), never fabricates a reject. Covers both auto-approved planner drafts
// AND auto-posted Radar replies (both carry the same policy actor). Owner-gated
// like every autonomy write; dev:live stays read-only. De-escalation only - it can
// never approve or publish anything.
export async function revokeAutoApprovals({ actor, clientId } = {}) { // eslint-disable-line no-unused-vars
  if (isDevReadonly()) return errorBody('dev_readonly', 'returning auto-approved posts to review is disabled in read-only dev (dev:live)');
  const actErr = requireActor(actor);
  if (actErr) return actErr;
  if (actor.trim() !== 'owner') {
    return errorBody('invalid_input', 'only the owner can revoke auto-approvals (autonomy is owner-authorized)');
  }
  try {
    const { campaigns } = loadPlanStore();
    const reverted = [];
    for (const c of campaigns) {
      const eligibleIds = (c.posts || [])
        .filter((p) => p.approvalBy === AUTO_APPROVE_ACTOR && p.approval === 'approved' && p.derivedState !== 'posted')
        .map((p) => p.id);
      if (!eligibleIds.length) continue;
      const found = findPlanEntry(c.id);
      if (found.error) continue;
      const done = await mutatePlan(found.absPlan, (plan) => {
        const ids = [];
        for (const post of (plan.posts || [])) {
          if (!eligibleIds.includes(post.id)) continue;
          // Re-check on the RAW post under the lock (the read model could be a tick stale):
          // only sweep a still-live policy approval that has not since published.
          if (post.approvalBy !== AUTO_APPROVE_ACTOR || (post.approval || 'draft') !== 'approved' || post.status === 'posted') continue;
          delete post.approval; // back to a pending draft (normalizePost fails it closed to 'draft')
          delete post.approvalBy;
          delete post.approvalAt;
          delete post.approvalNote;
          delete post.approvedContentHash;
      ...
      for (const id of done) {
        appendActivity({ campaign: c.id, postId: id, platform: null, action: 'auto-approve-revoke', ok: true, errorCode: null, errorMessage: null, lateMin: null, actor: actor.trim() });
        reverted.push({ campaign: c.id, id });
      }
    }
    // Spec 50 row 17: revoke reaches "Respond for me" too. Queued rows HOLD, and every row
    // inside its 15-minute grace window - the ones the owner could still have called back -
    // becomes a confirm ask, so nothing already decided goes out unread. Best-effort and
    // dynamic (lib/engage-asks.mjs imports engageState from this module): a project that
    // never turned engage on gets a byte-unchanged revoke.
    let engage = null;
    try {
      const { holdForRevoke } = await import('./engage-asks.mjs');
      engage = holdForRevoke();
      // ---- P6 BLOCK (spec 50 row 17): make the hold DURABLE -------------------------------
      // holdForRevoke marks the queued rows `waitingOn:'paused'`, but the pacer rewrites that
      // field on its very next pass: measured, a held row released and reached its executor
      // sixty seconds after the revoke. The flag the pacer actually honours is the config one
      // (D18's resumable pause, the same one engage_pause writes), so revoke sets it. Not Off:
      // the owner resumes from the ledger's own Resume control when they are ready.
      const { getConfig: readCfg, setConfig: writeCfg } = await import('./config.mjs');
      const mode = ((readCfg().posting || {}).radar || {}).engage || {};
      if (mode.mode && mode.mode !== 'off' && mode.paused !== true) {
        const paused = writeCfg({ ifRev: readCfg().rev, actor: 'owner', set: { posting: { radar: { engage: { paused: true } } } } });
        if (paused && paused.ok && engage && engage.ok) engage.paused = true;
      }
      // ---- end P6 BLOCK ---------------------------------------------------------------------
    } catch { /* no engage store - the planner sweep above is the whole answer */ }
    return { ok: true, reverted: reverted.length, posts: reverted, ...(engage && engage.ok ? { engage: { held: engage.held, asks: engage.asked, paused: engage.paused === true } } : {}) };
  } catch (err) {
    return errorBody(err.code || 'engine_failure', err.message);
  }
}
...
// The DELETE-side cancel list: everything nativeHandoffs cancels for
// unschedule/reschedule, PLUS the Discord guild scheduled event (dcEventId,
// spec 26). The event is deliberately NOT in nativeHandoffs: un/re-scheduling
// moves a post's PUBLISH, and the guild event is a separate calendar object
// with its own intent (dcEvent) that should survive a schedule move. Deleting
// the plan row is different - the row is the ONLY record and cancel path
// pendpost holds for that event, so an upcoming event would otherwise fire
// orphaned. The engine's delete-event verb counts an already-gone event (404,
// e.g. auto-removed after completion) as cancelled, so a past event never
// blocks the delete. Unlike nativeHandoffs this branch is status-agnostic:
// even a posted row's event orphans without its row.
function deleteSideHandoffs(post) {
  const handoffs = nativeHandoffs(post);
  if (post.dcEventId) handoffs.push({ lane: 'discord', script: 'scripts/discord-social.mjs', id: post.dcEventId, field: 'dcEventId', command: 'delete-event' });
  return handoffs;
}
...
// `slotSlip` is INTERNAL (sweepSlotSlip only, never a tool/API face): the move is
// the same reschedule, but the activity row must say WHY the slot moved - a
// machine-initiated "slot slip", not an operator reschedule.
// The PUBLIC media URL(s) a post would hand to a platform's server-side fetch (IG feed
// image, IG/pinterest carousel image slides, pinterest image pin) - the URLs whose
// liveness the pre-fire fail-safe + the readiness blocker + the guarded retry all probe.
// Empty for every post that does NOT publish from a public URL (no instagram/pinterest
// lane, a video/reel/text post, or no derivable URL). Resolution goes through the SAME
// effectivePublicUrl/effectiveSlideUrl precedence the engines use, so all three surfaces
// and the live publish can never disagree on which URL is checked.
function postPublicMediaUrls(post) {
  const lanes = post.platforms || [];
  if (!lanes.includes('instagram') && !lanes.includes('pinterest')) return [];
  const posting = getPosting();
  if (post.type === 'carousel') {
    return carouselItems(post)
      .filter((it) => carouselItemKind(it) === 'image')
      .map((it) => effectiveSlideUrl(it, posting))
      .filter((u) => typeof u === 'string' && u);
  }
  if (post.type === 'image') {
    const u = effectivePublicUrl(post, posting);
    return u ? [u] : [];
  }
  return [];
}

// The raw (on-disk) post for a campaign/postId, or null. Used where a surface needs
// fields the slim read-model omits (mediaItems / imageUrl for the media-URL probe).
function rawPostOf(campaign, postId) {
  const found = findPlanEntry(campaign);
  if (found.error) return null;
  try {
    const plan = JSON.parse(fs.readFileSync(found.absPlan, 'utf8'));
    return (plan.posts || []).find((p) => p.id === postId) || null;
  } catch { return null; }
}

export async function reschedulePost({ campaign, postId, scheduledAt, confirm, actor, slotSlip = false } = {}) {
  const idErr = requireIds(campaign, postId) || requireActor(actor);
  if (idErr) return idErr;
  ...
  try {
    ...
    // Guarded retry (B3): a "retry now" (a slot at/before now) on a post parked by a
    // PROBE-CONFIRMED dead media URL should not silently re-arm into an immediate re-fail -
    // that was the app's dead-end "Erneut versuchen" loop. Re-probe; if the URL is STILL
    // deterministically dead, refuse with the diagnosis so the operator re-mirrors first. A
    // FUTURE reschedule (runway to fix the mirror) is always allowed, and a non-mirror hold
    // is untouched. Fail-open: a transient/inconclusive probe re-arms as before.
    const heldDead = post.publishHold && typeof post.publishHold.message === 'string' && post.publishHold.message.includes(MEDIA_URL_DEAD_MARK);
    if (heldDead && (Date.parse(scheduledAt) || 0) <= Date.now()) {
      for (const url of postPublicMediaUrls(post)) {
        const diag = await deadMediaUrlDiagnosis(url);
        if (diag) return errorBody('media_url_dead', diag);
      }
    }
    ...
    const updated = await mutatePlan(found.absPlan, (freshPlan) => {
      ...
      // Rescheduling is the operator's "retry now" verb for a post parked under the
      // local failure cap (lib/publish-hold.mjs) - even a same-time reschedule
      // re-arms it. Safe to delete here: mutatePlan writes the whole file, so the
      // engines' field-merge cannot resurrect the stale hold. The transient-retry
      // backoff schedule is reset too, so "retry now" fires immediately.
      delete p.publishHold;
      delete p.publishRetry;
      return p;
    });
    // The CLOUD twin of the local hold-clear above. A prior cloud attempt leaves
    // relics in state (cloudFailures = a reconciled failure, cloudAccepted /
    // cloudRetriggered = the push round-trip). Because a rescheduled post is still
    // OPEN (approved + unposted), the openness-keyed pruneCloudMarkers can't reap
    // them - so a stale failure held the health dot RED with a phantom "Cloud: 1
    // nicht zugestellt" for a post that isn't even due yet. Rescheduling = "retry
    // from scratch", so wipe every cloud marker for this post here too.
    const stKey = `${campaign}:${postId}`;
    const postOf = (k) => k.split(':').slice(0, 2).join(':');
    const st = loadState();
    let clearedCloud = false;
    for (const map of ['cloudFailures', 'cloudAccepted', 'cloudRetriggered']) {
      const m = st[map];
      if (!m || typeof m !== 'object') continue;
      for (const k of Object.keys(m)) if (postOf(k) === stKey) { delete m[k]; clearedCloud = true; }
    }
    if (clearedCloud) saveState();
    appendActivity({
      campaign, postId, platform: handoffs.map((h) => h.lane).join('+') || null,
      action: slotSlip ? 'slot-slip' : 'reschedule', ok: true, errorCode: null,
      // The slip row NAMES the new time: the operator reading Activity must see where
      // the slot went, and the sweep's standing de-dupe compares this exact string
      // (same post, same target time = the same move, never a second row).
      errorMessage: slotSlip ? slotSlipMessage(scheduledAt) : null,
...
// ---------- R6a gate refinements: approval expiry + slot slip (both opt-in, default OFF) ----------
// ux-audit 2026-08-04 dim-1 proposals 3+5 (L5/L3). Two sweeps the publish walk runs
// BEFORE eligibleDuePosts enumerates (lib/scheduler.mjs runDueExclusive), so a
// refused/aged gate takes effect on the SAME run, never one tick late. Both are
// fail-closed by construction: each can only REVOKE eligibility (expiry) or MOVE
// time (slip) - neither can ever publish, approve, or widen anything.

// The slip row's message. ONE builder shared by the row writer (reschedulePost)
// and the sweep's standing de-dupe below, so the two strings can never drift.
export function slotSlipMessage(scheduledAt) {
  return `still awaiting approval - slot moved to ${scheduledAt} (opt-in slot slip; approve earlier to publish earlier)`;
}

// posting.approvalExpiryHours (owner-only, null = off): an approved post still
// unpublished after the window drops back to draft - the human gate attests to
// content that is STILL current, and editedSinceApproval only guards content
// drift, not time drift. The revert re-enters the normal review queue and the
// existing re-approve UX; nothing is deleted or published.
export async function sweepApprovalExpiry({ actor = 'pendpost' } = {}) {
  if (isDevReadonly()) return { ok: true, enabled: false, reverted: 0 };
  const hours = getPosting().approvalExpiryHours;
  if (!Number.isInteger(hours) || hours < 1) return { ok: true, enabled: false, reverted: 0 };
  const now = Date.now();
  const windowMs = hours * 3600 * 1000;
  let reverted = 0;
  for (const c of loadPlanStore().campaigns || []) {
    for (const post of c.posts || []) {
      if (post.approval !== 'approved') continue;
      if (post.status === 'posted') continue;
      const at = Date.parse(post.approvalAt || '');
      // No parseable approvalAt = no clock to age against - fail closed on the
      // REVERT (touch nothing), since reverting on a guess would thrash legacy posts.
      if (Number.isNaN(at) || now - at <= windowMs) continue;
      // A natively handed-off post (FB scheduled_publish_time, YouTube publishAt, ...)
      // already spent its approval on the platform; reverting the local flag would
      // not stop the platform and would lie about what is armed.
      if (nativeHandoffs(post).length) continue;
      if (inFlight.has(`${c.id}/${post.id}`)) continue;
      const found = findPlanEntry(c.id);
      if (found.error) continue;
      try {
        await mutatePlan(found.absPlan, (freshPlan) => {
          const p = (freshPlan.posts || []).find((x) => x.id === post.id);
          if (!p) throw Object.assign(new Error(`post ${post.id} vanished mid-write`), { code: 'unknown_post' });
          // Re-check on the FRESH read (the same in-lock discipline as every other
          // write here): a publish or re-approve that raced this sweep wins.
          if (p.approval !== 'approved' || p.status === 'posted') return p;
          const freshAt = Date.parse(p.approvalAt || '');
          if (Number.isNaN(freshAt) || now - freshAt <= windowMs) return p;
          p.approval = 'draft';
          delete p.approvalBy;
          delete p.approvalAt;
          delete p.approvedContentHash;
          delete p.editedSinceApproval;
          // The note rides the post into the review queue so the detail view says WHY
          // it is back (PostDetail renders approvalNote); re-approving overwrites it.
          p.approvalNote = `approval aged out after ${hours}h - review and approve again to publish`;
          return p;
        });
      } catch { continue; } // a raced/failed revert is retried next run - never fatal to the walk
      appendActivity({
        campaign: c.id, postId: post.id, platform: null, action: 'approval-expired', ok: true,
        errorCode: null, errorMessage: `approval aged out after ${hours}h - the post is back in review`,
        lateMin: null, actor,
      });
      logLine('info', `gate: ${c.id}/${post.id} approval aged out (> ${hours}h) - reverted to draft for re-approval`);
      reverted += 1;
    }
  }
  return { ok: true, enabled: true, reverted };
}

// posting.slotSlipMinutes (owner-only, null = off): a post still unapproved N
// minutes before its slot MOVES to the next same-time occurrence (+24h steps
// until it clears the window again) instead of bleeding into the red overdue
// alarm. There is no slot/cadence grid to snap to - scheduledAt is free-form -
// so "next occurrence" = same clock time, next day(s). Reuses reschedulePost
// (slotSlip arm) so the move rides the exact operator reschedule path.
export async function sweepSlotSlip({ actor = 'pendpost' } = {}) {
  if (isDevReadonly()) return { ok: true, enabled: false, moved: 0 };
  const minutes = getPosting().slotSlipMinutes;
  if (!Number.isInteger(minutes) || minutes < 1) return { ok: true, enabled: false, moved: 0 };
  const now = Date.now();
  const windowMs = minutes * 60 * 1000;
  let moved = 0;
  for (const c of loadPlanStore().campaigns || []) {
    for (const post of c.posts || []) {
      // Only a post the approval gate still blocks can slip: an approved, unedited
      // post will fire on time (nothing to move), and a rejected post is a DECIDED
      // no - rolling its slot forward forever would fake an open question.
      const blocked = post.approval !== 'approved' ? post.approval !== 'rejected' : post.editedSinceApproval === true;
      if (!blocked) continue;
      if (post.status === 'posted') continue;
      if (post.executionMode && post.executionMode !== 'fully-scheduled') continue; // parked = no due clock
      // The exported plans.mjs helper: a post that never shows red overdue while
      // unapproved (radar replies, self-post-only lanes) has no alarm to prevent,
      // so it never slips - in particular a Radar reply keeps its approve-then-
      // fire-promptly contract untouched.
      if (awaitingApproval(post)) continue;
      const due = Date.parse(post.scheduledAt || '');
      if (Number.isNaN(due)) continue;
      if (due - now > windowMs) continue; // slot still comfortably ahead
      if (inFlight.has(`${c.id}/${post.id}`)) continue;
      // Next same-time occurrence that clears the window with slack, so one move is
      // always enough (a window wider than 24h skips whole days in one hop).
      let next = due;
      while (next - now <= windowMs) next += 24 * 3600 * 1000;
      const nextIso = new Date(next).toISOString();
      // Standing de-dupe (the media-missing-defer pattern): the same move (same post,
      // same target time) never writes a second row, even if a raced tick re-enters.
      const latestForPost = (loadState().activity || []).find((e) => e.campaign === c.id && e.postId === post.id);
      if (latestForPost && latestForPost.action === 'slot-slip' && latestForPost.errorMessage === slotSlipMessage(nextIso)) continue;
      const res = await reschedulePost({ campaign: c.id, postId: post.id, scheduledAt: nextIso, actor, slotSlip: true });
      if (res.ok) {
        logLine('info', `gate: ${c.id}/${post.id} still unapproved ${minutes}min before its slot - moved to ${nextIso} (slot slip)`);
        moved += 1;
      }
    }
  }
  return { ok: true, enabled: true, moved };
}
...
export async function markPosted({ campaign, postId, actor, externalUrl, platform } = {}) {
  const idErr = requireIds(campaign, postId) || requireActor(actor);
  if (idErr) return idErr;
  ...
  // R5/G4/L4 - lane-scoped mark. A `platform` records manual completion for THAT
  // lane only, so a mixed multi-lane post keeps owing its still-open siblings
  // (the post-scoped path below closes ALL lanes at once, which is wrong when
  // pendpost still owes one). The platform must be a real registered lane; that
  // it is one the POST targets is validated per-post inside mutatePlan.
  const laneScoped = platform !== undefined && platform !== null;
  if (laneScoped && (typeof platform !== 'string' || !PLATFORM_ID_FIELDS[platform])) {
    return errorBody('invalid_input', `platform must be one of ${Object.keys(PLATFORM_ID_FIELDS).join('|')}`);
  }
  const found = findPlanEntry(campaign);
  if (found.error) return found.error;
  const flightErr = inFlightGuard(campaign, postId);
  ...
  try {
    ...
    const updated = await mutatePlan(found.absPlan, (plan) => {
      ...
      if (laneScoped) {
        ...
        if (!platforms.includes(platform)) {
          throw Object.assign(new Error(`post ${postId} does not target ${platform}`), { code: 'invalid_input' });
        }
        // A lane that already minted its platform id was engine-published - a manual
        // marker would falsely claim the owner posted it by hand. Refuse it.
        const minted = (PLATFORM_ID_FIELDS[platform] || []).some((f) => post[f]);
        if (minted) {
          throw Object.assign(new Error(`${platform} already carries a published id on ${postId}`), { code: 'invalid_input' });
        }
        post.manualCompletions = (post.manualCompletions && typeof post.manualCompletions === 'object') ? post.manualCompletions : {};
        const existing = post.manualCompletions[platform];
        if (existing) {
          // ONE legal re-entry per lane: attaching or correcting the lane's live
          // URL (the copy-draft flow often marks a lane first, gets the permalink
          // later). A bare re-mark with no new url is a no-op error, not silent.
          if (!externalUrl) {
            throw Object.assign(new Error(`${platform} on ${postId} is already marked posted`), { code: 'invalid_input' });
          }
          existing.externalUrl = String(externalUrl);
        } else {
          post.manualCompletions[platform] = { at: new Date().toISOString(), ...(externalUrl ? { externalUrl: String(externalUrl) } : {}) };
        }
        // Which lanes still owe publish evidence (a minted id OR a manual marker) -
        // the same predicate plans.platformPending / scheduler.lanesOwed use.
        const pendingPlatforms = platforms.filter((p) => {
          if (post.manualCompletions && post.manualCompletions[p]) return false;
          return !(PLATFORM_ID_FIELDS[p] || []).some((f) => post[f]);
        });
        if (pendingPlatforms.length === 0 && post.status !== 'posted') {
          post.status = 'posted';
          post.postedAt = new Date().toISOString();
          // publishedVia:'manual' ONLY when EVERY lane's evidence is manual. A mix
          // of engine-minted + manual leaves it unset - an honest "not all by hand".
          const anyMinted = platforms.some((p) => (PLATFORM_ID_FIELDS[p] || []).some((f) => post[f]));
          if (!anyMinted) post.publishedVia = 'manual';
        }
        laneResult = { platform, pendingPlatforms };
        return post;
      }

      // Post-scoped path (every lane at once) - unchanged.
      if (post.status === 'posted') {
        // ONE legal re-entry: recording (or correcting) the live URL of a post
        // that was already hand-published. The copy-draft flow often marks the
        // post first and only later has the permalink - without this, the link
        // could never be attached and the post detail stays linkless forever.
        if (post.publishedVia === 'manual' && externalUrl) {
          post.externalUrl = String(externalUrl);
          return post;
        }
        throw Object.assign(new Error(`post ${postId} is already marked posted`), { code: 'invalid_input' });
      }
      post.status = 'posted';
      post.postedAt = new Date().toISOString();
    ...
    appendActivity({ campaign, postId, platform: laneScoped ? platform : null, action: 'mark-posted', ok: true, errorCode: null, errorMessage: null, lateMin: null, actor: actor.trim() });
    if (laneScoped) {
      return { ok: true, platform: laneResult.platform, pendingPlatforms: laneResult.pendingPlatforms, post: { id: updated.id, status: updated.status, postedAt: updated.postedAt || null, publishedVia: updated.publishedVia || null, externalUrl: updated.externalUrl || null }, rev: postRev(updated) };
    }
...
// ---------- generic lane block: resume (the X credits breaker etc.) ----------

// Clear a lane-wide publish block (lib/state.mjs recordLaneBlock - e.g. X after
// an HTTP 402 credits refusal) AND release the posts that block's failure
// streak parked, so they re-enter the fire loop on the next tick. The Meta 368
// breaker keeps its own explicit-clear write (pendpost_record_block) - refusing
// 'meta' here keeps the two clear ceremonies from blurring.
export async function resumeLane({ platform, actor } = {}) {
  const actErr = requireActor(actor);
  if (actErr) return actErr;
  const p = String(platform || '').trim().toLowerCase();
  if (!p) return errorBody('invalid_input', 'platform is required (e.g. "x")');
  if (p === 'meta') return errorBody('invalid_input', 'the Meta 368 block is cleared via pendpost_record_block (blockedUntil: null), not the lane resume');
  const block = getLaneBlock(p);
  const cleared = clearLaneBlock(p);
  // Release the publishHolds this block's failure streak stamped: only holds on
  // THIS lane that carry the credits code (or the raw HTTP 402 message from a
  // pre-'credits' engine) - unrelated holds (media faults etc.) stay parked.
  const heldByBlock = (hold) => hold && hold.lane === p
    && (hold.code === 'credits' || (block && block.code && hold.code === block.code) || /HTTP 402/.test(String(hold.message || '')));
  let released = 0;
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns) {
      if (c.error) continue;
      const anyHeld = (c.posts || []).some((post) => heldByBlock(post.publishHold));
      if (!anyHeld) continue;
      await mutatePlan(resolvePlanPath(c.path), (plan) => {
        for (const rawPost of plan.posts || []) {
          if (!heldByBlock(rawPost.publishHold)) continue;
          rawPost.publishHold = null; // null, never delete - the engines' field-merge rule
          released += 1;
        }
      });
    }
  } catch (err) {
    return errorBody('engine_failure', `lane block cleared but releasing held posts failed: ${err.message}`);
  }
  appendActivity({ campaign: null, postId: null, platform: p, action: 'lane-resumed', ok: true, errorCode: null, errorMessage: null, lateMin: null, actor: actor.trim() });
  // ---- SPEC 50 P3 BLOCK: end an auto-engage cool-down (§8) ---------------------------------
  // A lane can be halted TWICE over, by two different mechanisms: the publish-side lane block
  // above (an X 402, a Meta 368) and the "Respond for me" circuit breaker, which cools a lane
  // down for 24h after a rate limit or three failed actions. The spec names resume_lane as the
  // one control that ends a cool-down (row 2e3's "Resume now" calls it), so it has to clear
  // both - a resume that only cleared half would leave the operator pressing a button that
  // visibly does nothing. Dynamic import: lib/engage.mjs imports this module.
  let engageCooldownCleared = null;
  try {
    const { clearLaneCooldown } = await import('./engage.mjs');
    const cleared = clearLaneCooldown(p);
    if (cleared) {
      engageCooldownCleared = cleared.wasCoolingFor || true;
      appendActivity({ campaign: null, postId: null, platform: p, action: 'engage-lane-resumed', ok: true, errorCode: null, errorMessage: null, reason: cleared.wasCoolingFor || null, lateMin: null, actor: actor.trim() });
    }
  } catch { /* no engage state for this client - nothing to clear, and never a reason to fail */ }
  // ---- end SPEC 50 P3 BLOCK -----------------------------------------------------------------
  // Credit recheck (owner-chosen): X exposes no credits-balance read, so the only real
  // test that credits are back is attempting the post. Fire the now-unblocked lane's due
  // posts NOW through the one existing fire path instead of waiting up to a tick. Gate on
  // `cleared` (a block existed) OR a released hold: the COMMON case is posts the lane block
  // dropped before dispatch, which never accrue a per-post publishHold - clearing the block
  // is what un-gates them, so releasing 0 holds must NOT skip the recheck. A repeat 402
  // re-arms the lane block (lib/scheduler.mjs) - reported as stillDepleted. `runDueExclusive`
  // is single-flight: an in_flight/dev-readonly return just means the posts fire on the next
  // natural sweep (pending), never a double-post (the publish-claim lease guards that).
  let published = 0;
  let stillDepleted = false;
  let pending = false;
  if (cleared || released > 0) {
    const run = await runDueExclusive(actor.trim(), {});
    if (run && run.ok === false) {
      pending = true; // in_flight or dev_readonly - the posts fire on the next tick
    } else {
      // `ran` is the per-run per-lane truth (lib/scheduler.mjs): ok ONLY when a result
      // row actually succeeded. Count what fired on THIS lane (both released-hold posts
      // and lane-block-dropped due posts); the lane re-blocks (getLaneBlock) iff the
      // re-fire hit the same 402 - the honest recheck signal.
      const ran = Array.isArray(run?.ran) ? run.ran : [];
      published = ran.filter((r) => r.lane === p && r.ok).length;
      stillDepleted = Boolean(getLaneBlock(p));
    }
  }
  return { ok: true, platform: p, cleared, released, published, stillDepleted, pending, ...(engageCooldownCleared ? { engageCooldownCleared } : {}) };
}
...
// Caption/data limits that silently truncate or hard-fail at publish time. The numbers
// live in lib/caption.mjs now (the SAME map the engine fail-closed backstop reads), so
// Pruefen and the engine can never name different caps; this narrow VIEW keeps only the
// four Composer lanes the generic-caption gate owns - x/gbp have their own effective-text
// checks below and are deliberately NOT capped through this generic path.
const CAPTION_LIMITS = {
  instagram: GENERIC_CAPTION_LIMITS.instagram,
  facebook: GENERIC_CAPTION_LIMITS.facebook,
  linkedin: GENERIC_CAPTION_LIMITS.linkedin,
  youtube: GENERIC_CAPTION_LIMITS.youtube,
};
...
export async function platformValidate({ campaign, postId } = {}) {
  ...
  const result = {};
  const ctx = { accounts, state, now, metaBlocked, captionLen: (post.caption || '').length };

  for (const platform of post.platforms || []) {
    const { problems, problemCodes, warnings, needsSetup } = lanePreflight(post, platform, ctx);
    result[platform] = laneResult(post, problems, problemCodes, warnings, needsSetup);
  }
...
// The per-lane pre-flight check: pure over (post, platform, ctx) so the enforced
// seams (setApproval gate, scheduler publish fence) share the SAME verdict
// platform_validate surfaces - no second checker can drift. ctx carries the
// per-invocation reads (accounts/state/now/metaBlocked/captionLen); everything
// else is a module fn (getPosting, IMAGE_LANES, ...). Returns the raw
// accumulators; laneResult below turns them into the {ready, ...} verdict.
export function lanePreflight(post, platform, ctx) {
  const { accounts, state, now, metaBlocked, captionLen } = ctx;
  const problems = [];
  ...
  let needsSetup = false;
    // Mock-id fence (defense in depth beside the scheduler's auto-clear): in LIVE
    // mode an id minted by the mock driver is FAKE publish evidence - block the
    // lane plainly instead of trusting it (lanesOwed would otherwise treat the
    // post as already published on this lane, forever).
    if (resolveMode() !== 'mock') {
      for (const f of PLATFORM_ID_FIELDS[platform] || []) {
        const v = post.ids ? post.ids[f] : post[f];
        if (isMockPlatformId(v)) {
          problems.push(`a mock (test-run) ${platform} id is stored on this post - fake publish evidence, not a real publish; clear it to publish for real`);
          break;
        }
      }
    }
    ...
    // A present-but-malformed video: its audio and video tracks are different lengths
    // (avSyncOk===false, drift over AV_DRIFT_MAX_SEC). Instagram rejects such a reel at
    // rupload with a non-retriable ProcessingFailedError (found live: a 5s desync -> HTTP
    // 400), so block it at author time rather than letting the cloud fire fail. Gated on a
    // MEASURED false only - null (unprobed/no-audio) never blocks. Media-level, but pushed
    // per-lane like the missing-file line above since every lane shares the one file.
    if (postNeedsMedia(post) && post.type !== 'carousel' && post.media.exists && post.media.avSyncOk === false) {
      problems.push(AV_SYNC_REASON);
    }
    // Specs 16/17/39 (+ X media tweet): the `image` TYPE publishes on the lanes in
    // IMAGE_LANES (capabilities.mjs) - container lanes (reddit local upload,
    // pinterest/instagram public imageUrl) and byte lanes (x/telegram/discord/mastodon,
    // direct kind-driven upload). Block it on every other lane (mirrors the type==='reel'
    // FB gate below) so the Composer's format select can never strand it on a lane whose
    // engine cannot publish it (youtube, nostr). The honest lane list is derived from the
    // allowlist so the message can never drift from the rule.
    if (post.type === 'image' && !IMAGE_LANES.includes(platform)) {
      const lanes = `${IMAGE_LANES.slice(0, -1).join(', ')} and ${IMAGE_LANES[IMAGE_LANES.length - 1]}`;
      problems.push(`${platform} does not publish an image post (the image TYPE is for ${lanes})`);
    ...
    if (platform === 'linkedin') {
      ...
      // Per-post member (personal-profile) target: fail closed if no person URN has been
      // captured yet. Reconnecting LinkedIn (auth) with the member scope granted writes it.
      // Block, not coerce: silently publishing to the Company Page instead is the wrong
      // destination, exactly what DestinationStrip exists to prevent.
      if (post.liAuthor === 'member' && !accounts.linkedin?.personUrn) {
        problems.push('LinkedIn member posting needs a captured person URN - reconnect LinkedIn (Setup: auth) to grant the member scope, or switch this post back to the Company Page');
        needsSetup = true;
      }
    }
    if (platform === 'youtube') {
      ...
      if (post.executionMode === 'fully-scheduled' && !post.ids.ytVideoId && !Number.isNaN(due) && due <= now) {
        // A past scheduledAt is NOT a hard block: the engine clamps an overdue publishAt to
        // ~2 min out and ships the video late (yt-social.mjs cmdSchedule), so an overdue
        // approved Short auto-ships instead of stranding. Refusing here was the LAST gate
        // defeating that auto-ship - it left an overdue post blocked with "reschedule first"
        // even though the engine can ship it. Warn (it publishes a little after its slot),
        // never refuse.
        warnings.push('scheduledAt is in the past - it will publish shortly (YouTube publishAt is clamped to about 2 minutes from now), a little later than its slot');
      }
    }
    if (platform === 'x') {
      ...
      const xText = effectiveLaneText(post, 'x');
    ...
    if (platform === 'telegram') {
      ...
      const tgText = effectiveLaneText(post, 'telegram');
    ...
    if (platform === 'discord') {
      ...
      const dcText = effectiveLaneText(post, 'discord');
    ...
    if (platform === 'pinterest') {
      ...
      const pinDesc = effectiveLaneText(post, 'pinterest');
    ...
    if (platform === 'tiktok') {
      ...
      const ttText = effectiveLaneText(post, 'tiktok');
    ...
    if (platform === 'mastodon') {
      ...
      const mText = effectiveLaneText(post, 'mastodon');
    ...
    if (platform === 'nostr') {
      ...
      if (post.type === 'nostr-longform') {
        ...
        const nText = effectiveLaneText(post, 'nostr');
  ...
  return { problems, problemCodes, warnings, needsSetup };
}

// Assemble the per-lane verdict from lanePreflight's accumulators. Approval is NOT
// a per-platform problem: a draft/pending/rejected post is the normal pre-publish
// state (shown by the post-level Entwurf badge), not a fault to flag on every lane.
// It still gates `ready` here, and publishPreview re-adds the dry-run blocker - but
// it never pollutes problems[] / the GUI panel. An edited-since-approval post reads
// as NOT ready for the same reason: the gate (eligibleDuePosts/buildPublishJob) will
// refuse it until re-approval. problemCodes normalizes to a same-length array (null
// where uncoded) so the SPA can zip problems[i]/problemCodes[i] without a dance.
function laneResult(post, problems, problemCodes, warnings, needsSetup) {
  const approved = post.approval === 'approved' && !post.editedSinceApproval;
  return { ready: approved && !problems.length, problems, problemCodes: problems.map((_, i) => problemCodes[i] || null), warnings, needsSetup };
}

// The enforced blocking set, shared by the approve gate and the publish fence: a
// lane blocks when it has any content-integrity problem. needsSetup lanes are
// EXCLUDED - a not-yet-connected lane is environment state, not a defective post,
// and unauth already fails closed downstream (engine refusal + publishHold). Meta-368
// and the FB policy toggle keep their own fence gates; they may also appear here and
// coarsely block approval, which is harmless (the owner can force). Returns
// [{ platform, problems }] for every blocking lane, or [] when the post is clear.
export function laneBlockers(post, ctx) {
  const blocked = [];
  for (const platform of post.platforms || []) {
    const { problems, needsSetup } = lanePreflight(post, platform, ctx);
    if (problems.length && needsSetup !== true) blocked.push({ platform, problems });
  }
  return blocked;
}

// Build the lanePreflight ctx from module state - the same five reads
// platformValidate computes inline, factored so the enforced seams share them.
export function preflightContext(post) {
  const state = loadState();
  return { accounts: accountStatus(), state, now: Date.now(), metaBlocked: isMetaBlocked(state), captionLen: (post.caption || '').length };
}
...
export function pendpostHealth({ horizon = 5, includeSetup = true } = {}) {
  ...
  const upcoming = campaigns
    ...
    .map((p) => {
      ...
      // The public media URL is not live (a probe-confirmed dead mirror: 404/non-image).
      // A DISTINCT blocker from mediaMissing - the render IS on local disk, but the URL a
      // platform's server-side fetch would hit is dead, so IG/pinterest cannot publish it.
      // State-only here (this health surface stays fast + cached, no live network); the
      // LIVE probe that catches it BEFORE the first failure lives in publishPreview.
      if (p.lastFailure && typeof p.lastFailure.message === 'string' && p.lastFailure.message.includes(MEDIA_URL_DEAD_MARK)) {
        postBlockers.push('public media URL not live - re-mirror the render');
        postBlockerCodes.push({ code: 'blocker.mediaUrlDead' });
      }
  ...
  // US-MC-10: schedulerRunning is the PER-CLIENT flag, default-ON - a client is
  // scheduling unless the operator EXPLICITLY stopped it (enabled === false). Same
  // predicate as scheduler.schedulerEnabledHere (inlined to avoid a static
  // scheduler<->writes import cycle), so the displayed flag matches the honored
  // behavior. The old `=== true` misreported a never-toggled default-on client as off.
  const schedulerRunning = state.scheduler?.enabled !== false;
  ...
  // Generic lane block (lib/state.mjs recordLaneBlock): a lane the scheduler
  // halted after an account-level terminal refusal. The first case is X HTTP
  // 402: the X API plan ran out of paid credits, so every X post is halted so
  // retries are not burned; the owner tops up credits in the X developer portal
  // and resumes the lane (POST /api/state/lane-resume). Meta's 368 breaker is
  // surfaced separately (setup/metaBlocked) and is not repeated here.
  for (const [lanePlatform, lb] of Object.entries(state.laneBlocks || {})) {
    if (!lb || !lb.at) continue;
    const text = lb.code === 'credits'
      ? `${lanePlatform === 'x' ? 'X' : lanePlatform} refused to publish: API credits depleted - the API plan ran out of paid credits, so every ${lanePlatform} post is halted so retries are not burned. Top up credits${lanePlatform === 'x' ? ' in the X developer portal' : ''}, then resume the lane.`
      : `${lanePlatform} lane is halted after a terminal refusal${lb.reason ? ` (${lb.reason})` : ''} - fix the account, then resume the lane.`;
    blockers.push(text);
    blockerCodes.push({ code: 'blocker.laneBlocked', params: { platform: lanePlatform, reason: lb.reason || null } });
  }
  ...
  if (schedulerRunning) {
    ...
    for (const c of campaigns) {
      ...
      for (const p of c.posts || []) {
        ...
        // Prefer the cloud's cached reason; a purely LOCAL failure (the engine's own
        // attempt record, lib/plans.mjs lastFailureFor) previously surfaced as a bare
        // "overdue" with no reason at all - the 2026-08 IG 9004 storm ran three days
        // that way. p.lastFailure is the normalized post's derived view of both halves.
        const localMsg = (!fail || !fail.message) && p.lastFailure && p.lastFailure.message
          ? `${p.lastFailure.terminal ? 'publish failed (held after repeated failures - reschedule or edit to retry)' : 'publish failing'}: ${p.lastFailure.message}`
          : null;
        const reason = fail && fail.message ? ` - cloud fire failed: ${fail.message}` : (localMsg ? ` - ${localMsg}` : '');
        blockers.push(`overdue: ${c.id}/${p.id} is approved and past due but not published${reason}`);
        blockerCodes.push({ code: 'blocker.overdueUnpublished', params: { campaign: c.id, postId: p.id, reason: (fail && fail.message) || localMsg || null } });
      }
    }
  }
  return {
  ...
  const rows = clients.map((c) => {
    ...
    try {
      return withClient(clientRoot(c.id), () => {
        ...
        // A4 archive safety: the in-flight COUNTS ride every row (booleans/counts
        // rule) so the Clients archive confirm can be honest with zero extra reads.
        const flight = inFlightWorkHere();
        return {
          ...base,
          ready: health.ready,
          ...
          inFlight: { total: flight.total, local: flight.local, native: flight.native },
...
// ---------- archive safety sweep (A4: make archive mean what it says) ----------

// A client's IN-FLIGHT work: the posts that would still publish (or silently die)
// if the client were archived and forgotten. Two kinds, split because their
// failure modes differ (ux-audit dim-4 matrix row 10):
//   native - the PLATFORM already holds a scheduled object (FB scheduled post,
//            YouTube publishAt, mastodon queue entry, WP 'future', Ghost
//            'scheduled'). It fires from the platform REGARDLESS of the local
//            registry status - an "archived" client that keeps publishing.
//            Detected via the same nativeHandoffs list the unschedule verb
//            cancels, so the count and the cancel can never disagree. NOT
//            filtered on approval/executionMode: parking or un-approving locally
//            does not delete the platform object.
//   local  - approved + fully-scheduled and not yet posted. The scheduler skips
//            archived clients, so these silently stop and sit overdue behind the
//            hidden health cell; on restore they fire unexpectedly.
// Runs on the CALLER's already-bound scope; clientInFlightWork wraps it per id.
function inFlightWorkHere() {
  const { campaigns } = loadPlanStore({ includePosts: true });
  const posts = [];
  for (const c of campaigns || []) {
    for (const p of c.posts || []) {
      if (p.status === 'posted') continue;
      // Normalized posts nest the platform ids under .ids; flatten them so
      // nativeHandoffs (the ONE cancel-list source of truth) is reused verbatim.
      const native = nativeHandoffs({ status: p.status, ...p.ids }).map((h) => h.lane);
      const localFires = p.approval === 'approved' && p.executionMode === 'fully-scheduled' && Boolean(p.scheduledAt);
      if (!native.length && !localFires) continue;
      posts.push({ campaign: p.campaign || c.id, postId: p.id, platforms: p.platforms || [], scheduledAt: p.scheduledAt || null, native });
    }
  }
  const native = posts.filter((p) => p.native.length > 0).length;
  return { total: posts.length, native, local: posts.length - native, posts };
}

// READ-ONLY: one client's in-flight work, scoped exactly like a clientsOverview
// row (its own withClient binding, no cross-client read).
export function clientInFlightWork(id) {
  return withClient(clientRoot(id), () => inFlightWorkHere());
}

// The guarded archive: compute the client's in-flight work FIRST, refuse a blind
// archive while the platform itself still holds scheduled objects, and offer the
// sweep (fan-out over the existing unschedule verb, which cancels the native
// objects and parks the post) as part of the same call. Shared by the REST route
// and the MCP client_archive tool so both faces keep identical semantics.
// Restores, unknown ids, bad input and the active-client refusal delegate
// STRAIGHT to archiveClient - validation and messages never fork.
export async function archiveClientSweep({ id, actor, acknowledgeInFlight = false, unscheduleInFlight = false } = {}) {
  const roster = listClients();
  const entry = (roster.clients || []).find((c) => c.id === id);
  const archiving = Boolean(entry) && (entry.status || 'active') === 'active';
  // Not an active->archived flip (restore/unknown/invalid), a malformed actor, or
  // the active-client case: archiveClient owns the answer, sweep does not apply.
  if (!archiving || typeof actor !== 'string' || !actor.trim() || roster.activeClientId === id) {
    return archiveClient({ id, actor });
  }
  let work;
  try {
    work = clientInFlightWork(id);
  } catch (err) {
    // Fail CLOSED: a subtree we cannot read is a subtree whose in-flight work we
    // cannot rule out - never archive over it blind.
    return errorBody(err.code === 'invalid_input' ? 'invalid_input' : 'manifest_error', `cannot read client ${id}'s plans before archiving: ${err.message}`);
  }
  if (work.native > 0 && acknowledgeInFlight !== true && unscheduleInFlight !== true) {
    return errorBody('needs_confirm',
      `client ${id} still has in-flight work (${work.local} waiting to publish locally, ${work.native} already scheduled on the platform itself). The platform-scheduled posts KEEP firing after archive. Pass unscheduleInFlight:true to cancel them as part of archiving, or acknowledgeInFlight:true to archive anyway.`,
      { inFlight: work });
  }
  let unscheduled = 0;
  if (unscheduleInFlight === true && work.total > 0) {
    const failures = [];
    await withClient(clientRoot(id), async () => {
      for (const p of work.posts) {
        // confirm:true is carried by the caller's acknowledge of THIS sweep - the
        // same consent the standalone unschedule confirm collects.
        const res = await unschedulePost({ campaign: p.campaign, postId: p.postId, confirm: true, actor });
        if (res && res.ok === true) unscheduled += 1;
        else failures.push(`${p.campaign}/${p.postId}: ${(res && (res.message || res.code)) || 'unschedule failed'}`);
      }
    });
    if (failures.length > 0) {
      // Abort the archive: a half-swept client must stay visible, not vanish with
      // live platform objects still attached.
      return errorBody('engine_failure',
        `archive aborted - ${failures.length} of ${work.total} in-flight posts could not be unscheduled (nothing was archived): ${failures.join('; ')}`,
        { inFlight: work, unscheduled });
    }
  }
  const result = archiveClient({ id, actor });
  if (!result || result.ok !== true) return result;
  return { ...result, inFlight: work, unscheduled };
}
...
export async function publishPreview({ horizon = 5, campaign = null } = {}) {
  ...
  for (const p of due) {
    ...
    // LIVE media-URL liveness (B2): for an instagram/pinterest post, probe the public
    // URL(s) a platform's server-side fetch would hit and, if one is DETERMINISTICALLY
    // dead (404/non-image), add a per-lane blocker BEFORE the post is ever fired - a
    // mirror drift is caught at dry-run time instead of at due-time. Fail-open: a
    // transient/inconclusive probe adds nothing. This is the deliberate-dry-run home of
    // the live probe; pendpostHealth stays fast/cached (state-only mediaUrlDead there).
    let deadMediaDiag = null;
    if ((p.platforms || []).some((pl) => pl === 'instagram' || pl === 'pinterest')) {
      const rawPost = rawPostOf(p.campaign, p.postId);
      for (const url of (rawPost ? postPublicMediaUrls(rawPost) : [])) {
        const d = await deadMediaUrlDiagnosis(url);
        if (d) { deadMediaDiag = d; break; }
      }
    }
    const platforms = (p.platforms || []).map((platform) => {
      ...
      const deadHere = Boolean(deadMediaDiag) && (platform === 'instagram' || platform === 'pinterest');
      if (deadHere) blockers.unshift(deadMediaDiag);
      ...
      return {
        ...
        // A dead public media URL genuinely won't fire (the engine pre-flight skips it),
        // so this lane is not ready even though platformValidate only checks URL derivability.
        ready: entry.ready === true && !deadHere,
        blockers,
      };
    });
...
export async function listComments({ campaign, postId, platform, objectId } = {}) {
  ...
  // Relationship-memory accretion (spec 49 R12): stamp each commenter as a 'they'-direction
  // exchange at the SINGLE comment-read chokepoint, so BOTH the REST and MCP faces accrete
  // from one place. Idempotent per (kind, ref) - re-reading the same thread never double-counts.
  // Fully wrapped + non-throwing: a stamp failure must never break the comment read.
  try {
    const state = loadState();
    for (const it of items) {
      stampEngager(state, {
        lane: target.lane, handle: it && it.author, kind: 'comment',
        ts: it && it.ts, ref: it && it.commentId, permalink: it && it.permalink,
        direction: 'they', excerpt: it && it.text,
      });
    }
    if (items.length) saveState();
  } catch { /* accretion never breaks a comment read */ }
  return { ok: true, items, platform: target.lane, targetPlatform: target.platform, postId, moderateActions: COMMENT_CAPABILITIES[target.lane]?.moderate || [], reactActions: COMMENT_CAPABILITIES[target.lane]?.react || [] };
}
...
// B2 confirm gate (ux-audit 2026-08-04, dim 2 G7/I9): a comment reply posts PUBLIC
// text on the live thread immediately - unlike the Radar reply lane there is no
// approval fence, so a prompt-injected chat agent could publish unseen. Gated HERE
// (the shared twin behind the MCP tool AND POST /api/comments/reply, mirroring
// moderateComment) so both faces inherit it: actor 'owner' (the Studio Comments
// panel, app/src/lib/api.js ACTOR) stays one-click; ANY other actor must pass
// confirm:true or is refused fail-closed with needs_confirm, BEFORE campaign
// resolution (garbage input never leaks past the gate).
export async function replyToComment({ campaign, postId, commentId, text, platform, actor, confirm, author } = {}) {
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const who = actor.trim();
  if (who !== 'owner' && confirm !== true) {
    return errorBody('needs_confirm', 'reply_to_comment posts public text to the live thread immediately; a non-owner actor must pass confirm: true (B2 comment-reply gate).');
  }
  const idErr = requireIds(campaign, postId);
  if (idErr) return idErr;
  ...
  // Relationship-memory accretion (spec 49 R12): a sent reply is a 'me'-direction exchange with
  // the person I replied TO (the comment's author, passed in by the caller). Keyed on the reply
  // TARGET's identity, so my side of the conversation accretes under the same person as their
  // comments. Wrapped + non-throwing: the reply already succeeded; a stamp failure must not turn
  // a sent reply into an error. Skipped when the caller carries no author (nothing to key on).
  if (author != null && String(author).trim()) {
    try {
      const state = loadState();
      stampEngager(state, {
        lane: target.lane, handle: author, kind: 'comment',
        ts: new Date().toISOString(), ref: commentId.trim(),
        direction: 'me', excerpt: text,
      });
      saveState();
    } catch { /* accretion never breaks a sent reply */ }
  }
...
// Inbound-reply round-trip (spec 23, Pattern P6/P8): the operator answers ONE inbound
// X Activity event from the Studio inbox - the paired WRITE of list_inbound_events.
// The event comes from the LOCAL inbound-event store (state.inboundEvents, per-client
// via activeRoot(), the same store reconcileInboundEvents merges into); we find it by
// eventId and ROUTE by its type: mention/comment -> a tweet reply (X's mention-
// exemption reaches the mentioner), message -> a DM to the sender, reaction/follow ->
// NOT repliable (a FINAL skip - there is no thread to answer). For X we spawn the
// x-social `reply`/`dm` verb (a FRESH spawn - X is NOT a comment lane, so this does not
// reuse the reply_to_comment argv); the engine threads the vaulted token and returns
// { ok, id } or a mapped code (needs_scope/credits/target_gone) which we surface in the
// shared error envelope so the inbox prompts to authorize / top up / stop retrying a
// gone target. B2 confirm gate (shared with replyToComment): this posts PUBLIC text /
// a DM immediately with no approval fence, so actor 'owner' (the Studio inbox) is
// one-click while ANY other actor must pass confirm:true or is refused fail-closed.
const INBOUND_REPLY_TIMEOUT_MS = 120_000;
const INBOUND_REPLY_CODES = new Set(['needs_scope', 'credits', 'target_gone', 'invalid_input']);
export async function replyToInboundEvent({ actor, eventId, text, confirm, clientId } = {}) {
  void clientId; // per-call client scoping is bound by withClient at the call site
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const who = actor.trim();
  if (who !== 'owner' && confirm !== true) {
    return errorBody('needs_confirm', 'reply_to_inbound_event posts public text / a DM immediately; a non-owner actor must pass confirm: true (B2 inbound-reply gate).');
  }
  if (typeof eventId !== 'string' || !eventId.trim()) return errorBody('invalid_input', 'eventId is required (from list_inbound_events events[].eventId)');
  if (typeof text !== 'string' || !text.trim()) return errorBody('invalid_input', 'text is required (the reply body)');

  // Find the event in the local per-client store (loadState resolves under activeRoot(),
  // the same access reconcileInboundEvents uses). A store miss is not_found, never a
  // blind spawn against an unknown target.
  let event = null;
  try {
    const state = loadState();
    const store = Array.isArray(state.inboundEvents) ? state.inboundEvents : [];
    event = store.find((e) => e && e.eventId === eventId) || null;
  } catch { event = null; }
  if (!event) return errorBody('not_found', `no inbound event ${eventId} in the local inbox store`);

  // Route by type. reaction/follow carry no thread to answer - a structured, final skip.
  const type = event.type;
  if (type === 'reaction' || type === 'follow') {
    return errorBody('not_repliable', `a ${type} event has no thread to answer (only mention/comment/message are repliable)`);
  }
  if (type !== 'mention' && type !== 'comment' && type !== 'message') {
    return errorBody('not_repliable', `event type '${type}' is not repliable`);
  }
  // This seam wires the X lane (spec 23 is the X Activity round-trip); other platforms
  // answer through their own reply_to_comment path, never this DM/tweet-reply engine.
  const platform = event.platform;
  if (platform && platform !== 'x') {
    return errorBody('not_configured', `inbound reply via this seam is wired for x only (got ${platform}); use reply_to_comment for that lane`);
  }

  // Always-on humanizer gate on the outbound body (shared with replyToComment).
  text = humanize(text, { locale: getContentLocale() }).text;

  const script = resolveEnginePath('x', 'scripts/x-social.mjs');
  let argv;
  let action;
  if (type === 'message') {
    // DM the sender: event.author.id is the X user id the message came from.
    const recipient = event.author && typeof event.author.id === 'string' ? event.author.id.trim() : '';
    if (!recipient) return errorBody('invalid_input', `inbound event ${eventId} carries no author id to DM`);
    argv = ['dm', '--recipient', recipient, '--text', text, '--json', '--actor', who];
    action = 'inbound-dm';
  } else {
    // Reply to the tweet: event.externalPostId is the tweet id to reply to.
    const replyTo = typeof event.externalPostId === 'string' ? event.externalPostId.trim() : '';
    if (!replyTo) return errorBody('invalid_input', `inbound event ${eventId} carries no externalPostId to reply to`);
    argv = ['reply', '--reply-to', replyTo, '--text', text, '--json', '--actor', who];
    action = 'inbound-reply';
  }

  const { envelope, err, stderrTail } = await execScript(script, argv, INBOUND_REPLY_TIMEOUT_MS);
  if (!envelope) {
    appendActivity({ campaign: null, postId: event.postId || null, platform: 'x', action, ok: false, errorCode: 'engine_failure', errorMessage: (stderrTail || '').slice(0, 200), lateMin: null, actor: who });
    return errorBody('engine_failure', stderrTail || (err && err.message) || 'inbound reply engine produced no envelope');
  }
  if (!envelope.ok) {
    // The engine's code (needs_scope/credits/target_gone/invalid_input) rides through
    // verbatim so the inbox surfaces the right remediation; anything else is engine_failure.
    const code = INBOUND_REPLY_CODES.has(envelope.code) ? envelope.code : 'engine_failure';
    appendActivity({ campaign: null, postId: event.postId || null, platform: 'x', action, ok: false, errorCode: code, errorMessage: String(envelope.error || '').slice(0, 200), lateMin: null, actor: who });
    return errorBody(code, envelope.error || 'inbound reply failed', code === 'needs_scope' ? { needsScope: true, platform: 'x' } : {});
  }
  const id = envelope.id || (envelope.results || []).find((r) => r && (r.action === 'reply' || r.action === 'dm'))?.id || null;
  appendActivity({ campaign: null, postId: event.postId || null, platform: 'x', action, ok: true, errorCode: null, errorMessage: null, lateMin: null, actor: who });
  return { ok: true, id, platform: event.platform, eventId };
}
...
export async function listReviews({ limit } = {}) {
  ...
  // Relationship-memory accretion (spec 49 R12): stamp each reviewer as a 'they'-direction
  // 'review' exchange carrying its star rating, so a returning reviewer's history (and rating
  // trend) accretes under the same person as their comments. Idempotent per (kind, reviewId).
  // Wrapped + non-throwing: a stamp failure must never break the review read.
  try {
    const state = loadState();
    for (const it of items) {
      stampEngager(state, {
        lane: 'gbp', handle: it && it.author, kind: 'review',
        ts: it && it.ts, ref: it && it.commentId, permalink: it && it.permalink,
        direction: 'they', excerpt: it && it.text, rating: it && it.rating,
      });
    }
    if (items.length) saveState();
  } catch { /* accretion never breaks a review read */ }
...
// logged as a 'review-reply' Activity entry. A review reply is reversible (edit/
// remove) but still PUBLIC text posted immediately, so it carries the same B2
// confirm gate as replyToComment (ux-audit 2026-08-04, dim 2 G7/I9), gated HERE so
// the MCP tool AND POST /api/reviews/reply inherit it together: actor 'owner' (the
// Studio Reviews panel) stays one-click; any other actor must pass confirm:true or
// is refused fail-closed with needs_confirm. A missing scope degrades to
...
export async function replyToReview({ reviewId, text, actor, confirm } = {}) {
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const who = actor.trim();
  if (who !== 'owner' && confirm !== true) {
    return errorBody('needs_confirm', 'reply_to_review posts a public owner reply immediately; a non-owner actor must pass confirm: true (B2 comment-reply gate).');
  }
...
function radarState() {
  ...
  // R5 piece 2 (dim-2 G2/N1): the copy-posted ledger ({ source, externalId, postedUrl?,
  // at, actor }) - the `seen`-ledger pattern applied to copy-draft lanes. A copy-draft /
  // karma post-idea has no engine publish path, so this is the ONLY durable record that
  // the operator posted it by hand; it must outlive the signal's presence in the volatile
  // cache, so listRadar can join it (-> signal.copyPosted) even after a re-scan.
  if (!Array.isArray(state.radar.copyPosted)) state.radar.copyPosted = [];
  ...
  // The declined-backlog ledger ({ key, at }) - the seen-ledger pattern applied to GEO
  // backlog entries (G8): comparisonBacklog() filters these at every derivation.
  if (!Array.isArray(state.radar.geo.dismissedBacklog)) state.radar.geo.dismissedBacklog = [];
...
// The state.engage store (spec 50 §7.3), the radarState() pattern applied to auto-engage.
// STATE, not config: the action list, the open asks, the community-rule cache, the themes,
// the push record, the per-lane runtime and the daily counters are all volatile, per-client
// and rewritten by the pacer - exactly the reason state.radar's feed is not in config.json.
// It lives in the SAME per-client state.json as state.radar (same atomic writer, same
// withClient binding), so a decision written on a signal and the action row it produced can
// never land in two different files.
//
//  queue       Action rows (§3.1): { id, signalKey, lane, kind, payload, status, waitingOn,
//              releaseAt, graceUntil, attempts, rung, result, askId }. Pruned 30 days after
//              a terminal status.
//  asks        Ask rows - what the "Needs you" strip renders. Pruned 90 days after resolution.
//  communities `${lane} ${community}` -> CommunityRule { rule:none|disclose|noAutomation, ... },
//              so a community that bans automation is skipped without opening Chrome again.
//  themes      clustered topics an original post may be enqueued from (D10).
//  pushes      { askId, channel, sentAt } - the ledger that makes "one push per ask per
//              channel" true across restarts, not just within one run.
//  lanes       lane -> LaneRuntime: what the platform is ACTUALLY doing (usable, reason,
//              handleSeen, pausedUntil), deliberately separate from the owner's intent in
//              config (engage.lanes[lane].enabled) so no control ever has an undefined middle.
//  counters    `${lane} ${kind} ${YYYY-MM-DD}` -> n, in the client's local date: the daily
//              cap accounting. Incremented on `done` only.
//
// Always the FULL shape, even on a fresh state.json, so no caller ever branches on undefined
// (a missing counters map would read as "nothing posted today" at exactly the wrong moment).
// Mutated in place by the caller, then persisted with the shared saveState().
export function engageState() {
  const state = loadState();
  state.engage = state.engage && typeof state.engage === 'object' && !Array.isArray(state.engage) ? state.engage : {};
  for (const k of ['queue', 'asks', 'themes', 'pushes']) {
    if (!Array.isArray(state.engage[k])) state.engage[k] = [];
  }
  for (const k of ['communities', 'lanes', 'counters']) {
    if (!state.engage[k] || typeof state.engage[k] !== 'object' || Array.isArray(state.engage[k])) state.engage[k] = {};
  }
  if (typeof state.engage.lastTriageRunId !== 'string') state.engage.lastTriageRunId = '';
  return state;
}

const RADAR_JOB_CAP = 20;

// Stage 4: roots with a research pipeline in flight. Between per-source lane spawns the
// agent-runner's `running` map is momentarily empty (one child exits before the next starts),
// but the job is NOT done - so isJobRunning() alone would let a concurrent scan slip into that
// gap and start a second row. This in-memory flag spans the WHOLE pipeline. Per-process, so a
// daemon restart clears it (a genuinely orphaned running row is settled by listRadar stale repair).
...
// Cadence is no longer per-query (it is the global posting.radar.dailyEnabled switch), so both
// the daily sweep and a manual scan run the SAME set: every enabled query. The daily arming
// lives in radar-sweep.mjs (dailyEnabled + dueDailyAt); this only selects what to scan.
function radarQueriesToRun(radar, queryId) {
  const all = Array.isArray(radar.queries) ? radar.queries : [];
  const enabled = all.filter((q) => q && q.enabled !== false);
...
export async function runRadarScan({ clientId, queryId, cadence } = {}) {
  void clientId; // per-call client scoping is bound by withClient at the call site
  void cadence; // deprecated: per-query cadence retired; still accepted so an older caller never errors
  const posting = getPosting();
  ...
  const queries = radarQueriesToRun(radar, typeof queryId === 'string' && queryId.trim() ? queryId.trim() : null);
  ...
  // The default fan-out for a query with no sources[] is the SAME derived set the agent brief
  // and the Studio strips read (effectiveRadarSources: Setup-card flags, then
  // posting.skippedPlatforms), narrowed to the searchable engines - not a blanket
  // RADAR_SOURCES. That blanket is how a skipped lane (bluesky, live client) was searched on
  // every scan and parked a standing not_connected row. Connection state is irrelevant for
  // the searchable lanes (their default is ON), so no accountStatus() read here.
  const defaultLanes = effectiveRadarSources(radar, () => false, posting.skippedPlatforms);
  for (const query of queries) {
    const wanted = radarQueryLanes(query, defaultLanes);
    for (const source of wanted) {
      // D7 (audit 2026-08-31): forward the CONFIGURED lookback to the engine so a lane with
      // fixed search windows (reddit's `t`) can size its query to it, instead of hardcoding
      // a week while withinLookback below enforces something else. Only a configured value
      // is forwarded - engines keep their historical defaults otherwise.
      const laneQuery = Number.isFinite(Number(radar.lookbackDays)) && Number(radar.lookbackDays) > 0
        ? { ...query, lookbackDays: Number(radar.lookbackDays) }
        : query;
      const res = await runLaneRadar(source, laneQuery);
      if (!res || res.ok !== true) {
        ...
        if (!sources[source] || sources[source].ok !== true) {
          sources[source] = { ok: false, error: (res && res.error) || 'engine_failure', ...(res && res.scope ? { scope: res.scope } : { scope: RADAR_SOURCE_SCOPE[source] || null }), ...(res && res.detail ? { detail: res.detail } : {}) };
        }
        continue;
      }
      // D6/D8 (audit 2026-08-31): a HALF-failed lane (items collected, then a sub-search
      // degraded - reddit's per-subreddit loop, mastodon's keyword-vs-hashtag split) used
      // to read as plain ok, masking a failing credential for as long as any other
      // sub-search still yielded. The ok result now carries the degrade; record it as the
      // per-source status (partial:true - items DID ingest) unless a clean ok already
      // stands. A later clean ok for the same source still wins (real health beats a
      // stale error - the same rule as the failure branch above).
      if (res.degrade && typeof res.degrade === 'object') {
        if (!sources[source] || sources[source].ok !== true) {
          sources[source] = {
            ok: false,
            partial: true,
            error: res.degrade.error || 'engine_failure',
            scope: res.degrade.scope || RADAR_SOURCE_SCOPE[source] || null,
            ...(res.degrade.retryAfter != null ? { retryAfter: res.degrade.retryAfter } : {}),
            ...(res.degrade.detail ? { detail: res.degrade.detail } : {}),
          };
        }
      } else {
      ...
      for (const raw of res.items || []) {
        ...
        // Honor the lookback window (posting.radar.lookbackDays, default 90): a thread older
        // than it never enters the feed, so a 7-year-old question can never surface looking
        // "found 3 hours ago". Undated is kept (fail-open) and flagged in the card - see
        // withinLookback. Applied here, BEFORE scoring/dedupe, exactly like excludeKeywords.
        if (!withinLookback(raw.ts, radar.lookbackDays, now)) continue;
  ...
  // B4/D12 (audit 2026-08-31): MERGE per source key, never overwrite the whole map - a scan
  // that ran only sources A,B must not erase C's persisted degrade note (the agent-scan lane
  // loop writes rows for ITS sources into the same map). Within-scan "a later clean ok wins"
  // is already applied while building `sources` above; across scans, the newest scan's row
  // for a source it actually ran replaces that source's standing row.
  state.radar.sources = mergeRadarSourceRows(state.radar.sources, sources);
  // A SKIPPED lane has no status to report: drop its standing row unless this very scan ran it
  // (a query naming it explicitly). Without this the not_connected row a skipped lane earned
  // before the skip reached Radar would outlive the skip forever (B4/D12 merges, never prunes).
  for (const id of Object.keys(state.radar.sources || {})) {
    if (!(id in sources) && radarSourceSkipped(id, posting.skippedPlatforms)) delete state.radar.sources[id];
  }
  ...
  // (never a live-recompute that could disagree with the persisted digest value). The
  // footprint is the second derivation input (R4/P1): rivals AI assistants repeatedly
  // name instead of the brand mint entries too.
  state.radar.geo.comparisonBacklog = comparisonBacklog(state.radar.signals, state.radar.geo.dismissedBacklog, state.radar.geo.footprint, radarBrandContext());
  saveState();

  // The MERGED map rides the response (not just this scan's slice), so a caller reads the
  // same per-source truth the panel renders from state.
  return { ok: true, enabled: true, items: sortByIntent(state.radar.signals), sources: state.radar.sources, lastScan: state.radar.lastScan, scanned: fresh.length };
}
...
export async function radarIngest({ clientId, actor, queryId, signals, suggestions } = {}) {
  ...
  const fresh = [];
  let dropped = 0;
  let staleDropped = 0; // of `dropped`, how many were a KNOWN-old post beyond the lookback window
  let droppedUndated = 0; // of `dropped`, how many carried NO usable ts while a lookback window was set
  ...
  for (const raw of batch) {
    ...
    // The lookback window applies to the agent path too (same call as runRadarScan, same
    // BEFORE-scoring seam): the scan prompt already asks for posts inside the window, but the
    // prompt is an instruction and this is the fence. A known-old find is reported dropped,
    // never silently discarded. An UNDATED find (normalizeSignal's coerceTs yields null for a
    // missing or unparseable ts) is dropped too - fail closed (Wave 3 Q3) - and itemized apart
    // from the stale ones, because the two have different fixes: stale = the child searched too
    // far back; undated = the child did not read the date, which the result text below names.
    if (!withinLookback(normalized.ts, radar.lookbackDays, now)) {
      dropped += 1;
      if (normalized.ts == null) droppedUndated += 1; else staleDropped += 1;
      continue;
    }
  ...
  if (job) {
    // A8/B6: the zero-result witness - count the CALL itself, before any per-signal verdicts,
    // so an all-dropped batch (and even an honest empty []) still proves the child tried.
    job.ingestCalls = (job.ingestCalls || 0) + 1;
  ...
  // The result is what the child READS (the MCP face renders it as text), so the undated tally
  // carries a sentence, not only a number: a child that sees "3 dropped for missing ts" sends ts on
  // its next call; a child that sees dropped:3 shrugs.
  const note = droppedUndated > 0
    ? `${droppedUndated} signal${droppedUndated === 1 ? '' : 's'} dropped for missing ts: the lookback window (${Number(radar.lookbackDays)} days) needs the post date, and an undated find is outside it. Send ts (ISO-8601) on every signal whose date you can read from the page; never guess one.`
    : null;
  // Spec 50 §7.4: triage after an ingest BATCH, so a chat agent's findings get decided without
  // waiting for the next scan. During a scan the runner refuses the second child anyway and the
  // scan's own post-draft hook covers those signals, so this never doubles up.
  if (fresh.length) triggerEngageTriage('ingest');
  return { ok: true, accepted: fresh.length, dropped, staleDropped, droppedUndated, deduped, total: state.radar.signals.length, lastScan: state.radar.lastScan, ...(note ? { note } : {}) };
}

// ---------------------------------------------------------------------------
// Spec 50 P1 (§7.4): THE TRIAGE HOOK. Two call sites - after a scan's draft phase, and after an
// ingest batch - and both are this one line, so the trigger rules live in exactly one place
// (lib/engage-triage.mjs owns the mode gate, the 15-minute floor, the candidate pick and the
// fence; this only decides WHEN to ask).
//
// Deliberately DETACHED and deliberately silent on failure. Detached because triage spawns a
// child that can take minutes, and a scan that already landed its signals must settle its job
// row now rather than wait; silent because a triage that could not run is not a failure of the
// scan or the ingest that triggered it - the signals stay undecided and the next run retries
// them, which is spec 50 row 3e's whole design.
//
// The dynamic import is what keeps the static module graph acyclic: engage-triage imports
// engageState from THIS file. Same idiom as the radar-sweep import further down.
function triggerEngageTriage(reason) {
  try {
    const engage = (getPosting().radar || {}).engage || {};
    if ((engage.mode || 'off') === 'off') return;
    const root = activeRoot();
    withClient(root, async () => {
      const { runEngageTriage } = await import('./engage-triage.mjs');
      return runEngageTriage({ reason });
    }).catch(() => { /* see the note above: an undecided signal is a retry, not an incident */ });
  } catch { /* never let the hook break its caller */ }
}
...
function newJob(queryId, providerId, sources = [], scope = 'feed', actor = null) {
  return {
    // L7 (audit 2026-08-31): WHO initiated this spend, stamped at creation - 'scheduler'
    // for the unattended daily scan, the operator/GUI/MCP actor otherwise. The daily
    // budget counts only scheduler-actor feed rows (agentJobsToday in radar-sweep.mjs +
    // the inline mirror below), because manual scans are documented budget-exempt.
    ...
    // KI-Sichtbarkeit recheck (no signal research, no drafting); 'draft-one' = the operator's
    // draft-this-signal-for-review tap (no research, one held-pending draft); 'followup' = the
    // agent-lane author-reply read (owner decision 4: x/youtube/nostr follow-up via the
    // connected agent - read-and-report only, never drafts, never posts). The row reads its
    // lead from this.
    scope: scope === 'geo' || scope === 'draft-one' || scope === 'followup' ? scope : 'feed',
    providerId,
    ...
    dropped: 0,
    deduped: 0,
    // A8/B6 (audit 2026-08-31): the ZERO-RESULT WITNESS - how many radar_ingest CALLS the
    // server saw while this job ran (tallied where `accepted` is, in radarIngest). It is the
    // server's measurement, never the child's claim, and it splits two states that used to be
    // byte-identical: "searched, found nothing" (ingestCalls>0 possible with everything
    // dropped - the counts tell the story) vs "never even tried" (ingestCalls===0).
    ingestCalls: 0,
    drafted: 0,
    exitCode: null,
    // Machine-readable WHY on the settled row. Failure reasons come from the runner
    // (timeout/sleep/limit/agent_error/...; 'sleep' = a timeout that overlapped metered
    // computer sleep, R4); 'stopped' is the operator's own act; 'partial'/
    // 'partial_timeout' caveat a promoted partial. A8/B6: a feed run that settles CLEAN
    // `done` with zero ingest calls AND zero accepted carries reason:'no_results' - a
    // DEGRADE MARKER on a done row (state stays 'done', it is not a failure and not in any
    // retry whitelist); the UI/MCP reads it to say "the agent never called radar_ingest"
    // instead of a bare "done, 0 signals".
    reason: null,
    // Stage 2: a research spawn that overran the budget but had already ingested signals is
    // promoted to a done-with-a-caveat run instead of a red dead-end. `partial` is what the row
    // reads to render an amber caveat + retry rather than a failure. False on a clean run.
    partial: false,
    // Stage 3: bounded auto-retry bookkeeping. `attempt` is 1-based; `retrying` is true only during
    // the backoff between a transient failure and the next try, so the row can say "retrying".
    attempt: 1,
    maxAttempts: 2,
    retrying: false,
    tail: null,
    // NOT initialized here, stamped at settle only when it applies (B9, audit 2026-08-31):
    // `draftSkipped` - machine-readable reason the drafting phase was skipped on an otherwise
    // successful run ('no_campaign': signals landed but no active campaign existed to file
    // replies under, and no copy-lane signal was picked). Locale-independent, so the UI/MCP can
    // render a real CTA (create a campaign) instead of parsing the prose tail.
    // Also NOT initialized here (B11): `draftRefusals` - a { code: count } tally of
    // queueRadarReply refusals seen while this job was in flight (below_threshold, the target
    // fence's invalid_input, ...), stamped by the queueRadarReply wrapper. Present only when a
    // refusal happened, so old rows and clean runs carry nothing.
...
function draftableSignals(state) {
  ...
  // THE DRAFT THRESHOLD (engagement engine, owner decision 2026-08-17): drafting.minScore,
  // DECOUPLED from autoReply.minScore. The old coupling under-drafted by design: the shared
  // threshold guarded auto-POSTING, so it had to fail closed (engine-scored and unscored
  // signals dropped too). A draft that lands PENDING is harmless by construction - the human
  // gate is still in front of it - so the drafting rule is LOOSENED: only a signal the AGENT
  // scored BELOW drafting.minScore is skipped (the agent judged it not worth answering);
  // engine-scored and unscored signals draft regardless, ranked by score, capped below.
  // The auto-approve gate in queueRadarReply keeps the strict agent-scored-only rule on
  // autoReply.minScore, byte-unchanged - volume changed, autonomy did not.
  // The cap is drafting.maxPerRun (the DRAFT-pick budget); agent.maxPerRun stays the
  // research-ingest ask - two knobs, two budgets, no longer one knob serving both.
  const radarCfg = (() => { try { return getPosting().radar || {}; } catch { return {}; } })();
  const drafting = radarCfg.drafting || {};
  const draftMin = Number.isInteger(drafting.minScore) ? drafting.minScore : 30;
  const draftCap = Number.isInteger(drafting.maxPerRun) ? drafting.maxPerRun : 20;
  // PER-CLIENT source sets (owner round 3, point 6): an xEnterprise client's x signals route to
  // the real reply lane, everyone else's to the copy path - the same effectiveRadarCapabilities
  // view queueRadarReply already applies at the door, so the pick and the door can never disagree.
  const replyLanes = radarReplySources(radarCfg);
  const copyLanes = radarCopyDraftSources(radarCfg);
  ...
  return (state.radar.signals || [])
    ...
    .filter((s) => replyLanes.includes(s.source) || copyLanes.includes(s.source))
    ...
    // Skip ONLY what the agent itself judged below the drafting threshold; a not-yet-judged
    // signal drafts (pending is harmless; see the header comment).
    .filter((s) => !(s.scoredBy === 'agent' && Number(s.intentScore) < draftMin))
...
// A8/B6 (audit 2026-08-31): should a CLEAN done row carry the 'no_results' degrade marker?
// Only for a FEED-scope run (geo/followup/draft-one jobs do not ingest signals by design),
// and only when the server witnessed ZERO ingest calls AND zero accepted signals - the
// "authenticated-but-idle child settles done with only free-text evidence" case. An
// all-dropped run (ingestCalls>0, accepted 0) is NOT stamped: the child demonstrably tried,
// and the dropped/deduped counts already tell that story. Pure; exported for tests.
export function radarNoResultsReason({ scope = 'feed', ingestCalls = 0, accepted = 0 } = {}) {
  return scope === 'feed' && ingestCalls === 0 && accepted === 0 ? 'no_results' : null;
}
...
function finishJob(jobId, patch) {
  ...
  // L8 (audit 2026-08-31): finishedAt is stamped ONLY by a patch that SETTLES the row
  // (state done|failed). This function is also the mid-run patcher (phase transitions,
  // attempt/retrying bookkeeping), and stamping those left live rows reading
  // state:'running' with a finishedAt 16ms after start - which `lastProduced` and the
  // panel then read as "when the run ended". A running row honestly has finishedAt:null.
  const settles = patch && (patch.state === 'done' || patch.state === 'failed');
  Object.assign(job, patch, ...(settles ? [{ finishedAt: now }] : []));
...
// Stage 3: run ONE research spawn with a bounded auto-retry. A TRANSIENT start failure - the child
// would not spawn, or quit before saying anything - is retried once after a short backoff before the
// operator ever sees an error, the kind of blip a second press would have fixed anyway. NOT retried:
// timeout / sleep (wall-clock facts, promoted to partial instead; a slept lane is the daily
// resume's job, radar-sweep.mjs resumeFailedLanes), stopped (the operator's own act),
// agent_error / limit / no_credential / unknown_provider (permanent - a retry just re-spends the
// subscription). The retry lives inside the SAME job row, so the daily budget still counts it once;
// the spend is up to 2x. The caller keeps the draft fence armed across the whole call.
const RETRYABLE_RESEARCH = new Set(['spawn_failed', 'exit']);
async function researchWithRetry(job, runArgs, { maxAttempts = 2, backoffMs = 2000 } = {}) {
  let run;
  let attempt = 0;
  while (true) {
    attempt++;
    finishJob(job.id, { attempt, retrying: false });
    run = await runAgentJob(runArgs);
    if (run.ok || attempt >= maxAttempts || !RETRYABLE_RESEARCH.has(run.error)) break;
    finishJob(job.id, { retrying: true }); // the row reads "retrying" during the backoff
    await new Promise((resolve) => { const timer = setTimeout(resolve, backoffMs); timer.unref?.(); });
  }
  finishJob(job.id, { retrying: false });
  return run;
}
...
// The active tenant's identity for GEO comparison-backlog derivation: the brand DISPLAY name
// (this client's own, never a hardcoded 'pendpost' - the leak that emailed a second client's
// operator "pendpost vs Buffer") plus the union of the operator's DECLARED rivals
// (competitorsDefault ∪ every saved query's competitors). comparisonBacklog + shareOfVoice
// take these to (a) label pages "<brand> vs X" and (b) constrain the vs/alt/better extractor
// to real competitor comparisons for THIS tenant, so an off-topic "Arduino vs Evil" HN thread
// mints nothing. Reads the ALS-bound client (the scan/ingest/footprint verbs void their
// clientId arg - scoping is the withClient boundary), so it is correct wherever it is called.
function radarBrandContext() {
  const radar = (getPosting().radar) || {};
  const competitors = [
    ...(Array.isArray(radar.competitorsDefault) ? radar.competitorsDefault : []),
    ...((Array.isArray(radar.queries) ? radar.queries : [])
      .flatMap((q) => (Array.isArray(q && q.competitors) ? q.competitors : []))),
  ];
  return { brand: activeBrandName(alsClientId()), competitors };
}
...
// No phase 2, no drafting. The caller creates + saves the running row (scope:'geo') so the panel
// shows live progress before this returns; this only runs the spawn and settles the row.
// Engagement engine (owner decision 4): enumerate the AGENT-lane follow-up targets due
// right now - the server-side twin of reconcileAuthorReplies' engine-lane grouping. ONE
// derivation shared by the followup-scope job, the reconcile's arming decision, and
// listRadar's nextScan.followup.agentDue, so the three can never disagree:
//   - posted reply plan posts on an agent lane, non-terminal (needsAgentFollowupCheck),
//     carrying the author snapshot recorded at queue time (without an author there is
//     nobody to look for - skipped fail-soft, the reconcileCopyFollowups posture);
//   - PLUS non-terminal copy-posted ledger entries on agent lanes (the non-Enterprise x
//     path), joined to their cached signal for the author.
// Pure read; never throws (an unreadable plan store just yields the ledger targets).
export function agentFollowupTargets() {
  const targets = [];
  const seen = new Set();
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
      for (const p of c.posts || []) {
        if (!needsAgentFollowupCheck(p)) continue;
        const rr = p.radarReplyTo;
        const author = rr.author || null;
        if (!author) continue;
        const key = `${String(rr.source || '').toLowerCase()} ${rr.externalId}`;
        if (seen.has(key)) continue;
        seen.add(key);
        const evidence = resolveReplyPermalink(p);
        targets.push({
          kind: 'post',
          source: String(rr.source || '').toLowerCase(),
          externalId: rr.externalId,
          threadUrl: rr.url || null,
          author,
          ourReplyUrl: (evidence && evidence.url) || null,
          // The engine-minted id of OUR reply (loadPlanStore projects the minted ids into
          // post.ids; raw plan files carry them top-level - accept both).
          ourReplyId: (p.ids && (p.ids.ytCommentId || p.ids.xPostId || p.ids.nostrEventId)) || p.ytCommentId || p.xPostId || p.nostrEventId || null,
          postedAt: p.postedAt || null,
        });
      }
    }
  } catch { /* plan store unreadable -> the ledger targets below still enumerate */ }
  const state = radarState();
  const signalsByKey = new Map();
  for (const s of state.radar.signals || []) signalsByKey.set(signalKey(s), s);
  for (const e of state.radar.copyPosted || []) {
    const src = String(e.source || '').toLowerCase();
    if (!RADAR_AGENT_FOLLOWUP_SOURCES.includes(src)) continue;
    if (e.radarReplyState === 'author_replied' || e.radarReplyState === 'target_gone') continue;
    const key = `${src} ${e.externalId}`;
    if (seen.has(key)) continue;
    const sig = signalsByKey.get(signalKey(e));
    const author = sig && sig.author;
    if (!author) continue; // fail-soft: without the author there is nobody to look for
    seen.add(key);
    targets.push({
      kind: 'copy', source: src, externalId: e.externalId,
      threadUrl: (sig && sig.url) || null, author,
      ourReplyUrl: e.postedUrl || null, ourReplyId: null, postedAt: e.at || null,
    });
  }
  return targets;
}

// How many targets ONE follow-up job may carry. Bounded like the draft pick: a read pass over
// ten threads is one cheap spawn; an unbounded one is a job whose length nobody chose.
const FOLLOWUP_MAX_PER_RUN = 10;

// The followup-scope spawn (execGeoJob's sibling): ONE read-and-report child over the
// server-enumerated targets, the follow-up fence armed with exactly those keys for the
// child's lifetime (disarmed in finally - fail-closed again the moment the job settles),
// and the tally counted from DISK (spec 41 §4.5), never from the child's closing line.
async function execFollowupJob({ job, boundClientId, providerId, targets }) {
  const onEvent = jobActivityRecorder(job.id, providerId);
  const jobStartMs = Date.parse(job.startedAt) || Date.now();
  let run;
  beginFollowupFence(targets.map((t) => `${t.source} ${t.externalId}`));
  try {
    run = await runAgentJob({
      providerId,
      prompt: radarFollowupPrompt(targets, { clientId: boundClientId, noteLocale: getPosting().locale || null }),
      allowedTools: [...AGENT_FOLLOWUP_TOOLS],
      jobId: job.id,
      stream: true,
      onEvent,
    });
  ...
  const tally = tallyFollowupStamps(targets, jobStartMs);
  ...
  return { ok: true, enabled: true, job: finished, checked: tally.checked, replied: tally.replied, repliedSources: tally.sources };
}

// The disk tally for a follow-up job: an enumerated target counts CHECKED when its
// radarFollowup.lastCheckedTs is at/after the job start, and REPLIED when it now carries the
// terminal author_replied stamped in this window (enumeration excluded already-terminal
// targets, so a terminal state + a fresh lastCheckedTs is NEW). Plan files are re-read from
// disk (the in-process plan store caches; the reconcile reads fresh for the same reason).
function tallyFollowupStamps(targets, sinceMs) {
  let checked = 0;
  let replied = 0;
  const sources = new Set();
  const want = new Map();
  for (const t of targets) want.set(`${t.source} ${t.externalId}`, t);
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
      if (!c || !c.path || !want.size) continue;
      const absPath = path.isAbsolute(c.path) ? c.path : path.resolve(activeRoot(), c.path);
      let fresh = null;
      try { fresh = JSON.parse(fs.readFileSync(absPath, 'utf8')); } catch { continue; }
      for (const p of fresh.posts || []) {
        const rr = p && p.radarReplyTo;
        if (!rr || !rr.externalId) continue;
        const key = `${String(rr.source || '').toLowerCase()} ${rr.externalId}`;
        if (!want.has(key)) continue;
        const ts = Date.parse((p.radarFollowup && p.radarFollowup.lastCheckedTs) || '') || 0;
        if (ts >= sinceMs) {
          checked += 1;
          if (p.radarReplyState === 'author_replied') { replied += 1; sources.add(String(rr.source || '').toLowerCase()); }
        }
        want.delete(key); // each target counts once
      }
    }
  } catch { /* plan store unreadable -> the ledger half still counts */ }
  const state = radarState();
  for (const e of state.radar.copyPosted || []) {
    const key = `${String(e.source || '').toLowerCase()} ${e.externalId}`;
    if (!want.has(key)) continue;
    const ts = Date.parse((e.radarFollowup && e.radarFollowup.lastCheckedTs) || '') || 0;
    if (ts >= sinceMs) {
      checked += 1;
      if (e.radarReplyState === 'author_replied') { replied += 1; sources.add(String(e.source || '').toLowerCase()); }
    }
    want.delete(key);
  }
...
async function execGeoJob({ job, boundClientId, radar, providerId, geoQuestions }) {
  const onEvent = jobActivityRecorder(job.id, providerId);
  ...
  try {
    run = await runAgentJob({
      providerId,
      prompt: radarGeoPrompt(geoQuestions, { clientId: boundClientId, brandName: activeBrandName(boundClientId), competitors, noteLocale: getPosting().locale || null }),
...
export async function radarAgentScan({ clientId, queryId, actor, cadence, scope, target, sources, wait = true } = {}) {
  void cadence; // deprecated: per-query cadence retired (global posting.radar.dailyEnabled now); still accepted so an older caller never errors
  ...
  // process cannot inherit AsyncLocalStorage. `|| alsClientId()` (lib/context.mjs boundClientId)
  // resolves the withClient binding FIRST - the scheduler tick calls this with no clientId from
  // inside withClient(clientRoot(id)), and the registry's activeClientId is whichever client the
  // operator's GUI happens to have open, NOT the client this sweep is for. Reading the registry
  // here is exactly how months of daily scans embedded the WRONG clientId in the child prompt
  // (campaign computed for client A, findings filed against client B - the 2026-08-31 audit's
  // L2/L9). MCP/HTTP entries are unchanged: callTool/handleApi bind the same id they resolved.
  const boundClientId = clientId || alsClientId();
  // Captured once for the guards, the withClient re-binding, and the pipeline-flag cleanup - the
  // detached continuation runs after our caller's AsyncLocalStorage scope unwinds, so activeRoot()
  // inside a .finally could resolve differently; this is the stable client key.
  const root = activeRoot();
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const posting = getPosting();
  ...
  if (scope === 'geo') {
    ...
    if (isJobRunning() || activePipelines.has(root)) return errorBody('in_flight', 'an agent job is already running for this client - stop it or wait for it to finish', { retryAfter: 30 });
    // Stage 1: create the running row synchronously, then run the check DETACHED. A geo recheck is
    // a live spawn too, and awaiting it here is what let the browser's multi-second POST die at the
    // network layer and dump a raw "Load failed". The row is saved before we return, so the panel
    // renders it from GET /api/radar polling. wait:true (the MCP twin) still returns the result.
    const geoState = radarState();
    const geoJob = newJob(null, providerId, [], 'geo', actor);
    geoState.radar.jobs = [geoJob, ...geoState.radar.jobs].slice(0, RADAR_JOB_CAP);
    saveState();
    activePipelines.add(root);
    const gp = withClient(root, () => execGeoJob({ job: geoJob, boundClientId, radar, providerId, geoQuestions })).finally(() => activePipelines.delete(root));
    if (wait) return await gp;
    gp.catch(() => {});
    return { ok: true, enabled: true, job: geoJob };
  }

  // ---- scope:'draft-one' - the operator's draft-THIS-signal-for-review tap -----------------------
  // The "Wird automatisch gepostet" badge's action: the agent drafts a reply for exactly one
  // cached signal NOW, with the fence armed holdApproval - the auto-reply policy stands down and
  // the draft lands PENDING for the human who asked to read it. No research spawn, one cheap
  // drafting spawn. Auto-post for every untapped signal is untouched.
  if (scope === 'draft-one') {
    const src = String(target?.source || '').trim().toLowerCase();
    const ext = String(target?.externalId || '').trim();
    if (!src || !ext) return errorBody('invalid_input', 'scope:"draft-one" needs target { source, externalId } (from radar_list items[])');
    const st0 = radarState();
    const sig = (st0.radar.signals || []).find((s) => s && s.source === src && s.externalId === ext);
    if (!sig) return errorBody('invalid_input', 'target signal is not in the cached feed - run a scan first');
    if (sig.draft) return errorBody('invalid_input', 'this signal already carries a draft - open it on the card');
    if (countRadarRepliesFor([sig]) > 0) return errorBody('invalid_input', 'this signal already has a queued or posted reply');
    const draftCampaign = defaultRadarCampaign();
    const copyLane = radarCopyDraftSources(radar).includes(src);
    if (!draftCampaign && !copyLane) return errorBody('invalid_input', 'no active campaign to file the reply under - create one in the planner first');
    if (isJobRunning() || activePipelines.has(root)) return errorBody('in_flight', 'an agent job is already running for this client - stop it or wait for it to finish', { retryAfter: 30 });
    const doState = radarState();
    const doJob = newJob(null, providerId, [src], 'draft-one', actor);
    // The target key rides the job row so the one signal card can render its own busy
    // state ("Entwurf wird erstellt") while the child runs - same `source externalId`
    // key the fence and the DOM anchor use.
    doJob.target = signalKey(sig);
    doState.radar.jobs = [doJob, ...doState.radar.jobs].slice(0, RADAR_JOB_CAP);
    saveState();
    activePipelines.add(root);
    const dp = withClient(root, async () => {
      const onEvent = jobActivityRecorder(doJob.id, providerId);
      finishJob(doJob.id, { phase: 'drafting', draftTargets: 1 });
      // ARM THE FENCE for exactly this signal, WITH the review hold (see queueRadarReply).
      beginDraftFence([signalKey(sig)], { holdApproval: true });
      let draftRun;
      try {
        draftRun = await runAgentJob({
          providerId,
          prompt: radarDraftPrompt([sig], {
            voice: (getPosting().radar || {}).replyVoiceDefault || '',
            brand: (getPosting().radar || {}).brand || null,
            campaign: draftCampaign,
            clientId: boundClientId,
            // Held for review BY CONSTRUCTION: this child's words never post unread.
            autoPosts: false,
            // The operator picked this exact thread; a threshold skip would be refusing them.
            minScore: null,
            noteLocale: getPosting().locale || null,
          }),
      ...
      // The tally is OURS (spec 41 §4.5): count what exists on disk, never the child's claim.
      const drafted = countRadarRepliesFor([sig]) + countCopyDraftsFor([sig]);
      // An agent that examined the thread and WROTE nothing made a decision, not an error.
      // Stamp it on the signal so the pre-fire badge stops predicting an auto-post the agent
      // already declined (best-effort: a later re-scan may replace the signal object).
      if (draftRun.ok && drafted === 0) {
        const st1 = radarState();
        const live = (st1.radar.signals || []).find((s) => s && s.source === src && s.externalId === ext);
        if (live) {
          live.agentDeclined = { ts: new Date().toISOString(), reason: String(draftRun.detail || draftRun.tail || '').slice(0, RADAR_FIELD_CAP) || null };
          saveState();
        }
      }
      const finished = finishJob(doJob.id, {
        phase: null,
        drafted,
        state: draftRun.ok ? 'done' : 'failed',
        reason: draftRun.ok ? (drafted > 0 ? null : 'declined') : (draftRun.error || 'failed'),
        exitCode: draftRun.exitCode ?? null,
        tail: draftRun.detail || draftRun.tail || null,
      });
      notifyRadarScanDone(finished);
      return { ok: true, enabled: true, job: finished };
    }).finally(() => activePipelines.delete(root));
    if (wait) return await dp;
    dp.catch(() => {});
    return { ok: true, enabled: true, job: doJob };
  }

  // ---- scope:'followup' - the agent-lane author-reply read (owner decision 4) ------------------
  // The x/youtube/nostr twin of the engine-verb reconcile. The SERVER enumerates the due
  // targets (never the child), spawns ONE read-and-report child with the follow-up fence
  // armed for exactly those keys, and tallies from disk afterwards. Budget-wise it sits
  // with scope:'geo' BEFORE the scheduler budget gate below: the CALLER owns the spend
  // decision (the scheduled reconcile refuses over-budget before spawning; a forced check
  // is the operator's own money) - and the job row still counts toward agentJobsToday by
  // construction, so the budget is respected without a second gate.
  if (scope === 'followup') {
    const due = agentFollowupTargets();
    // ZERO due targets -> no spawn, no job row, no spend. An honest empty, not a failure.
    if (!due.length) return { ok: true, enabled: true, job: null, checked: 0, replied: 0 };
    if (isJobRunning() || activePipelines.has(root)) return errorBody('in_flight', 'an agent job is already running for this client - stop it or wait for it to finish', { retryAfter: 30 });
    const targets = due.slice(0, FOLLOWUP_MAX_PER_RUN);
    const fuState = radarState();
    const fuJob = newJob(null, providerId, [...new Set(targets.map((t) => t.source))], 'followup', actor);
    // Additive row facts: how many targets this job carries (the cap made visible) and the
    // disk-counted results (finished by execFollowupJob).
    fuJob.targets = targets.length;
    fuJob.checked = 0;
    fuJob.replied = 0;
    fuState.radar.jobs = [fuJob, ...fuState.radar.jobs].slice(0, RADAR_JOB_CAP);
    saveState();
    activePipelines.add(root);
    const fp = withClient(root, () => execFollowupJob({ job: fuJob, boundClientId, providerId, targets })).finally(() => activePipelines.delete(root));
    if (wait) return await fp;
    fp.catch(() => {});
    return { ok: true, enabled: true, job: fuJob };
  }

  // Every enabled query (per-query cadence retired; the daily arming is the global
  // posting.radar.dailyEnabled switch, applied in radar-sweep.mjs before we get here).
  const queries = radarQueriesToRun(radar, queryId);
  if (!queries.length) {
  ...
  // `running` row that nothing will ever finish. The activePipelines flag also closes the
  // between-lane gap (Stage 4), where the per-source loop is mid-pipeline but no child is live.
  if (isJobRunning() || activePipelines.has(root)) return errorBody('in_flight', 'an agent research job is already running for this client - stop it or wait for it to finish', { retryAfter: 30 });
  ...
  if (actor.trim() === 'scheduler') {
    ...
    // Mirrors agentJobsToday() in radar-sweep.mjs (kept inline - importing it here would be a
    // cycle): a limit-refused job spent nothing, so it does not consume the unattended budget;
    // and (L7) only scheduler-actor FEED rows count - manual scans are budget-exempt, and
    // geo/followup/draft-one rows are separately gated. Legacy actor-less rows count.
    const spent = (state.radar.jobs || []).filter((j) => j.reason !== 'limit'
      && (Date.parse(j.startedAt) || 0) > Date.now() - 24 * 3600 * 1000
      && (j.scope || 'feed') === 'feed'
  ...
  const scanSources = effectiveRadarSources(radar, (id) => {
    // accountStatus() is keyed by LANE, not radar source id: Instagram lives under the
    // `meta` lane (there is no top-level `instagram` key), so map it or auto-arm-on-connect
    // would never fire for an `instagram` source. linkedin/x/youtube/nostr key directly.
    const a = acct[id === 'instagram' ? 'meta' : id];
    return Boolean(a && (a.authenticated || a.configured));
  }, posting.skippedPlatforms);
  // The agent researches only the AGENT-FOUND sources (search:false: x/youtube/nostr/linkedin/
  // instagram). The SEARCH lanes (reddit/mastodon/bluesky/hackernews) are the native ENGINE's
  // job (runRadarScan, real API search) - having the agent WebFetch them is wasted budget (Reddit
  // is even blocked for the agent) and, because lanes run sequentially under the 15-min job cap,
  // it pushes the agent-found lanes to the back where they may never run. Fallback: if nothing is
  // agent-found, keep the full set so a GEO-only feed scan still has a lane to ride. The job row
  // keeps the full `scanSources` (coverage = engine + agent), so the strip is unchanged.
  const researchSources = agentResearchSources(scanSources);
  let scanLanes = researchSources.length ? researchSources : scanSources;
  // Wave 1 R3 (2026-09-04): an OPTIONAL `sources` narrows the research to exactly those lanes -
  // the daily resume (radar-sweep.mjs resumeFailedLanes) re-runs only the lanes that died at
  // their slice, and an operator/agent can re-run one lane without re-spending on the rest.
  // Validated against BOTH fences: a key of RADAR_CAPABILITIES (a real source id) AND a member
  // of the effective scan set above (the operator's toggles + connections still decide what may
  // be researched at all - `sources` can narrow that set, never widen it). The job row keeps the
  // full `scanSources` as its coverage strip; `lanes` below names what this job actually ran.
  if (sources !== undefined && sources !== null) {
    if (!Array.isArray(sources) || !sources.length || !sources.every((x) => typeof x === 'string' && x.trim())) {
      return errorBody('invalid_input', 'sources must be a non-empty array of radar source ids (e.g. ["x","youtube"])');
    }
    const want = [...new Set(sources.map((x) => x.trim().toLowerCase()))];
    const unknown = want.filter((x) => !RADAR_CAPABILITIES[x]);
    if (unknown.length) return errorBody('invalid_input', `unknown radar source '${unknown[0]}' (sources: ${Object.keys(RADAR_CAPABILITIES).join(', ')})`);
    const outside = want.filter((x) => !scanSources.includes(x));
    if (outside.length) return errorBody('invalid_input', `source '${outside[0]}' is not in the effective scan set (${scanSources.join(', ') || 'none'}) - enable it in Setup (posting.radar.sources) or connect its lane first`);
    scanLanes = want;
  }

  const job = newJob(queryId, providerId, scanSources, 'feed', actor);
  // Additive row fact (R3): the lanes this job researches - the narrowed set when `sources` was
  // given, else the derived agent-found subset. `sources` above stays the coverage strip.
  job.lanes = [...scanLanes];
  state.radar.jobs = [job, ...state.radar.jobs].slice(0, RADAR_JOB_CAP);
  saveState();

  // Stage 1: run the scan DETACHED. The running row is already saved above, and useSignals polls
  // GET /api/radar every 3s while a job runs - so the multi-minute agent spawn does NOT need the
  // browser to hold the POST open (that is exactly what died at the network layer and dumped a raw
  // "Load failed" into the panel). withClient(root) keeps this client's AsyncLocalStorage alive
  // across the detached awaits; wait:true (the MCP twin, the scheduler) still awaits the settle.
  activePipelines.add(root);
  const p = withClient(root, async () => {
  ...
  // Stage 4: PER-SOURCE ISOLATION. The one-child chokepoint (lib/agent-runner.mjs) refuses a second
  // concurrent spawn per client - by design, for cost + tally soundness - so isolation is SEQUENTIAL:
  // one research spawn per source, each with its own slice of the budget. A source that hangs dies at
  // its own sub-cap; the others still land, and signals ingest onto the running row as each lane runs.
  // The GEO buying-question check folds into the LAST lane only (once, no N-fold duplicate checks),
  // and only when the deadline still leaves room - signal research runs first and is never starved
  // by the 10-question visibility check (see the loop's laneGeo guard). The SCHEDULER packs exactly
  // like a manual scan (Wave 1 R1, 2026-09-04): the
  // old scheduler-only single-spawn exception was justified by "the daily budget counts jobs, not
  // spawns" - and it still does (one job row, however many spawns), so the exception bought nothing
  // but the cap-death the packing exists to prevent. A single-source scan => one combined spawn.
  // (agent.perSourceIsolation was a DEAD read here: never in the validator or the defaults, so no
  // config could ever set it - the planner alone decides the spawn count.)
  // The per-spawn slice FOLLOWS THE WORK (incident 2026-09-04): one lane per spawn only while
  // each spawn can still fit every saved query; otherwise lanes pack into fewer, longer spawns,
  // and in the limit into today's one combined spawn. The packed slices never sum past
  // AGENT_TIMEOUT_MS, so the job's total wall-clock (and the stale-repair bound) is unchanged.
  const { lanes, perLaneMs } = planAgentScanLanes({ actor, scanLanes, queryCount: queries.length });
  const sourceResults = {};
  let stoppedMid = false;
  let run = null;
  // FAIL-SAFE (2026-09-06): ONE overall deadline for the whole scan. Before, per-lane retries
  // (researchWithRetry, up to 2 attempts each) could push the total past AGENT_TIMEOUT into the
  // 20-min stale-repair cut (repairStaleRadarJobs), which then marked a STILL-RUNNING job
  // failed/stale - the exact 20-min death seen on 2026-09-06. Now: each lane gets the SMALLER of
  // its planned slice and the time actually left; a lane retries only if a full retry still fits;
  // and once the budget is spent the loop STOPS starting lanes and settles clean partial_timeout
  // with whatever already ingested. Many small, individually-bounded searches - never one monolith
  // that runs until the watchdog kills it. LANE_FLOOR: below this a spawn can't do useful work.
  const LANE_FLOOR_MS = 30_000;
  const scanDeadline = Date.now() + AGENT_TIMEOUT_MS;
  beginDraftFence([]);
  try {
    for (let i = 0; i < lanes.length; i++) {
      const remaining = scanDeadline - Date.now();
      if (remaining < LANE_FLOOR_MS) break; // budget spent: stop cleanly, keep what already landed
      const laneSources = lanes[i];
      // GEO folds into the LAST lane, not the first, so the 10-question KI-Sichtbarkeit check never
      // STARVES signal research (it did: a single-lane scan for one client spent its whole budget on GEO
      // and ingested nothing). And only when the deadline still leaves room for it - else it is the
      // first thing dropped, because a found buying-signal is worth more than a visibility check.
      const laneGeo = (i === lanes.length - 1 && remaining > LANE_FLOOR_MS * 3) ? geoContext : null;
      // The lane's own budget: never more than the time left, so no lane can overrun the deadline.
      const laneMs = Math.max(LANE_FLOOR_MS, Math.min(perLaneMs, remaining));
      // Retry only if a SECOND full attempt (plus the 2s backoff) still fits before the deadline.
      const laneAttempts = remaining >= laneMs * 2 + 3_000 ? 2 : 1;
      // Stage 3 retry applies PER LANE; the fence stays armed across the whole loop.
      const laneRun = await researchWithRetry(job, {
        providerId,
        // noteLocale: the closing log line renders in the operator's UI + digest, so it follows
        // posting.locale (the UI language), NOT getContentLocale() - a de-CH dashboard posting
        // English content still reads its own job notes in German.
        // noteLocale = getPosting().locale (the UI-log language); searchLocale = getContentLocale()
        // (the SEARCH language + regional-source default). Deliberately two different locales: a
        // de-CH dashboard posting English content reads its job notes in German, but a brand whose
        // CONTENT is German must SEARCH in German - that is the leak this fixes. getContentLocale()
        // is ALS-bound here (withClient(root)), so it reads THIS client's contentLanguage->locale->en.
        prompt: radarScanPrompt(queries, maxPerRun, boundClientId, laneSources, laneGeo, getPosting().locale || null, radar.brand || null, radar.lookbackDays, laneMs, getContentLocale()),
        ...
        allowedTools: laneGeo ? [...AGENT_SCAN_TOOLS, 'mcp__pendpost__radar_footprint_log'] : [...AGENT_SCAN_TOOLS],
        timeoutMs: laneMs,
        jobId: job.id,
        stream: true,
        onEvent,
      }, { maxAttempts: laneAttempts });
      for (const s of laneSources) sourceResults[s] = laneRun.ok ? { ok: true } : { ok: false, error: laneRun.error };
      run = laneRun; // keep the last lane's envelope for tail/detail/exitCode
      if (laneRun.stopped) { stoppedMid = true; break; } // operator Stop halts the whole loop
    }
  } finally {
    endDraftFence();
  }

  // B4/D12 (audit 2026-08-31): persist the per-lane outcomes into state.radar.sources - the
  // SAME map the keyword engine writes and the panel's per-source degrade notice reads.
  // Before this, agent-scan lane failures were rolled into the job verdict but never reached
  // the sources map, so a dead x/youtube lane looked healthy on the Setup strip for as long
  // as no keyword scan touched it. Merged per source key (mergeRadarSourceRows): this scan
  // updates exactly the lanes it ran; every other source keeps its standing row.
  {
    const rows = {};
    for (const [s, r] of Object.entries(sourceResults)) {
      rows[s] = r.ok ? { ok: true } : { ok: false, error: r.error || 'agent_error', scope: RADAR_SOURCE_SCOPE[s] || null };
    }
    if (Object.keys(rows).length) {
      const st = radarState();
      st.radar.sources = mergeRadarSourceRows(st.radar.sources, rows);
      saveState();
    }
  }

  // ---- Roll the per-lane outcomes into ONE research verdict (Stage 2 partial-success folds in) ----
  //  - every lane ok             -> clean success, draft normally.
  //  - some lanes ok, some not   -> PARTIAL ('partial'): the sources that finished still landed.
  //  - no lane ok but signals in -> PARTIAL ('partial_timeout' when a lane timed out or slept): promote what
  //    already ingested rather than discard the operator's spend.
  //  - no lane ok, nothing found -> honest FAILURE with the dominant lane error.
  //  - operator Stop mid-loop    -> honest FAILURE ('stopped'), ingested signals kept, no drafting.
  // `radar_ingest` tallies onto the running row, so `accepted` is the sound count of what landed.
  const laneVals = Object.values(sourceResults);
  const laneErrors = laneVals.filter((r) => !r.ok).map((r) => r.error);
  const liveJob = runningRadarJob(radarState());
  const ingested = liveJob?.accepted || 0;
  // A8/B6: the witness count, read at the same seam as `accepted` (ingest only happens
  // during research, so this is final by the time the verdict rolls up).
  const ingestCallsSeen = liveJob?.ingestCalls || 0;
  // laneVals is empty only when there were no sources to scan; fall back to the single run's verdict.
  const researchOk = laneVals.length ? laneVals.some((r) => r.ok) : Boolean(run && run.ok);
  const anyFailed = laneVals.length ? laneVals.some((r) => !r.ok) : !researchOk;
  const partial = !stoppedMid && ((researchOk && anyFailed) || (!researchOk && ingested > 0));
  const clockErr = (e) => e === 'timeout' || e === 'sleep';
  const partialReason = researchOk ? 'partial' : ((laneErrors.some(clockErr) || (run && clockErr(run.error))) ? 'partial_timeout' : 'partial');
  // Draft when the research half succeeded OR there is a promotable partial - but never after a Stop.
  let drafted = 0;
  let autoPosted = 0;
  let draftTail = null;
  // B9 (audit 2026-08-31): WHY the drafting phase was skipped, machine-readable. The prose
  // draftTail alone left the UI/MCP nothing to key a CTA on - "done, drafted 0, no campaign"
  // was a locale-bound sentence, not a state. Research still succeeded, so the row settles
  // `done`; this field is the honest degrade marker ('no_campaign' today; the field, not the
  // value list, is the contract). Absent on rows where drafting ran or had nothing to draft.
  let draftSkipped = null;
  if (!stoppedMid && (researchOk || partial)) {
    // The DRAFT pick reads its own budget (drafting.maxPerRun) inside draftableSignals;
    // maxPerRun above stays the research-ingest ask only.
    let picked = draftableSignals(radarState());
    if (picked.length) {
      ...
      if (!picked.length) {
        ...
        const policy = (getPosting().autoApprove || {}).radarReplies || {};
        ...
        try {
          draftRun = await runAgentJob({
            ...
            prompt: radarDraftPrompt(picked, {
              ...
              voice: (getPosting().radar || {}).replyVoiceDefault || '',
              // The per-tenant fact sheet: replaces PRODUCT_FACTS for a non-pendpost tenant so the
              // drafter stops judging every thread "unrelated to pendpost". Empty => PRODUCT_FACTS.
              brand: (getPosting().radar || {}).brand || null,
              campaign,
              clientId: boundClientId,
              autoPosts,
              // The DRAFT threshold (drafting.minScore, decoupled from the auto-post
              // policy), restated in the child's RULES: the picked list is already
              // filtered to it, and radar_queue_reply refuses below it - the line tells
              // the child a below_threshold refusal is a final skip, not a retry.
              minScore: (() => { const d = (getPosting().radar || {}).drafting || {}; return Number.isInteger(d.minScore) ? d.minScore : 30; })(),
              // No reply locale: a reply's language is the thread's, chosen by the child at
              ...
              // noteLocale governs ONLY the closing log line, which renders in the
              // operator's own UI - so it follows posting.locale, never the threads.
              noteLocale: getPosting().locale || null,
            }),
            allowedTools: [...AGENT_DRAFT_TOOLS],
  ...
  // Spec 50 §7.4 row 3: triage runs AFTER the draft phase of every agent scan, so it judges the
  // signals this run just landed (and any older ones still undecided) with the drafts in hand.
  // It fires whether or not drafting produced anything: a scan that found signals and drafted
  // none still has threads worth a skip decision.
  if (!stoppedMid) triggerEngageTriage('scan');

  const finished = finishJob(job.id, {
    phase: null,
    drafted,
    autoPosted,
    // R3: the per-lane verdicts, persisted on the row so the daily resume (radar-sweep.mjs
    // resumeFailedLanes) and the panel can name WHICH lanes died and why. Same shape as the
    // sources map rows written above: { ok:true } | { ok:false, error }.
    sourceResults,
    // B9: only stamped when drafting was skipped for a machine-readable reason; the field is
    // simply absent otherwise (additive - old rows and clean runs carry nothing).
    ...(draftSkipped ? { draftSkipped } : {}),
    // A promoted partial settles `done` (results DID land) with partial:true, so the row reads as an
    // amber caveat, not a red failure. A Stop is always an honest `failed` (the operator ended it).
    state: stoppedMid ? 'failed' : (researchOk || partial ? 'done' : 'failed'),
    partial: partial || undefined,
    // `reason` is the machine-readable why; `tail` is the child's own words. S6: the operator
    // sees "Not logged in - Please run /login", not a button that shrugged. A clean run (every lane
    // ok) has no reason; a partial carries its caveat; otherwise the dominant failure.
    // A8/B6: a CLEAN done with zero ingest calls and zero accepted is stamped
    // reason:'no_results' (a degrade marker on a done row, never a failure) - see
    // radarNoResultsReason. An all-dropped run (ingestCalls>0) keeps reason null: the
    // counts already show the story.
    reason: stoppedMid ? 'stopped' : ((researchOk && !anyFailed)
      ? radarNoResultsReason({ scope: job.scope, ingestCalls: ingestCallsSeen, accepted: ingested })
      : partial ? partialReason : (run?.error || laneErrors[0] || 'failed')),
    exitCode: run?.exitCode ?? null,
  ...
  if (wait) return await p;
  // Detached: the running row is already visible to the poller. Swallow late rejections - the
  // pipeline degrades-not-throws, but the withClient promise must not become an unhandled rejection.
  p.catch(() => {});
  return { ok: true, enabled: true, job };
}
...
export async function radarAgentComparison({ clientId, backlogKey, campaign, platform, actor } = {}) {
  // ALS-aware like radarAgentScan: the withClient binding wins over the registry's active
  // client, so a scheduler-side caller can never embed another client's id in the child prompt.
  const boundClientId = clientId || alsClientId();
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const radar = getPosting().radar || {};
...
// Per-tenant GEO footprint reset: drop the AI-visibility state (footprint log + derived comparison
// backlog + dismissed ledger) for the BOUND client. This is maintenance, not tuning: the footprint
// is agent-logged history, and a tenant seeded with another brand's rows (the pendpost-competitor
// pollution under a second client) has no config edit that clears it, because this is STATE not config.
// Owner-gated: it is destructive to that history. Bound by withClient at the call site (callTool),
// so `clientId` only carries the binding - the write always lands under the job's own client, never
// the merely-active one. comparisonBacklog is derived and recomputes empty on the next scan.
export async function radarGeoReset({ clientId, actor } = {}) {
  void clientId; // bound by withClient at the call site (mirrors radarAgentStop / listRadar)
  const actErr = requireActor(actor);
  if (actErr) return actErr;
  if (String(actor).trim() !== 'owner') {
    return errorBody('invalid_input', 'only the owner can reset Radar GEO footprint (it drops agent-logged AI-visibility history)');
  }
  const state = radarState();
  const geo = state.radar.geo || {};
  const cleared = {
    footprint: Array.isArray(geo.footprint) ? geo.footprint.length : 0,
    comparisonBacklog: Array.isArray(geo.comparisonBacklog) ? geo.comparisonBacklog.length : 0,
    dismissedBacklog: Array.isArray(geo.dismissedBacklog) ? geo.dismissedBacklog.length : 0,
  };
  state.radar.geo.footprint = [];
  state.radar.geo.comparisonBacklog = [];
  state.radar.geo.dismissedBacklog = [];
  saveState();
...
export async function radarFollowupCheck({ clientId } = {}) {
  ...
  // agentFollowup (owner decision 4) rides through when the reconcile ran the agent-lane
  // pass - { due, job } or { due, skipped } - so the caller sees the whole check, not
  // just the engine half.
  return { ok: true, enabled: true, checked: (r && r.checked) || 0, replied: (r && r.replied) || 0, sources: (r && r.sources) || [], ...(r && r.agentFollowup ? { agentFollowup: r.agentFollowup } : {}) };
}

// Engagement engine (owner decision 4): the agent-lane follow-up REPORT. The spawned
// follow-up child re-reads the public threads our x/youtube/nostr replies sit in and
// reports, per target, whether the thread's ORIGINAL author answered us. This is the ONE
// write it holds, and it is evidence-gated end to end because the child reads UNTRUSTED
// threads and "the author replied" is a stamp that promotes a signal to the top of the
// feed and arms the round-2 reply target (radarReplyTo.parentExternalId):
//   - THE FENCE (fail-CLOSED, unlike the draft fence): inert unless a follow-up job armed
//     it, and then only for the exact targets pendpost enumerated. No chat agent, operator
//     surface or injected tool call can stamp through it.
//   - THE EVIDENCE GATES: the claimed author must normAuthor-match the author snapshot
//     pendpost itself recorded at queue time; the permalink must be an absolute http(s)
//     URL on the lane's own host (x: an /<handle>/status/<id> path whose handle matches
//     the claimed author; youtube: must reference the same video id; nostr: njump.me or a
//     bech32 commentId); the commentId must have the lane's native shape AND stay inside
//     the queueRadarReply parentExternalId charset so the stamped id is a LEGAL round-2
//     target; text is required (the operator reads it on the card); a ts, when given,
//     must parse and postdate our reply. A claim that fails any gate stamps NOTHING.
//   - replied:false stamps lastCheckedTs only, and NEVER un-sets a terminal state
//     (stampFollowup's contract - author_replied stays found).
// On a proven hit: stampFollowup (the byte-identical shape the engine verbs write) +
// stampFollowupEngager (relationship memory) + an ADDITIVE via:'agent' provenance mark set
// AFTER the stamp, so nothing downstream keys on it. Stamping never posts anything.
const AGENT_FOLLOWUP_COMMENT_ID = Object.freeze({
  x: /^\d{1,25}$/, // the author's reply tweet id
  youtube: /^[A-Za-z0-9_.-]{5,100}$/, // the native comment id (commentThreads shape)
  nostr: /^([0-9a-f]{64}|(note1|nevent1)[02-9ac-hj-np-z]{6,290})$/i, // event id hex or bech32
});
const AGENT_FOLLOWUP_HOSTS = Object.freeze({
  x: ['x.com', 'twitter.com', 'mobile.twitter.com'],
  youtube: ['youtube.com', 'm.youtube.com', 'youtu.be'],
  nostr: ['njump.me'],
});
const FOLLOWUP_REPORT_TEXT_MAX = 400;

export async function radarFollowupReport({ clientId, actor, source, externalId, replied, author, permalink, commentId, text, ts } = {}) {
  void clientId; // bound by withClient at the call site (mirrors queueRadarReply)
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const radar = getPosting().radar || {};
  if (radar.enabled !== true) return errorBody('disabled', 'Radar is off for this project - there is nothing to report a follow-up against');
  const src = String(source || '').trim().toLowerCase();
  if (!RADAR_AGENT_FOLLOWUP_SOURCES.includes(src)) {
    return errorBody('invalid_input', `source must be an agent follow-up lane (${RADAR_AGENT_FOLLOWUP_SOURCES.join('|')}); the engine lanes (reddit/mastodon/bluesky) are read by their own verbs`);
  }
  const ext = String(externalId || '').trim();
  if (!ext) return errorBody('invalid_input', 'externalId is required (the original signal thread id from your brief)');
  if (typeof replied !== 'boolean') return errorBody('invalid_input', 'replied must be a boolean - did the thread\'s ORIGINAL author answer OUR posted reply?');
  // THE FENCE. Fail-closed when disarmed: outside a follow-up job this tool does nothing.
  if (!followupTargetAllowed(`${src} ${ext}`)) {
    return errorBody('invalid_input', 'that target is not one of the follow-up checks this job was asked to run - report only the targets listed in your brief (outside a follow-up job this tool is inert)');
  }

  // Resolve the target pendpost enumerated: a posted reply plan post carrying this
  // radarReplyTo, else the copy-posted ledger entry (the non-Enterprise x path).
  const state = radarState();
  let planTarget = null; // { campaign, postId, post }
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
      for (const p of c.posts || []) {
        const rr = p && p.radarReplyTo;
        if (rr && String(rr.source || '').toLowerCase() === src && rr.externalId === ext && p.status === 'posted') {
          planTarget = { campaign: c.id, postId: p.id, post: p };
        }
      }
    }
  } catch { /* plan store unreadable -> fall through to the ledger */ }
  const ledgerEntry = planTarget ? null : (state.radar.copyPosted || []).find((e) => String(e.source || '').toLowerCase() === src && e.externalId === ext);
  if (!planTarget && !ledgerEntry) {
    return errorBody('invalid_input', 'no posted reply is on record for that target - there is nothing to stamp a follow-up onto');
  }

  const now = new Date().toISOString();
  const stampMiss = async () => {
    // "We looked, no answer (or no verifiable claim)": lastCheckedTs only. stampFollowup
    // never un-sets a terminal author_replied - that state is a found fact.
    if (planTarget) {
      const found = findPlanEntry(planTarget.campaign);
      if (found.error) return found.error;
      await mutatePlan(found.absPlan, (plan) => {
        const p = (plan.posts || []).find((x) => x.id === planTarget.postId);
        if (p) stampFollowup(p, null, now);
        return p;
      });
    } else {
      stampFollowup(ledgerEntry, null, now);
      saveState();
    }
    return null;
  };
  ...
  if (replied !== true) {
    const err = await stampMiss();
    if (err) return err;
    return { ok: true, source: src, externalId: ext, replied: false, stamped: true };
  }

  // ---- replied:true - the evidence gates, all fail-closed ------------------------------
  // The author snapshot pendpost recorded when the reply was queued (replyContextFrom) or,
  // for a copy-posted signal, the cached signal's author. Without one there is NOTHING to
  // verify the claim against - record we looked, refuse the stamp.
  const recordedAuthor = planTarget
    ? (planTarget.post.radarReplyTo.author || null)
    : ((((state.radar.signals || []).find((s) => s && s.source === src && s.externalId === ext)) || {}).author || null);
  if (!recordedAuthor || !normAuthor(recordedAuthor)) {
    const err = await stampMiss();
    if (err) return err;
    return { ok: true, source: src, externalId: ext, replied: false, stamped: false, reason: 'no_recorded_author' };
  }
  const claimedAuthor = String(author || '').trim();
  if (!claimedAuthor) return errorBody('invalid_input', 'author is required on a replied:true report (the handle that answered)');
  if (normAuthor(claimedAuthor) !== normAuthor(recordedAuthor)) {
    return errorBody('invalid_input', 'author does not match the thread author pendpost recorded for this target - someone else replying is not the author answering; report replied:false instead');
  }

  const link = String(permalink || '').trim();
  if (!/^https?:\/\//.test(link)) {
    return errorBody('invalid_input', 'permalink is required on a replied:true report and must be an absolute http(s) URL - a claim without a real permalink is refused');
  }
  let host = '';
  let linkPath = '';
  try {
    ...
    return errorBody('invalid_input', 'permalink is not a parseable URL');
  }
  const cid = String(commentId || '').trim();
  if (!AGENT_FOLLOWUP_COMMENT_ID[src].test(cid)) {
    return errorBody('invalid_input', `commentId is required and must be the ${src} native reply id (${String(AGENT_FOLLOWUP_COMMENT_ID[src])})`);
  }
  // Belt over braces: whatever the lane shape matched must ALSO be a legal round-2 target
  // for queueRadarReply's parentExternalId charset - the stamped id is that target.
  if (!/^[A-Za-z0-9_:/.@%-]{1,300}$/.test(cid)) {
    return errorBody('invalid_input', 'commentId must stay a legal reply-target id ([A-Za-z0-9_:/.@%-], 1-300 chars)');
  }
  const nostrBech = src === 'nostr' && /^(note1|nevent1)[02-9ac-hj-np-z]{6,290}$/i.test(cid);
  if (!AGENT_FOLLOWUP_HOSTS[src].includes(host) && !nostrBech) {
    return errorBody('invalid_input', `a ${src} follow-up permalink must be on ${AGENT_FOLLOWUP_HOSTS[src].join('/')}${src === 'nostr' ? ' (or carry a bech32 note1/nevent1 commentId)' : ''}`);
  }
  if (src === 'x') {
    const m = linkPath.match(/^\/([A-Za-z0-9_]{1,15})\/status\/\d+/);
    if (!m) return errorBody('invalid_input', 'an x follow-up permalink must be a /<handle>/status/<id> tweet URL');
    if (normAuthor(m[1]) !== normAuthor(claimedAuthor)) {
      return errorBody('invalid_input', 'the permalink\'s handle does not match the claimed author - the linked tweet is not their reply');
    }
  } else if (src === 'youtube') {
    if (!link.includes(ext)) {
      return errorBody('invalid_input', 'a youtube follow-up permalink must reference the SAME video the reply was posted on (the signal externalId)');
    }
  }
  const answer = String(text || '').trim();
  if (!answer) return errorBody('invalid_input', 'text is required on a replied:true report - what the author actually wrote (the operator reads it on the card)');
  const postedAtIso = planTarget ? (planTarget.post.postedAt || null) : (ledgerEntry.at || null);
  let tsIso = null;
  if (ts != null && String(ts).trim() !== '') {
    const ms = Date.parse(String(ts));
    if (!Number.isFinite(ms)) return errorBody('invalid_input', 'ts must be a parseable timestamp (ISO)');
    const postedMs = Date.parse(postedAtIso || '') || 0;
    if (postedMs && ms <= postedMs) {
      return errorBody('invalid_input', 'ts is not after our reply was posted - the author cannot have answered it then');
    }
    tsIso = new Date(ms).toISOString();
  }

  // ---- the stamp: byte-identical shape to the engine verbs, plus additive provenance ----
  const hit = { replied: true, author: normAuthor(claimedAuthor), text: answer.slice(0, FOLLOWUP_REPORT_TEXT_MAX), permalink: link, ts: tsIso, commentId: cid };
  if (planTarget) {
    const found = findPlanEntry(planTarget.campaign);
    if (found.error) return found.error;
    await mutatePlan(found.absPlan, (plan) => {
      const p = (plan.posts || []).find((x) => x.id === planTarget.postId);
      if (!p) return null;
      stampFollowup(p, hit, now);
      p.radarFollowup.via = 'agent';
      return p;
    });
  } else {
    stampFollowup(ledgerEntry, hit, now);
    ledgerEntry.radarFollowup.via = 'agent';
  }
  // Relationship-memory accretion (spec 49 R12), same as both reconcile paths. Rides the
  // already-loaded state; ONE trailing saveState persists it (and the ledger stamp).
  try { stampFollowupEngager(state, planTarget ? planTarget.post : ledgerEntry, hit, now); } catch { /* a memory stamp never breaks the report */ }
  saveState();
  return { ok: true, source: src, externalId: ext, replied: true, stamped: true, state: 'author_replied', commentId: cid };
}
...
// The orphaned-running-row REPAIR (WP4; widened S1.6, audit 2026-08-31). A daemon restart
// mid-scan orphans a `running` job row - the in-memory spawn is gone (isJobRunning() is
// per-process), but the persisted row keeps saying running forever, so the panel polls
// forever and the bar never settles (the "infinite loading" failure mode). Any running row
// older than the hard kill bound with no live spawn behind it is settled failed/stale.
//
// TWO callers, one truth: listRadar (every read passes here) AND the scheduler tick
// (lib/scheduler.mjs) - the tick call is what heals a restarted daemon's row promptly
// instead of only when someone next opens the panel. Cheap per tick: one cached state
// read, a save only when something was actually repaired. The activePipelines guard is
// belt-and-braces for the between-lane gap (no live child, pipeline still working): a
// root with an active pipeline is never repaired, however old the row looks.
// Returns the number of rows repaired (the tick test reads it).
export function repairStaleRadarJobs() {
  const state = radarState();
  const staleCut = Date.now() - (AGENT_TIMEOUT_MS + 5 * 60_000);
  let repaired = 0;
  if (activePipelines.has(activeRoot())) return repaired;
...
export async function listRadar({ clientId, source, action, minScore, queryId, view } = {}) {
  ...
  const radar = posting.radar || {};
  const state = radarState();
  // REPAIR, not cosmetics: any orphaned running row is settled as failed here, where every
  // reader passes - and (S1.6) on the scheduler tick, so a restarted daemon heals the row
  // even before anyone opens the panel. Shared helper below.
  repairStaleRadarJobs();
  ...
  const geo = {
    ...
    // R4/P2: the server-derived per-competitor share-of-voice tally (signals + footprint,
    // most-frequent-first with counts) - the ONE truth the panel renders and MCP reads,
    // replacing the panel's old ad-hoc client-side rivals aggregation.
    shareOfVoice: shareOfVoice(state.radar.signals || [], state.radar.geo?.footprint || [], radarBrandContext()),
  ...
  // never throws):
  //   - a POSTED reply -> `replied` { url, via, postId, campaign } - the evidence-first
  //     answer state. url comes from resolveReplyPermalink (lib/radar.mjs) and is the REPLY's
  //     address or null, NEVER the signal's own thread url ("Beantwortet" linking back to the
  //     question was the lie this replaced). via says what the state can prove: 'published'
  //     (minted engine id / verify read-back), 'external' (operator-supplied URL), 'manual'
  //     (mark_posted with zero evidence - rendered as a distinct claim, not a fact).
  //     `repliedUrl` stays as a deprecated alias of replied.url for external radar_list readers.
  ...
  // R5 piece 2: the copy-posted ledger join. A copy-draft / karma post-idea has no plan post,
  // so it can never carry repliedUrl; this durable marker is the only proof the operator
  // posted it by hand ({ postedUrl?, at }), and it drives isAnswered on the copy path.
  const copyPostedByKey = new Map();
  for (const e of state.radar.copyPosted || []) {
    if (!e || !e.externalId) continue;
    // R5 piece 3: a copy marker that the reconcile stamped author_replied carries the same
    // radarFollowup shape a reply post does - surface it as authorReplied so the copy signal
    // shows the buyer-answered payoff identically (and isAnswered counts it).
    const authorReplied = (e.radarReplyState === 'author_replied' && e.radarFollowup && typeof e.radarFollowup === 'object')
      ? { author: e.radarFollowup.author || null, text: e.radarFollowup.text || null, permalink: e.radarFollowup.permalink || null, ts: e.radarFollowup.ts || null, commentId: e.radarFollowup.commentId || null }
      : null;
    // ADDITIVE: the follow-up sweep stamps the same radarFollowup shape on copy markers,
    // so their lastCheckedTs feeds the follow-up glyph identically (S1 glyph state 3).
    const copyLastCheckedTs = (e.radarFollowup && typeof e.radarFollowup === 'object' && e.radarFollowup.lastCheckedTs) || null;
    copyPostedByKey.set(`${String(e.source || '').toLowerCase()} ${e.externalId}`, { postedUrl: e.postedUrl || null, at: e.at || null, authorReplied, lastCheckedTs: copyLastCheckedTs });
  }
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
      for (const p of c.posts || []) {
        ...
        if (p.status === 'posted') {
          ...
          // ADDITIVE: lastCheckedTs (the engine-owned radarFollowup stamp) rides the
          // replied join so the follow-up glyph can say "Zuletzt geprüft {time}" -
          // a forced check that found nothing must still have a visible outcome
          // (S1 glyph state 3). null when no check ever ran.
          const lastCheckedTs = (f && typeof f === 'object' && f.lastCheckedTs) || null;
          repliedByKey.set(key, { url: evidence.url, via: evidence.via, postId: p.id, campaign: c.id, lastCheckedTs });
          if (p.radarReplyState === 'author_replied' && f && typeof f === 'object') {
            authorRepliedByKey.set(key, { author: f.author || r.author || null, text: f.text || null, permalink: f.permalink || null, ts: f.ts || null, commentId: f.commentId || null });
          }
        } else if (p.approval === 'pending' || p.approval === 'approved') {
          // The live draft: awaiting a human (pending) or cleared to fire (approved). Last write
          // wins. scheduledAt rides along so an auto-approved card can SAY when it goes out -
          // "cleared to fire" without a when reads as a surprise waiting to happen.
          draftByKey.set(key, { text: p.caption || '', approval: p.approval, postId: p.id, campaign: c.id, scheduledAt: p.scheduledAt || null });
        }
      }
    }
  } catch { /* plan store unreadable -> just no markers */ }
  if (repliedByKey.size || draftByKey.size || authorRepliedByKey.size || copyPostedByKey.size) {
    items = items.map((s) => {
      ...
      const copyPosted = copyPostedByKey.get(key) || null;
      // A posted reply supersedes any open draft (the loop closed); never show both.
      const draft = hasReplied ? null : draftByKey.get(key);
      if (!hasReplied && !draft && !authorReplied && !copyPosted) return s;
      const next = { ...s };
      if (hasReplied) {
        next.replied = repliedByKey.get(key);
        // Deprecated alias for external radar_list readers. Its VALUE is fixed too:
        // null instead of the signal's own url - a wrong-but-plausible link is worse
        // than no link.
        next.repliedUrl = next.replied.url;
      }
      if (authorReplied) next.authorReplied = authorReplied;
      if (draft) next.draft = draft;
      if (copyPosted) {
        next.copyPosted = { postedUrl: copyPosted.postedUrl, at: copyPosted.at, lastCheckedTs: copyPosted.lastCheckedTs || null };
        // The copy path's author-reply (R5 piece 3) surfaces identically to a reply post's.
        if (copyPosted.authorReplied && !next.authorReplied) next.authorReplied = copyPosted.authorReplied;
      }
      return next;
    });
  }

  // ---- Spec 50 P2: the "Respond for me" join -------------------------------------------
  // Every action row for a signal, folded into ONE badge the feed row renders inside its
  // existing status slot (S5) - no new call, no new filter. `decision` already rides along on
  // the signal object itself (triage writes it there so the feed needs no join at all); this
  // adds the part that does NOT live on the signal: what the pacer and the executor did with it.
  //
  // The REPLY row is the one a human reads a signal's state off ("Replied 14:20"), so it leads;
  // with no reply among the rows the first one does. The remaining kinds ride as `kinds` for the
  // "+ liked, followed" tail. A pure read of the already-loaded state - no second file read, and
  // a signal with no rows keeps its object byte-unchanged.
  try {
    const engageRows = Array.isArray(state.engage?.queue) ? state.engage.queue : [];
    if (engageRows.length) {
      const engageByKey = new Map();
      for (const r of engageRows) {
        if (!r || typeof r.signalKey !== 'string') continue;
        const list = engageByKey.get(r.signalKey) || [];
        list.push(r);
        engageByKey.set(r.signalKey, list);
      }
      items = items.map((s) => {
        const rows = engageByKey.get(`${String(s.source || '').toLowerCase()} ${s.externalId}`);
        if (!rows || !rows.length) return s;
        const lead = rows.find((r) => r.kind === 'reply') || rows[0];
        return {
          ...s,
          engage: {
            status: lead.status,
            ...
            result: {
              ...
              // Only present on a browser row (P4): "Would have replied" is only honest when
              // the child actually reached the post box (D19 / §5.1 finding 1), so the field
              // is absent rather than false where nothing ever checked.
  ...
  } catch { /* the engage badge is additive - a hiccup never breaks the feed read */ }

  // The forward-looking clocks (engagement engine): WHEN does the next unattended scan run,
  // and what did the last one produce? Derived from the SAME gate the scheduler ticks
  // (dueDailyAt, inverted by nextDueDailyAt) so the panel's claim and the tick's behaviour
  // cannot drift. Dynamic import mirrors radarFollowupCheck (radar-sweep imports this
  // module, so a static import would be a cycle). agent/keyword stay SPLIT: both share
  // dailyAt but have independent stamps and arming, and a merged single time could claim
  // "next scan 07:00" when only the free keyword sweep will fire - the budget is stated
  // honestly via { budget, spent } instead (owner decision 2). Fail-soft: a derivation
  // hiccup yields nextScan:null, never a broken read.
  let nextScan = null;
  let lastProduced = null;
  try {
    const { nextDueDailyAt, agentJobsToday, radarTimezone } = await import('./radar-sweep.mjs');
    // L5: the SAME timezone resolver the scheduler gates use (client registry first, then
    // posting.defaultTimezone, then UTC) - the displayed clock and the firing gate agree.
    const tz = radarTimezone(posting);
    ...
    // The SAME arming dailyAgentScan/dailyRadarScan gate on: the global dailyEnabled switch on
    // + at least one enabled query (plus, for the agent, a connected provider).
    const hasDaily = radar.dailyEnabled === true && queries.some((q) => q && q.enabled !== false);
    const lastReconcile = state.radar.lastAuthorReplyReconcile || null;
    nextScan = {
      timezone: tz,
      dailyAt,
      agent: {
        armed: Boolean(agentCfg.provider) && hasDaily,
        // Reported even at budget-spent: the 24h window rolls, and the UI phrases the
        // budget from { budget, spent } rather than a silently missing time.
        at: nextDueDailyAt(state.radar.lastAgentScan, dailyAt, tz),
        lastAt: state.radar.lastAgentScan || null,
        budget: Number.isInteger(agentCfg.dailyBudget) ? agentCfg.dailyBudget : 1,
        spent: agentJobsToday(state),
      },
      keyword: {
        armed: hasDaily,
        at: nextDueDailyAt(state.radar.lastDailyScan, dailyAt, tz),
        lastAt: state.radar.lastDailyScan || null,
      },
      followup: {
        // The author-reply reconcile rides a plain 24h clock, not dailyAt.
        at: lastReconcile ? new Date((Date.parse(lastReconcile) || 0) + 24 * 3600 * 1000).toISOString() : null,
        lastAt: lastReconcile,
        // Agent-lane follow-up targets currently due (x/youtube/nostr via the connected
        // agent) - the SAME enumeration the followup-scope job and the reconcile use.
        agentDue: agentFollowupTargets().length,
      },
    };
    // What the last unattended-or-manual FEED scan produced, as ONE summary the panel can
    // render without re-deriving it from `jobs` (single source of truth): the newest
    // SETTLED scope-feed job. Rows persisted by older builds carry no scope - they were
    // feed scans.
    const settled = (state.radar.jobs || []).find((j) => j && (j.scope || 'feed') === 'feed' && j.state !== 'running');
    if (settled) {
      lastProduced = {
        jobId: settled.id,
        finishedAt: settled.finishedAt || null,
        state: settled.state,
        partial: settled.partial === true,
        reason: settled.reason ?? null,
        accepted: settled.accepted || 0,
        drafted: settled.drafted || 0,
        autoPosted: settled.autoPosted || 0,
      };
    }
  } catch { /* the clocks are a bonus - a failed derivation never breaks the feed read */ }
  return {
    ok: true,
    ...
    // Engagement engine: the forward clocks + the last settled feed job's produce summary.
    nextScan,
    lastProduced,
...
// R4/P4: the assistant label is a SHORT surface name ("ChatGPT", "Claude web search"),
// so the trend can one day separate training-corpus presence from live retrieval.
const FOOTPRINT_ASSISTANT_CAP = 60;
export async function logRadarFootprint({ clientId, actor, question, mentioned, competitorsMentioned, excerpt, assistant } = {}) {
  void clientId;
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  ...
  if (assistant !== undefined && assistant !== null && typeof assistant !== 'string') {
    return errorBody('invalid_input', 'assistant must be a short string (the assistant surface checked, e.g. "ChatGPT")');
  }
  const state = radarState();
  const entry = {
    question: q,
    ...
    // Optional and clipped, never dropped: a label is context, not a gate.
    assistant: typeof assistant === 'string' && assistant.trim() ? assistant.trim().slice(0, FOOTPRINT_ASSISTANT_CAP) : null,
  ...
  // R4/P1: a fresh check is a derivation input now, so the backlog recomputes HERE too -
  // the third persist site beside the scan and the ingest. Without this, the bridge would
  // only fire on the NEXT scan, and a standalone geo recheck would never close the loop.
  state.radar.geo.comparisonBacklog = comparisonBacklog(state.radar.signals, state.radar.geo.dismissedBacklog, state.radar.geo.footprint, radarBrandContext());
  saveState();
...
//
// G8 (ux-audit 2026-08-04): the SAME machinery also triages one GEO comparison-backlog
// entry, identified by `backlogKey` INSTEAD of source+externalId. A backlog entry has
// no thread to pin, so only dismiss/clear apply:
//   - dismiss: drop it from the persisted state.radar.geo.comparisonBacklog AND append
//     { key, at } to state.radar.geo.dismissedBacklog - the seen-ledger pattern, which
//     comparisonBacklog() filters at every scan recompute so it never resurrects.
//   - clear: remove the ledger entry (the next scan may honestly surface it again).
const RADAR_TRIAGE_ACTIONS = ['dismiss', 'watch', 'clear'];
const RADAR_BACKLOG_TRIAGE_ACTIONS = ['dismiss', 'clear'];
export async function triageSignal({ clientId, source, externalId, action, actor, backlogKey } = {}) {
  void clientId;
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const bKey = typeof backlogKey === 'string' ? backlogKey.trim() : '';
  if (bKey) {
    // The backlog path. Naming BOTH a signal and a backlog entry is ambiguous - refuse
    // rather than guess which one the caller meant.
    if (String(source || '').trim() || String(externalId || '').trim()) {
      return errorBody('invalid_input', 'pass EITHER source+externalId (a signal) OR backlogKey (a GEO backlog entry), not both');
    }
    const act = String(action || '').trim();
    if (!RADAR_BACKLOG_TRIAGE_ACTIONS.includes(act)) {
      return errorBody('invalid_input', `action for a backlog entry must be one of ${RADAR_BACKLOG_TRIAGE_ACTIONS.join('|')} (a backlog entry has no thread to watch)`);
    }
    const state = radarState();
    const now = Date.now();
    const geo = state.radar.geo;
    geo.dismissedBacklog = geo.dismissedBacklog.filter((e) => e && e.key !== bKey);
    if (act === 'dismiss') {
      geo.comparisonBacklog = geo.comparisonBacklog.filter((b) => b && b.key !== bKey);
      geo.dismissedBacklog.push({ key: bKey, at: new Date(now).toISOString() });
      geo.dismissedBacklog = pruneSeen(geo.dismissedBacklog, now);
    }
    saveState();
    return { ok: true, backlogKey: bKey, action: act };
  }
  if (!String(source || '').trim() && !String(externalId || '').trim()) {
    return errorBody('invalid_input', 'identify what to triage: source+externalId (a signal) or backlogKey (a GEO backlog entry)');
  }
  const src = String(source || '').trim().toLowerCase();
...
// Record that the operator posted a COPY-DRAFT signal by hand (spec R5 piece 2, Pattern P4
// write). Signal-level, durable, idempotent - the paired twin of triageSignal for the copy
// path. A copy-draft lane (HN, non-Enterprise X) and a karma post-idea have NO engine
// publish path (no plan post, so no repliedUrl), so before this the feed could not tell a
// drafted-but-unposted reply from an answered one - isAnswered over-counted. This stores a
// durable {postedUrl?, ts} on the copyPosted ledger (the `seen` pattern) keyed by signal, so
// isAnswered / listRadar can join it even after the signal leaves the volatile cache. Never
// touches approval/publish, no platform reach. Source validation matches triageSignal (any
// known Radar source) rather than the copy-draft subset, because a reddit KARMA post-idea is
// a reddit signal posted by hand too - the feed, not the source id, decides where the
// affordance appears. Required actor; last write wins (a bare mark, then a link correction).
export async function markCopyPosted({ clientId, source, externalId, postedUrl, actor } = {}) {
  void clientId;
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const src = String(source || '').trim().toLowerCase();
  ...
  const ext = String(externalId || '').trim();
  if (!ext) return errorBody('invalid_input', 'externalId is required (from a signal in the feed)');
  if (postedUrl !== undefined && postedUrl !== null && String(postedUrl).trim() !== '' && !/^https?:\/\//.test(String(postedUrl).trim())) {
    return errorBody('invalid_input', 'postedUrl must be an absolute http(s) URL');
  }
  const url = (typeof postedUrl === 'string' && postedUrl.trim()) ? postedUrl.trim() : null;
  const state = radarState();
  const key = signalKey({ source: src, externalId: ext });
  const now = Date.now();
  const prior = (state.radar.copyPosted || []).find((e) => signalKey(e) === key);
  if (prior) {
    prior.at = new Date(now).toISOString();
    prior.actor = actor.trim();
    if (url) prior.postedUrl = url; // a bare re-mark never wipes an existing link
  } else {
    state.radar.copyPosted.push({ source: src, externalId: ext, postedUrl: url, at: new Date(now).toISOString(), actor: actor.trim() });
  }
  // Same 30-day retention + cap as the seen ledger: after the window the signal is gone from
  // the feed anyway, so the marker has nothing left to join to.
  state.radar.copyPosted = pruneSeen(state.radar.copyPosted, now);
  saveState();
  // Spec 50 §7.7: a HAND-OFF ask (the ladder ran out, so the owner posted it themselves)
  // resolves through THIS verb rather than a new one - marking the copy posted IS the answer.
  // Dynamic import: lib/engage-asks.mjs reaches back into this module for engageState().
  // Best-effort by contract: the copy-draft path predates engage and must never fail on it.
  try {
    const { resolveHandoffFor } = await import('./engage-asks.mjs');
    resolveHandoffFor(key, url);
  } catch { /* no engage store, or it could not be read - the mark itself still stands */ }
  const saved = (state.radar.copyPosted || []).find((e) => signalKey(e) === key);
  return { ok: true, source: src, externalId: ext, postedUrl: saved ? (saved.postedUrl || null) : url, at: saved ? saved.at : new Date(now).toISOString() };
}

// Bulk-dismiss a LIST of cached signals in ONE write (granular-filters follow-up to
// triageSignal). Same LOCAL, idempotent, non-destructive contract as the single `dismiss`
// branch above - it just fans that branch over an array and persists ONCE at the end. The
// operator use is "this low-quality query produced 30 junk signals, clear them all" without
// 30 round-trips. Required actor; never touches approval/publish state.
//
// TWO guards make it safe to run over a whole feed slice:
//   - a WATCHED signal is NEVER dismissed. The GUI pre-excludes watched rows, but the engine
//     is the source of truth, so it re-checks state.radar.signals[].watched here and skips +
//     counts any that are pinned (skippedWatched). A bulk clear must never silently drop a
//     thread the owner explicitly kept.
//   - an unknown/malformed source is skipped and counted (skippedInvalid) rather than
//     aborting the batch - one bad id must not lose the other 29 good dismissals.
const RADAR_BULK_TRIAGE_ACTIONS = ['dismiss'];
const RADAR_BULK_IDS_CAP = 1000;
export async function triageSignalsBulk({ clientId, ids, action, actor } = {}) {
  void clientId;
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  const act = String(action || '').trim();
  if (!RADAR_BULK_TRIAGE_ACTIONS.includes(act)) {
    return errorBody('invalid_input', `action must be one of ${RADAR_BULK_TRIAGE_ACTIONS.join('|')} (bulk triage only dismisses)`);
  }
  if (!Array.isArray(ids) || ids.length === 0) {
    return errorBody('invalid_input', 'ids must be a non-empty array of { source, externalId }');
  }
  if (ids.length > RADAR_BULK_IDS_CAP) {
    return errorBody('invalid_input', `too many ids (${ids.length}); dismiss at most ${RADAR_BULK_IDS_CAP} per call`);
  }
  const knownSources = Object.keys(RADAR_CAPABILITIES);
  const state = radarState();
  const now = Date.now();
  // Snapshot watched keys once, up front - dismissing mutates state.radar.signals as we go.
  const watchedKeys = new Set((state.radar.signals || []).filter((s) => s && s.watched === true).map(signalKey));
  let dismissed = 0;
  let skippedWatched = 0;
  let skippedInvalid = 0;
  for (const id of ids) {
    const src = String((id && id.source) || '').trim().toLowerCase();
    const ext = String((id && id.externalId) || '').trim();
    if (!src || !ext || !knownSources.includes(src)) { skippedInvalid += 1; continue; }
    const key = signalKey({ source: src, externalId: ext });
    if (watchedKeys.has(key)) { skippedWatched += 1; continue; }
    // Dismiss semantics mirror triageSignal's single `dismiss` branch: drop from the cache
    // and append to the seen ledger (idempotent - never double-add). `dismissed` counts
    // signals that were actually PRESENT and removed, so a repeat call over already-gone ids
    // honestly reports dismissed:0 rather than re-counting suppressions that already existed.
    const present = state.radar.signals.some((s) => signalKey(s) === key);
    state.radar.signals = state.radar.signals.filter((s) => signalKey(s) !== key);
    if (!state.radar.seen.some((e) => signalKey(e) === key)) {
      state.radar.seen.push({ source: src, externalId: ext, at: new Date(now).toISOString() });
    }
    if (present) dismissed += 1;
  }
  state.radar.seen = pruneSeen(state.radar.seen, now);
  saveState();
  return { ok: true, action: 'dismiss', dismissed, skippedWatched, skippedInvalid };
}

// UNDO a dismiss - single OR bulk (granular-filters follow-up to triageSignal). `dismiss`
// REMOVES the signal object from state.radar.signals AND records it in state.radar.seen, so
// the existing `clear` action cannot bring it back (clear only un-watches signals that are
// STILL present). Undo therefore has to RE-INSERT the objects, which is why the GUI passes
// back the exact signal objects it just dismissed - the engine no longer holds them.
//
// Per signal: clear the seen-ledger entry (so the next scan won't re-suppress it), and if
// nothing with that key is currently cached, insert the passed object back. Dedupe by key
// (never create a duplicate row). Defensive - a malformed entry (no source/externalId) is
// ignored, not thrown on. One sortByIntent + one saveState at the end. Required actor;
// LOCAL, non-destructive, never touches approval/publish. GUI-only affordance (no MCP tool):
// an agent composes dismiss + rescan/ingest instead.
const RADAR_RESTORE_CAP = 1000;
export async function restoreSignals({ clientId, signals, actor } = {}) {
  void clientId;
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  if (!Array.isArray(signals) || signals.length === 0) {
    return errorBody('invalid_input', 'signals must be a non-empty array of signal objects');
  }
  if (signals.length > RADAR_RESTORE_CAP) {
    return errorBody('invalid_input', `too many signals (${signals.length}); restore at most ${RADAR_RESTORE_CAP} per call`);
  }
  const state = radarState();
  const present = new Set((state.radar.signals || []).map(signalKey));
  let restored = 0;
  for (const sig of signals) {
    const src = String((sig && sig.source) || '').trim().toLowerCase();
    const ext = String((sig && sig.externalId) || '').trim();
    if (!src || !ext) continue; // malformed - ignore, don't throw
    const key = signalKey({ source: src, externalId: ext });
    // Always lift the suppression, even if the object is somehow still cached.
    state.radar.seen = state.radar.seen.filter((e) => signalKey(e) !== key);
    if (present.has(key)) continue; // already in the feed - just the seen clear was needed
    // Re-insert with a normalized-lowercase source so its key matches everywhere.
    state.radar.signals.push({ ...sig, source: src, externalId: ext });
    present.add(key);
    restored += 1;
  }
  state.radar.signals = sortByIntent(state.radar.signals);
...
//
// ONE opt-in exception, at the bottom of this function: if the OWNER has explicitly enabled
// posting.autoApprove.radarReplies for this lane, the reply is approved here under the policy
// actor. That is NOT a hole in the fences above - all three still hold
...
// confirm-gated.
// R11/N2: the author-reply comment id pendpost captured for a signal, read back from the SAME
// two places listRadar joins authorReplied from - the posted reply-post's ENGINE_OWNED
// radarFollowup, and the copy-posted ledger. Returns the native comment id (round-2 target) or
// null. Pure read; never throws (an unreadable plan store just yields null). The lookup is by
// the ORIGINAL reply's target (radarReplyTo.externalId === the signal root), so a round-2 queue
// finds the round-1 author reply to thread under.
function capturedFollowupCommentId(src, ext) {
  const state = radarState();
  for (const e of state.radar.copyPosted || []) {
    if (String(e.source || '').toLowerCase() === src && e.externalId === ext
      && e.radarReplyState === 'author_replied' && e.radarFollowup && e.radarFollowup.commentId) {
      return String(e.radarFollowup.commentId);
    }
  }
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
      for (const p of c.posts || []) {
        const r = p && p.radarReplyTo;
        if (!r || String(r.source || '').toLowerCase() !== src || r.externalId !== ext) continue;
        if (p.radarReplyState === 'author_replied' && p.radarFollowup && p.radarFollowup.commentId) {
          return String(p.radarFollowup.commentId);
        }
      }
    }
  } catch { /* plan store unreadable -> no captured target */ }
  return null;
}

// B11 (audit 2026-08-31): the refusal-visibility wrapper. During the drafting phase every
// queueRadarReply refusal (below_threshold, the target fence, copy-lane shape errors,
// invalid_input...) used to vanish into the child's transcript - only disk tallies moved,
// so a run that attempted 8 drafts and landed 2 read as "drafted 2" with no trace of the 6
// refusals. Tally refusals BY CODE onto the running job row (job.draftRefusals), the exact
// attribution radarIngest already uses for accepted/dropped: one job per client is what
// makes it sound, and a stray refused call from another actor mid-job is at worst counted,
// never lost. Additive - the field appears only when a refusal happened; ok results and
// no-job-in-flight calls change nothing. The tally is a bonus: it must never break the
// reply path, hence the try/catch.
export async function queueRadarReply(args = {}) {
  const res = await queueRadarReplyCore(args);
  try {
    if (res && res.code) {
      const state = radarState();
      const job = runningRadarJob(state);
      if (job) {
        const tally = job.draftRefusals && typeof job.draftRefusals === 'object' ? job.draftRefusals : {};
        tally[res.code] = (tally[res.code] || 0) + 1;
        job.draftRefusals = tally;
        saveState();
      }
    }
  } catch { /* the tally is a bonus - a refusal must still reach the caller unchanged */ }
  return res;
}

async function queueRadarReplyCore({ clientId, campaign, signalUrl, source, externalId, parentExternalId, text, executionMode, actor, confirm } = {}) {
  void clientId; // per-call client scoping is bound by withClient at the call site
  const actorErr = requireActor(actor);
  if (actorErr) return actorErr;
  if (confirm !== true) {
    return errorBody('needs_confirm', 'radar_queue_reply seeds a reply that will post to an EXTERNAL thread once approved - pass confirm:true to queue it (it does NOT post now). By default a DISTINCT human must approve it. If the OWNER has explicitly enabled posting.autoApprove.radarReplies for this lane, an approved-by-policy reply posts on the next scheduler tick without a human reading it first.');
  }
  const src = String(source || '').trim().toLowerCase();
  ...
  // R11/N2: the optional round-2 target - the author's follow-up comment id, threaded UNDER
  // instead of the thread root. Shape-checked here; VETTED below against the id pendpost itself
  // captured for this signal (never a caller-invented target).
  const parentExt = parentExternalId == null ? '' : String(parentExternalId).trim();
  if (parentExt && !/^[A-Za-z0-9_:/.@%-]{1,300}$/.test(parentExt)) {
    return errorBody('invalid_input', 'parentExternalId must be a bounded opaque id ([A-Za-z0-9_:/.@%-], 1-300 chars) - the authorReplied.commentId from radar_list');
  }
  ...
  // THE DRAFT THRESHOLD (engagement engine, owner decision 2026-08-17): drafting.minScore,
  // DECOUPLED from autoReply.minScore (which now gates only the auto-approve decision at the
  // bottom). A signal the AGENT scored below the drafting threshold gets NO draft at all -
  // on the reply path AND the copy path (a draft is a draft). Hoisted lookup, shared with
  // the context snapshot below. Only an agent-scored signal is refused: an engine keyword
  // score and an agent's judgment are incommensurable (Spec C), and an uncached signal may
  // still queue - a pending draft a human reads is harmless, and the auto-approve gate at
  // the bottom stays fail-closed for exactly those two cases. The draft-one hold BYPASSES
  // the door (draftFenceHoldsApproval): the operator picked this exact thread to read a
  // draft for, and a threshold skip would be refusing them.
  const cached = (radarState().radar.signals || []).find((s) => signalKey(s) === `${src} ${ext}`);
  const draftingCfg = (getPosting().radar || {}).drafting || {};
  const draftMin = Number.isInteger(draftingCfg.minScore) ? draftingCfg.minScore : 30;
  if (!draftFenceHoldsApproval() && cached && cached.scoredBy === 'agent' && Number(cached.intentScore) < draftMin) {
    return errorBody('below_threshold', `this signal scored ${Number(cached.intentScore)}, below the drafting threshold ${draftMin} (posting.radar.drafting.minScore) - skip it, do not retry`);
  }
  ...
  if (isCopy) {
    // A copy lane has no reply post to thread, so it can never carry a round-2 target.
    if (parentExt) return errorBody('invalid_input', `${src} is a copy-paste lane with no reply API, so it cannot thread to a follow-up comment - omit parentExternalId and paste the reply by hand`);
    const st = radarState();
  ...
  // R11/N2, THE ROUND-2 FENCE. parentExternalId may ONLY be the author-reply comment id
  // pendpost itself captured for this signal (stampFollowup -> radarFollowup.commentId, joined
  // onto the feed as authorReplied.commentId). A caller cannot invent a deeper target: the same
  // posture as the excerpt snapshot and the target fence - pendpost decides what there is to
  // reply to, from author replies it actually read back. With no captured follow-up (the lane
  // never threaded one, or the author has not answered), parentExternalId is refused and the
  // caller falls back to a root reply - never a broken target.
  if (parentExt) {
    const captured = capturedFollowupCommentId(src, ext);
    if (!captured) return errorBody('invalid_input', 'no author follow-up comment is on record for this signal, so there is nothing to thread under - omit parentExternalId to reply to the thread root');
    if (captured !== parentExt) return errorBody('invalid_input', 'parentExternalId does not match the author follow-up comment pendpost captured for this signal (radar_list authorReplied.commentId) - omit it to reply to the thread root');
  }
  ...
  const radarReplyTo = { url, source: src, externalId: ext, ...replyContextFrom(cached), ...(parentExt ? { parentExternalId: parentExt } : {}) };
  ...
  // The OPT-IN auto-reply decision (owner Q2: merged into the auto-approve trust model). It lives
  // HERE, at the end, because it is the only place with full context - and because the alternatives
  // are broken:
  //   1. inAutoApproveScope runs inside createPost, and the pending flip above would demote any
  //      approval it granted moments earlier.
  //   2. its radarReplyTo/MANUAL_LANES guards deliberately REFUSE every radar reply on the
  //      createPost path (the broad fence), and they stay byte-unchanged.
  // So the arming is a SEPARATE, narrower, owner-authorized scope - autoApprove.radarReplies -
  // decided by the pure radarReplyAutoApproveDecision (enabled + lane + xEnterprise + score + lint).
  // The config/state-coupled fences stay HERE: the draft-review hold, the radar.enabled
  // precondition and the foreign-link fence. Still not self-approval: the approver is the policy
  // actor, never the drafting agent (setApproval remains the single enforcement point). Downstream
  // is untouched - eligibleDuePosts, the buildPublishJob fire-time fence, brand-lint and the
  // cadence caps all still apply at publish time.
  ...
  try {
    ...
    // autonomy, not expression. It removes the PAYOFF from the hostile-thread chain (agent drafts
    // -> auto-approved -> posted): a stranger's link is monetizable, so a draft carrying one stays
    // pending where a human sees it. Config-coupled (defaultLink), so it stays here, not in the pure fn.
    const foreign = foreignLinksIn(draft, posting.defaultLink);
    const decision = radarReplyAutoApproveDecision(
      { caption: draft, platforms: [src], radarReplyTo },
      posting.autoApprove,
      { lane: src, scoredBy: cached?.scoredBy, intentScore: cached?.intentScore, xEnterprise: radar.xEnterprise === true },
    );
    // The draft-for-review hold (the operator's "Wird automatisch gepostet" tap) and the beta gate
    // stand the policy down entirely - the human asked to READ this draft, and Radar-off means no
    // reply goes anywhere. Not a refusal row either: the policy was never in play.
    const held = draftFenceHoldsApproval() || radar.enabled !== true;
    if (!held && decision.approve && !foreign.length) {
      ...
      // The policy WOULD have auto-posted this reply (armed = enabled + lane + score), but a
      // CONTENT gate held it back. The digest's autonomy report (R2, dim-5 AU3) reports these by
      // reason class: a foreign link (spec 42 §4.3) or a brand-lint error. It stays pending for a
      // human either way - this row just makes the refusal legible. A NOT-armed policy
      // (disabled / lane / x-tier / below-score) writes nothing.
      const armed = decision.approve || String(decision.reason || '').startsWith('brand_lint');
      const reason = armed ? (foreign.length ? 'foreign_link' : (decision.approve ? null : 'lint')) : null;
      if (reason) appendActivity({ campaign, postId, platform: src, action: 'auto-approve-refused', ok: true, errorCode: null, errorMessage: null, reason, lateMin: null, actor: AUTO_APPROVE_ACTOR });
    }
  } catch { /* fail CLOSED: any hiccup leaves the reply pending for a human (mirrors createPost) */ }

  return { ok: true, campaign, postId, source: src, externalId: ext, parentExternalId: parentExt || null, approval, radarReplyTo };
}
