...
const UPDATABLE_FIELDS = ['caption', 'firstComment', 'title', 'scheduledAt', 'platforms', 'type', 'file', 'path', 'executionMode', 'link', 'image', 'imageUrl', 'description', 'liDescription', 'xCaption', 'xReplyTo', 'tags', 'blogSlug', 'audience', 'interactiveStory', 'hashtags', 'captionPath', 'captionLang', 'body', 'excerpt', 'canonicalUrl', 'ghostEmail', 'mastodonCaption', 'nostrCaption', 'gbp', 'tgCaption', 'dcCaption', 'ttCaption', 'redditText', 'pinTitle', 'pinDescription'];
...
export function validateFieldValues(fields) {
  ...
  for (const k of ['caption', 'firstComment', 'title', 'file', 'path', 'link', 'image', 'imageUrl', 'description', 'liDescription', 'xCaption', 'tags', 'blogSlug', 'audience', 'captionPath', 'captionLang', 'body', 'excerpt', 'canonicalUrl', 'mastodonCaption', 'nostrCaption', 'tgCaption', 'dcCaption', 'ttCaption', 'redditText', 'pinTitle', 'pinDescription']) {
  ...
  // public Pinterest pin image) and canonicalUrl (the article's canonical
  // source) are absolute http(s) URLs.
  for (const k of ['link', 'image', 'imageUrl', 'canonicalUrl']) {
...
export async function platformValidate({ campaign, postId } = {}) {
  ...
  for (const platform of post.platforms || []) {
    ...
    if (platform === 'reddit') {
      ...
      else if (!accounts.reddit?.subreddit) { problems.push('Reddit subreddit not set (REDDIT_SUBREDDIT)'); needsSetup = true; }
    ...
    if (platform === 'pinterest') {
      ...
      // Pins take media by PUBLIC image URL only (v5 create-pin media_source
      // image_url) - the engine never uploads the local render.
      if (!(post.imageUrl || '').trim()) problems.push('Pinterest needs a public image URL (set post.imageUrl - the engine does not host media)');
    ...
    if (platform === 'mastodon') {
      ...
      if (mText.length > MASTODON_NOTE_LIMIT) problems.push(`note text is ${mText.length} chars - Mastodon caps at ${MASTODON_NOTE_LIMIT} (set a shorter mastodonCaption)`);
    }
    ...
    if (platform === 'nostr') {
      ...
      // Text-only lane: a media post still publishes, but only its text.
      if (postNeedsMedia(post)) warnings.push('nostr carries no media - the note publishes text only');
    }
    if (platform === 'gbp') {
