...
export async function deletePost({ campaign, postId, force, actor } = {}) {
  ...
  try {
    const removed = await mutatePlan(found.absPlan, (plan) => {
      ...
      // B3/G2: a natively scheduled post (FB/YouTube/Mastodon/WordPress/Ghost
      // object already living platform-side, waiting to fire) refuses delete
      // even WITH force. deletePost is a plan-only mutation by contract
      // ("deleting the plan row does NOT remove anything from the platforms"),
      // and force means "I accept the platform copy stays" - true for an
      // already-fired post, a lie for a pending native object that would fire
      // later with no plan record and no cancel path. Platform mutations ride
      // the scheduling verbs' confirm ceremony: unschedule owns the cancel.
      const handoffs = nativeHandoffs(post);
      if (handoffs.length) {
        throw Object.assign(
          new Error(`post is natively scheduled on ${handoffs.map((h) => h.lane).join(' + ')} (${handoffs.map((h) => h.id).join(', ')}) - deleting the plan row would leave the platform object(s) to fire with no plan record; unschedule first (unschedule cancels the native platform objects), then delete`),
          { code: 'invalid_input' },
        );
      }
      // Publish-evidence gate: EVERY lane's minted platform id counts, derived
      // from the shared PLATFORM_ID_FIELDS registry (lib/plans.mjs) - never a
      // hardcoded subset a newer lane can fall through.
...
export async function reschedulePost({ campaign, postId, scheduledAt, confirm, actor, slotSlip = false } = {}) {
  ...
  try {
    ...
    const updated = await mutatePlan(found.absPlan, (freshPlan) => {
      ...
      // engines' field-merge cannot resurrect the stale hold.
...
export async function radarAgentScan({ clientId, queryId, actor, cadence, scope } = {}) {
  ...
  try {
    run = await runAgentJob({
      ...
      prompt: radarScanPrompt(queries, maxPerRun, boundClientId, scanSources, geoContext, getPosting().locale || null, radar.brand || null),
...
export async function listRadar({ clientId, source, action, minScore, queryId, view } = {}) {
  ...
  for (const e of state.radar.copyPosted || []) {
    ...
    const authorReplied = (e.radarReplyState === 'author_replied' && e.radarFollowup && typeof e.radarFollowup === 'object')
      ...
      : null;
    copyPostedByKey.set(`${String(e.source || '').toLowerCase()} ${e.externalId}`, { postedUrl: e.postedUrl || null, at: e.at || null, authorReplied });
  }
  try {
    const { campaigns } = loadPlanStore();
    for (const c of campaigns || []) {
  ...
  if (repliedByKey.size || draftByKey.size || authorRepliedByKey.size || copyPostedByKey.size) {
    items = items.map((s) => {
      ...
      if (copyPosted) {
        next.copyPosted = { postedUrl: copyPosted.postedUrl, at: copyPosted.at };
...
export async function queueRadarReply({ clientId, campaign, signalUrl, source, externalId, parentExternalId, text, executionMode, actor, confirm } = {}) {
  ...
  try {
    ...
    const policyArmed = radar.enabled === true && policy.enabled === true && laneAllowed && scoreGateOk;
