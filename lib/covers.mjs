...
// The SINGLE guard shared by every auto-cover path (assets.scanAssets,
// writes.uploadAsset, writes.backfillCovers): a REAL cover extraction may run only
// when we are NOT in mock mode (PENDPOST_MODE=mock keeps hosted demos + CI
// network/binary-free, exactly like the mock-gated engine lanes) AND ffmpeg is
// actually present. resolveBin returns an absolute path only when the binary
// exists in a known install dir; a bare name means "hope it's on PATH" (attempted,
// and any real failure is caught by callers), so only a resolved-but-missing
// absolute path is a definite no. Keeping this in one place is what stops the
// guard from drifting between the scan and the upload/backfill paths.
export function coverExtractionEnabled() {
  if (resolveMode() === 'mock') return false;
  return !FFMPEG.startsWith('/') || fs.existsSync(FFMPEG);
}

// The DEFAULT cover extractor for the boot/upload/scan paths. It carries the
// coverExtractionEnabled() guard so a mock demo or an ffmpeg-less box NEVER shells
// out: it returns { skipped:true } WITHOUT writing, instead of spawning ffmpeg.
// When extraction IS enabled it delegates to extractDefaultCover verbatim. Callers
// that inject their OWN extractor (the tests) bypass this guard entirely - the
// same seam probeMedia uses in assets.scanAssets. Returning a { skipped } marker
// (rather than throwing or silently no-writing) lets backfillCovers count a
// binary-free skip as a skip, never a failure.
export async function extractDefaultCoverGuarded(mediaPath, destJpeg, opts) {
  if (!coverExtractionEnabled()) return { ok: false, skipped: true };
  return extractDefaultCover(mediaPath, destJpeg, opts);
}

// Trust gate for cover swaps. A cover is a publishable content decision like the
// caption, but `cover` is deliberately excluded from POST_CONTENT_FIELDS, so a cover
// change never moves postContentHash and the hash-comparison path in updatePost cannot
// catch it. When the post is already approved we therefore raise editedSinceApproval
// directly, so the shared publish fences (eligibleDuePosts + buildPublishJob) refuse it
// until re-approval - consistent with every other post-approval content edit. Approval
// stays 'approved' (a flag, not an auto-revert); a cover set on a not-yet-approved draft
// has nothing to invalidate and is left untouched.
function flagCoverEditIfApproved(post) {
  if (post.approval === 'approved') post.editedSinceApproval = true;
}
  ...
  for (const platform of post.platforms || []) {
    if (platform === 'facebook') {
      ...
      if (post.type === 'story') {
        ...
        map.instagram = { canApply: true, when: 'publish', note: 'Applied as cover_url (a public image) on the reel container at publish - the calling app hosts a grid-safe title card; the engine falls back to a frame if IG rejects it. No post-hoc change via API.' };
      } else {
        map.instagram = {
          canApply: false,
          note: 'IG accepts only a frame offset (thumb_offset) or a PUBLIC cover_url (source:url) - a local FILE cover cannot reach IG; the engine publishes frame 0 instead (where the render pipeline bakes the title card). Pick a frame or pass coverUrl for a different cover.',
        };
      }
...
export async function setCover(args = {}) {
  ...
  // coverUrl mode (COVER-12): a PUBLIC image URL the calling app already hosts
  // (its own grid-safe title card - pendpost stays dependency-free and never
  // renders text). Stored as { source:'url', url } and consumed by Instagram as
  // cover_url at publish; there is no local JPEG to materialize, so this branch
  // skips the ffmpeg path entirely. The other platforms fall back to their own
  // default cover (render-sibling / frame) since they take a file, not a URL.
  if (args.coverUrl !== undefined && args.coverUrl !== null) {
    const otherSources = ['frameSec', 'filePath', 'base64'].filter((k) => args[k] !== undefined && args[k] !== null);
    if (otherSources.length) return errorBody('invalid_input', 'coverUrl cannot be combined with frameSec/filePath/base64');
    const url = String(args.coverUrl).trim();
    if (!/^https:\/\/\S+$/i.test(url)) return errorBody('invalid_input', 'coverUrl must be an absolute https URL');
    const foundU = findPost(campaign, postId);
    if (foundU.error) return foundU.error;
    const { absPlan: absPlanU, post: postU } = foundU;
    const coverField = { source: 'url', url };
    await mutatePlan(absPlanU, (freshPlan) => {
      const freshPost = (freshPlan.posts || []).find((p) => p.id === postId);
      if (!freshPost) throw new Error(`post ${postId} vanished mid-write`);
      freshPost.cover = coverField;
      flagCoverEditIfApproved(freshPost);
    });
    return { ok: true, cover: coverField, applicability: coverApplicability(postU, 'url') };
  }

  const sources = ['frameSec', 'filePath', 'base64'].filter((k) => args[k] !== undefined && args[k] !== null);
  if (sources.length !== 1) {
    return errorBody('invalid_input', 'pass exactly one source: frameSec (number) | filePath (string) | base64 (string) | coverUrl (https)');
  }
...
export async function clearCover({ campaign, postId } = {}) {
  ...
  try {
    ...
    await mutatePlan(absPlan, (freshPlan) => {
      const freshPost = (freshPlan.posts || []).find((p) => p.id === postId);
      if (freshPost) {
        // Only a real change re-trips approval: clearing a cover the post never had
        // is a no-op and must not fence an approved post (mirrors restoring content).
        const hadCover = freshPost.cover !== undefined && freshPost.cover !== null;
        delete freshPost.cover;
        if (hadCover) flagCoverEditIfApproved(freshPost);
      }
    });
