...
// The client id this async subtree is scoped to: the withClient() binding's
// basename if one is bound, else the registry's active client (or 'default').
// Mirrors activeRoot()'s own resolution, kept self-contained here so it never
// pulls in multi-client.mjs (import-cycle safe). This is the single home for the
// boundRoot()?basename:active pattern that scheduler.publishClientId and
// cloud-config.currentClientId each re-derive; new callers import THIS.
export function boundClientId() {
  const bound = als.getStore();
  if (bound) return path.basename(bound);
  const registry = readRegistry();
  return (registry && typeof registry.activeClientId === 'string' && registry.activeClientId)
    ? registry.activeClientId
    : 'default';
}

// Scope a hand-run ceremony command to the client this async subtree is FOR. On a
// multi-client workspace enforceCeremonyClient (lib/cli-client.mjs) requires an
// explicit --client target or it prompts / fails-closed, so any surfaced command
// (setup.mjs connectAction, accounts.mjs hints) must already carry it - otherwise a
// copied `... auth` targets whatever client is active, not the one whose card the
// operator is looking at. NO-OP on a single-workspace or cloud install (<=1 active
// client): the bare command is correct and unambiguous there. Homed here (the single
// owner of "which client are we") so setup.mjs and accounts.mjs share one definition
// without a back-import cycle.
export function connectFor(base) {
  const registry = readRegistry();
  const active = registry && Array.isArray(registry.clients)
    ? registry.clients.filter((c) => c && c.id && (c.status || 'active') === 'active')
    : [];
  return active.length <= 1 ? base : `${base} --client ${boundClientId()}`;
}
