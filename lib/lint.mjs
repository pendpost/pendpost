...
// The cap NUMBERS come from lib/caption.mjs (the SAME map the pre-flight gate and the
// engine backstop read). This advisory keeps its own SELECTION though: it adds a
// conservative `default` for lanes with no context, and deliberately lets telegram/tiktok
// fall to that default (their media-caption caps only apply to a media post, which the
// free-text advisory can't distinguish) - so those two stay off this map on purpose.
const CAPTION_LIMITS = {
  ...
  x: GENERIC_CAPTION_LIMITS.x,
  mastodon: GENERIC_CAPTION_LIMITS.mastodon,
  gbp: GENERIC_CAPTION_LIMITS.gbp,
  default: 2200,
};
...
const MATCHERS = {
  ...
  // Hard API caps the engine cannot publish past, per platform. Deliberately
  // SEPARATE from captionLength (warn advisory with a conservative default):
  // this one fires only where over-cap is a guaranteed publish failure, so its
  // rule can carry severity "error" without blocking advisory-only lanes.
  // x: 280 for non-Premium accounts (Premium long posts are not wired up).
  captionHardCap(text, ctx = {}) {
    const HARD_CAPS = { x: 280 };
    const cap = ctx.platform && HARD_CAPS[ctx.platform];
    if (!cap || text.length <= cap) return [];
    return [{ match: `${text.length} chars (hard cap ${cap} on ${ctx.platform})`, index: cap }];
  },
