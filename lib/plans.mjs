...
export const POST_CONTENT_FIELDS = [
  ...
  'publishAsDraft',
  // LinkedIn per-post author target: 'organization' (Company Page, default) | 'member'
  // (the connected member's personal profile). It changes WHO publishes - a destination
  // the owner blessed at approval time - so it is content: an edit re-raises
  // editedSinceApproval like any copy change.
  'liAuthor',
...
// The canonical object over POST_CONTENT_FIELDS only, fixed field order, each missing
// field normalised to null. The SINGLE source the three integrity hashes share so they
// can never drift (spec 51 R2): the 12-hex postContentHash (the existing approval fence),
// contentSha256 (full-strength evidence), both over these EXACT bytes.
export function contentCanonical(post) {
  const canonical = {};
...
export function postContentHash(post) {
  return crypto.createHash('sha1').update(JSON.stringify(contentCanonical(post))).digest('hex').slice(0, 12);
}

// Full-strength (spec 51 R2) sha256 over the SAME canonical bytes as postContentHash,
// so an authorization can bind exactly which reviewed copy + destinations were fired.
// No mediaSha256 (v1 scope decision D3 - media byte binding is deferred).
export function contentSha256(post) {
  return crypto.createHash('sha256').update(JSON.stringify(contentCanonical(post))).digest('hex');
}
...
// Activity actions that mean "this post did not publish" - used as the fallback source
// for the card's failure reason when a SKIP (not a real upload attempt) is the cause.
// Cosmetic sub-steps (set-thumbnail/set-caption/post-comment) and verify are excluded.
const FAILURE_SURFACING_ACTIONS = new Set(['engine-run', 'schedule-native', 'publish', 'release']);
...
function lastFailureFor(campaignId, post) {
  ...
  // Activity-feed fallback: a SKIP (engine skipRow - bad status, media missing, caption
  // over-cap - or the scheduler's no_result backstop) records its reason on the activity
  // feed, NOT post.attempts, because a skip is not a publish attempt. Without this, the
  // planner showed a bare "overdue" pill for a post the engine had a concrete reason to
  // skip. Use the NEWEST publish-related row for this post, and only when it is a failure
  // (a newer success row supersedes it). Attempts/cloud failures take precedence.
  let feedFail = null;
  if (!cloud && !localFail) {
    const latestPublish = (state.activity || []).find(
      (e) => e.postId === post.id && e.campaign === campaignId && FAILURE_SURFACING_ACTIONS.has(e.action) && e.platform,
    );
    if (latestPublish && latestPublish.ok === false) feedFail = latestPublish;
  }
  if (!cloud && !localFail && !feedFail) return null;
  const lane = cloud?.lane || localFail?.platform || feedFail?.platform || null;
  ...
  return {
    ...
    code: (localFail && localFail.errorCode) || (feedFail && feedFail.errorCode) || null,
    lane,
    message: cloud?.message || localFail?.errorMessage || feedFail?.errorMessage || null,
    at: cloud?.at || localFail?.ts || feedFail?.ts || null,
...
function deriveState(post, now, failure = null, reviewPending = false, overdueGraceMs = 0) {
  ...
  // visible treatment, still filtering under the same needs-attention bucket. A real failure
  // is honest the instant it is recorded (no grace).
  if (pastDue && failure) return 'publish-failed';
  // Overdue GRACE (posting.overdueGraceSeconds, default 90s): a merely-past-due post that has
  // NOT failed reads the calm 'waiting-due' until it is > slot + grace, so it never flashes the
  // red overdue alarm during the normal publish+confirm window (the scheduler ticks every 60s).
  // pastDue (above) is left ungraced so the native-scheduled branch is unaffected, and this
  // never gates publishing (the scheduler fires on due<=now, not derivedState).
  const overdueDue = pastDue && (due + overdueGraceMs) < now;
  if (overdueDue) return 'overdue';
  return 'waiting-due';
}
...
export function normalizePost(planEntry, plan, post, now = Date.now(), reviewRequired = getPosting().review?.required === true, overdueGraceMs = Math.max(0, Number(getPosting().overdueGraceSeconds ?? 90)) * 1000) {
  ...
  return {
    ...
    derivedState: deriveState(post, now, lastFailure, reviewPending, overdueGraceMs),
    ...
    // Spec 50 §7.10: where the post came from ('radar-theme' when the "Respond for me"
    // policy wrote it out of a Radar theme), null when the owner authored it in the
    // calendar. Surfaced per the write/read parity rule - a field that persists on write
    // but is dropped by this DTO would be invisible to plan_get / the dashboard with no
    // error, which is exactly how spec 50 §5.5 P6-3 stayed hidden.
    ...
    // LinkedIn per-post author target - surfaced per the write/read parity rule so the
    // Composer round-trip + PostDetail chip see it. Defaults to the Company Page.
    liAuthor: post.liAuthor === 'member' ? 'member' : 'organization',
    ...
    // Spec 51: the signed authorization + outcome receipt maps, mirrored per platform
    // (post.attestation[platform] / post.receipt[platform]). Surfaced per the write/read
    // parity rule so plan_get / the Studio can render the ReceiptGlyph with no extra call.
    attestation: post.attestation || null,
...
export function loadPlanStore({ includePosts = true } = {}) {
  ...
  // Overdue-grace window (posting.overdueGraceSeconds, default 90s), resolved ONCE for the
  // whole store walk and threaded into every normalizePost - one config read, not one per post
  // (the reviewRequired precedent). deriveState defers the red overdue pill by this margin.
  const overdueGraceMs = Math.max(0, Number(getPosting().overdueGraceSeconds ?? 90)) * 1000;
  const campaigns = plans.map((entry) => {
    ...
    const posts = plan ? (plan.posts || []).map((p) => normalizePost(entry, plan, p, now, reviewRequired, overdueGraceMs)) : [];
