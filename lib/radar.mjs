...
// (review #5: never mint a nonsensical "pendpost alternative" to-write item). Shared by
...
const validCompetitor = (c) => !!c && c.length >= 2 && !isStopword(c) && notSelf(c);
...
function competitorMentions(text) {
  const out = [];
  const push = (comp, title, phrase) => { if (validCompetitor(comp)) out.push({ comp, title, phrase }); };
  let m;
  ALT_RE.lastIndex = 0;
  while ((m = ALT_RE.exec(text))) { const comp = cleanCompetitor(m[1]); push(comp, `${comp} alternative`, m[0]); }
  ...
  while ((m = VS_CHAIN_RE.exec(text))) {
    ...
    for (const side of m[0].split(/\s+vs\.?\s+/i)) { const c = cleanCompetitor(side); push(c, `pendpost vs ${c}`, m[0]); }
  }
  BETTER_RE.lastIndex = 0;
  while ((m = BETTER_RE.exec(text))) { const comp = cleanCompetitor(m[1]); push(comp, `${comp} alternative`, m[0]); }
  return out;
}
...
export function comparisonBacklog(signals = [], dismissed = [], footprint = []) {
  ...
  const add = (competitor, title, phrase, url) => {
    ...
    // whichever regex (or footprint check) produced it. Sits here rather than at the
    // vs-split so "alternatives to the big ones" and "better than that" are refused
    // by the same rule.
    if (!validCompetitor(comp)) return;
  ...
  for (const s of signals || []) {
    ...
    for (const m of competitorMentions(String(s.text || ''))) add(m.comp, m.title, m.phrase, url);
  }
  ...
  for (const c of footprint || []) {
    ...
    for (const raw of (Array.isArray(c.competitorsMentioned) ? c.competitorsMentioned : [])) {
      ...
      if (!validCompetitor(comp)) continue;
      const key = comp.toLowerCase();
  ...
  for (const e of misses.values()) {
    ...
    if (!qs.length) add(e.comp, `pendpost vs ${e.comp}`, '', '');
    for (const q of qs) add(e.comp, `pendpost vs ${e.comp}`, `AI assistants name ${e.comp} for: ${q}`, '');
  }
...
export function shareOfVoice(signals = [], footprint = []) {
  ...
  for (const s of signals || []) {
    ...
    for (const m of competitorMentions(String(s.text || ''))) {
