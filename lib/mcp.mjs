...
export const TOOLS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        post: { type: 'object', description: '{ id, type, platforms, caption?, firstComment?, title?, link?, image?, scheduledAt?, file?, path?, executionMode?, description?, tags?, blogSlug?, audience? }' },
        clientId: { type: 'string', description: 'Optional client id to scope this call (defaults to the active client)' },
      },
  ...
  {
    name: 'unschedule',
    description: 'Take a post off the schedule (executionMode: parked, so the scheduler ignores it). If the post is NATIVELY scheduled (FB scheduled post / YouTube publishAt video), the platform object is DELETED via the engine - that needs confirm: true.',
    inputSchema: {
      type: 'object',
      properties: {
        campaign: { type: 'string' },
  ...
  {
    name: 'reschedule',
    description: 'Move a post to a new scheduledAt (ISO datetime). Waiting-due posts just change their due time; NATIVELY-scheduled posts have their platform object deleted and re-queue for the new time (confirm: true required).',
    inputSchema: {
      type: 'object',
      properties: {
        campaign: { type: 'string' },
  ...
  {
    name: 'validate_media',
    description: 'Probe one post\'s local media file (ffprobe): resolution/codec/faststart/duration spec checks for 9:16 story / 4:5 feed. Read-only.',
    inputSchema: {
      type: 'object',
      properties: { campaign: { type: 'string' }, postId: { type: 'string' }, clientId: { type: 'string', description: 'Optional client id to scope this call (defaults to the active client)' } },
      required: ['campaign', 'postId'],
      additionalProperties: false,
    },
  },
  {
    ...
    description: 'Per-platform readiness of one post: media present, caption length caps, credentials/auth, Meta action block, YouTube future-publishAt, approval state. Returns { platform: { ready, problems[] } }. Read-only.',
    inputSchema: {
      type: 'object',
      properties: { campaign: { type: 'string' }, postId: { type: 'string' }, clientId: { type: 'string', description: 'Optional client id to scope this call (defaults to the active client)' } },
      required: ['campaign', 'postId'],
  ...
  {
    name: 'health_recheck',
    description: 'Run a live liveness probe per platform and store the result in pendpost state (account_status.<platform>.live). Each probe is a single read-only call that proves the credential actually authenticates (LinkedIn token introspection, YouTube channels.list, Meta GET me) - it can never publish. The Meta probe is skipped while a 368 block is recorded. Returns { ok, health }.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'One-call readiness check (SS-10): { ready, blockers[], schedulerRunning, nextDue[] } - global blockers (manifest, credentials, Meta block, scheduler off) plus per-post blockers for the next N due posts. Read-only.',
    inputSchema: {
      type: 'object',
      properties: {
...
// / title). Required by the Claude Desktop Extension (.mcpb) directory review, and
// useful to any MCP client (safer agent behavior, better UX). DERIVED, never
// hand-kept: read-only comes from READ_TOOL_NAMES; DESTRUCTIVE_TOOL_NAMES marks the
// write tools that remove or irreversibly drop data; every other write tool is a
// non-destructive create/update. TOOLS itself is left unchanged (parity-check
// reads it), so this never affects the read/write split or tool count.
const DESTRUCTIVE_TOOL_NAMES = new Set([
  'plan_delete_post', 'delete_asset', 'clear_cover', 'unschedule', 'client_archive',
]);
