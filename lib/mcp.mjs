...
export const TOOLS = [
      ...
      properties: {
        ...
        post: {
          ...
          properties: {
            ...
            platforms: { type: 'array', items: { type: 'string', enum: ['facebook', 'instagram', 'linkedin', 'youtube', 'x', 'telegram', 'discord', 'reddit', 'pinterest', 'tiktok'] }, description: 'Target platforms for this post. reddit, pinterest and tiktok are beta (built but not yet live-verified) - treat their delivery as unconfirmed until the operator has proven a real post.' },
  ...
  {
    name: 'reschedule',
    description: 'Move a post to a new scheduledAt (ISO datetime). Waiting-due posts just change their due time; NATIVELY-scheduled posts have their platform object deleted and re-queue for the new time (confirm: true required). Use this to move a post\'s time; use unschedule to park it entirely.',
    inputSchema: {
      type: 'object',
      properties: {
        campaign: campaignProp,
...
//   native FB/YouTube object under confirm:true.
