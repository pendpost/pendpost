...
// A pushed job still awaiting read-back/remediate - a non-empty in-flight state map. Feeds the
// always-on gate (so an acked post reconciles within a tick of the cloud firing it, not only once
// it goes overdue) and the paused-brand gate below.
function cloudInFlight() {
  try {
    const s = loadState();
    const nonEmpty = (m) => m && typeof m === 'object' && Object.keys(m).length > 0;
    return nonEmpty(s.cloudAccepted) || nonEmpty(s.cloudFailures) || nonEmpty(s.cloudRetriggered);
  } catch { return true; } // fail-safe: if state is unreadable, poll
}
...
async function runDue(actor, { campaign = null, postId = null } = {}) {
  ...
  if (cloudManaged) {
    ...
    if (cloudConnectedForActive()) {
      ...
      try {
        const cc = await import('./cloud-client.mjs');
        // Neon economy (Branch B): read back / re-assert only when the cloud could have fired a due
        // cloud-lane post the local walk would otherwise re-fire (or a pushed job is still in flight).
        // A paused brand with nothing due for a cloud lane makes zero Neon calls.
        if (cloudReconcileNeeded()) {
