...
export function lanesOwed(post) {
  ...
  if (on('x') && platforms.includes('x') && !post.ids.xPostId && !radarGone) owed.push('x');
  ...
  if (on('youtube') && platforms.includes('youtube') && !post.ids.ytVideoId && !post.radarReplyTo) owed.push('youtube');
  ...
  if (on('reddit') && platforms.includes('reddit') && !post.ids.redditPostId && !radarGone) owed.push('reddit');
  ...
  if (on('mastodon') && platforms.includes('mastodon') && !post.ids.mastodonStatusId && !post.ids.mastodonScheduledId && !radarGone) owed.push('mastodon');
  ...
  if (on('nostr') && platforms.includes('nostr') && !post.ids.nostrEventId && !post.radarReplyTo) owed.push('nostr');
...
export function* eligibleDuePosts(campaigns, { campaign = null, postId = null } = {}) {
  for (const c of campaigns) {
    ...
    for (const post of c.posts || []) {
      ...
      // missing" forever.
      if (post.type !== 'text' && post.type !== 'poll' && post.type !== 'nostr-longform' && !post.media.exists) continue;
