...
import { carouselItems } from './carousel.mjs';
...
export function buildPublishJob(post, lane, ctx = {}) {
  ...
  return Object.freeze({
    ...
    payloadRef: Object.freeze({
      ...
      // A CAROUSEL carries its ORDERED slide PATHS (relative, like mediaPath - never bytes,
      // never the sha; the cloud resolves each to a content ref via the mediaManifest and
      // presigns a public GET url per slide). Empty for every single-media / non-carousel
      // post. carouselItems is the ONE ordered source shared with the slide upload, so the
      // manifest keys and these paths can never disagree.
      mediaItems: Object.freeze(carouselItems(post).map((it) => it.path || it.file).filter(Boolean)),
      ...
      // 2026-07-25 a second client's post published onto the pendpost Instagram account
