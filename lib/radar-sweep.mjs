...
// 24h, GUARDED by posting.radar.enabled===true AND the global posting.radar.dailyEnabled
// switch AND at least one enabled query. (Cadence is now decided ONCE, globally, not per
// query.) It runs the SAME generic per-source scan (runRadarScan) the manual Scan uses,
// refreshes state.radar.signals, stamps state.radar.lastDailyScan (a SEPARATE clock from the
// manual lastScan, so a manual scan never suppresses the daily one), and refreshes the GEO
// comparison-page backlog. A no-op when Radar is off / dailyEnabled off (fail-closed): an
...
export async function dailyAgentScan() {
  ...
  // Arming is now ONE global switch: dailyEnabled on + at least one enabled query. (Was: any
  // query carrying cadence:'daily'. The owner decides the cadence once, not per search.)
  const queries = Array.isArray(radar.queries) ? radar.queries : [];
  if (radar.dailyEnabled !== true || !queries.some((q) => q && q.enabled !== false)) return null;
  ...
  // Global daily arming ⇒ the unattended run covers every enabled query (no per-query cadence
  // to scope by any more). The dailyBudget below is still the spend fence.
  const result = await radarAgentScan({ actor: 'scheduler' });
...
export async function resumeFailedLanes(job, now = Date.now()) {
  ...
  saveState();
  const result = await radarAgentScan({ actor: 'scheduler', sources: lanes });
...
export async function reconcileCopyFollowups(now = Date.now(), fetchThread = defaultCopyThreadFetch) {
  ...
  if (dirty) saveState();
  // Spec 50 P4 (§7.6 "half-done from a previous attempt"): the SAME fetched thread answers a
  // second question, so it rides this pass rather than opening a second network surface.
  const browser = await reconcileStaleBrowserReplies(now, fetchThread);
  return { checked, replied, sources: [...sources], browserReconciled: browser.reconciled, browserChecked: browser.checked };
}

/**
 * "Did it actually post?" for a browser reply whose child died mid-batch (spec 50 §7.6, §8
 * idempotency: "a rung never re-posts a reply if the reconcile has already found it").
 *
 * A `releasing` row older than 20 minutes lost its executor - the daemon restarted, the Mac
 * slept, the batch was killed - and the one thing that must not happen next is a retry that
 * posts a second copy of the same reply under the same thread. So before the ladder gets the
 * row back, this goes and LOOKS: it fetches the thread and asks the lane's own follow-up parser
 * whether a comment by OUR configured handle appeared after the row was released. Found means
 * the row is marked `done` with that permalink and never re-executed.
 *
 * The parser is the existing parseHackerNewsFollowup with our handle in the `author` slot - the
 * identical "did <author> comment in this thread after <ts>" question the author-reply pass
 * asks, pointed at ourselves. Registering a source in COPY_FOLLOWUP_PARSERS wires it here too.
 *
 * READ-only and fail-soft on every path, like the pass above: a throwing fetch or parser is a
 * failure to LOOK, so the row is left exactly where it was and comes back next time. Nothing
 * here ever fails a row - the tick's own 20-minute recovery owns that, and it requeues rather
 * than failing, precisely because the row may well have posted.
 */
async function reconcileStaleBrowserReplies(now, fetchThread) {
  let mod = null;
  try { mod = await import('./engage-browser.mjs'); } catch { return { checked: 0, reconciled: 0 }; }
  let rows = [];
  try { rows = mod.staleBrowserReplies(now); } catch { return { checked: 0, reconciled: 0 }; }
  if (!rows.length) return { checked: 0, reconciled: 0 };

  const { enginePolicy } = await import('./engage.mjs');
  const lanes = (enginePolicy().lanes) || {};
  let checked = 0;
  let reconciled = 0;
  for (const row of rows) {
    const parser = COPY_FOLLOWUP_PARSERS[row.lane];
    if (!parser) continue; // no way to look on this lane - the tick's requeue owns the row
    const handle = String((lanes[row.lane] || {}).handle || '').trim().replace(/^@/, '');
    if (!handle) continue; // without OUR handle there is nothing to recognise our own reply by
    const [source, externalId] = String(row.signalKey || '').split(' ');
    if (!source || !externalId) continue;
    const sinceTs = Date.parse(row.releasedAt || row.releaseAt || '') || 0;
    let json = null;
    try { json = await fetchThread(source, externalId); } catch { continue; }
    if (!json) { checked += 1; continue; }
    let hit = null;
    try { hit = parser(json, { author: handle, sinceTs }); } catch { continue; }
    checked += 1;
    if (!hit || !hit.replied) continue;
    try {
      const res = mod.reconcileBrowserRow(row.id, { permalink: hit.permalink || null, now });
      if (res && res.ok) reconciled += 1;
    } catch { /* the row stays in flight and the tick's own recovery requeues it */ }
  }
...
export async function dailyRadarScan() {
  ...
  if (radar.enabled !== true) return null;
  // ONE global cadence switch: dailyEnabled on + at least one enabled query. (Was: any query
  // carrying cadence:'daily'. The owner decides the cadence once, for all searches.)
  const queries = Array.isArray(radar.queries) ? radar.queries : [];
  if (radar.dailyEnabled !== true || !queries.some((q) => q && q.enabled !== false)) return null;
  ...
  // Sweep every enabled query (no per-query cadence to filter by any more). runRadarScan scores,
  // dedupes, persists state.radar.signals + lastScan AND the GEO comparison backlog.
