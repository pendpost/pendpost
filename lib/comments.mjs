...
export function isOwnAuthor(lane, { author, authorId } = {}) {
  switch (lane) {
    ...
    case 'linkedin': {
      // author = c.actor (a URN). "Own" is EITHER the Company Page or the connected member's
      // personal profile - a post published with liAuthor:'member' carries the person URN as
      // actor, and must count as ours (else our own replies read as a stranger's in the sweep).
      const orgUrn = readEnv('LINKEDIN_ORG_URN');
      const personUrn = readEnv('LINKEDIN_PERSON_URN');
      return (Boolean(orgUrn) && author === orgUrn) || (Boolean(personUrn) && author === personUrn);
    }
    case 'mastodon': {
...
// Which of the owner's OWN comments the inbox sweep may HIDE (owner decision 5,
// 2026-08-17). isOwnAuthor above answers WHO wrote a comment; this answers whether it is
// HOUSEKEEPING: the first-comment pendpost itself posted under the post (post.firstComment
// - hashtags, links, the engagement-pattern comment), or a hashtag-only comment. A
// SUBSTANTIVE own reply must stay visible - answers TO it are how a conversation
// continues, and hiding it made the inbox thread illegible (the pre-refinement sweep hid
// ALL own comments). Comparison is normalized (trim, collapse whitespace,
// case-insensitive) so a lane echoing the first comment with different spacing still
// matches. Hashtag-only = a non-empty token list where EVERY whitespace token starts with
// '#'. Pure, and it only ever NARROWS what isOwnAuthor already flagged - a stranger's
// comment never reaches it, so the fail-open posture is preserved by construction.
function normOwnComment(s) {
  return String(s == null ? '' : s).replace(/\s+/g, ' ').trim().toLowerCase();
}
export function isOwnHousekeepingComment(text, firstComment) {
  const t = normOwnComment(text);
  if (!t) return false; // nothing to compare - keep (fail-open)
  const fc = normOwnComment(firstComment);
  if (fc && t === fc) return true; // the first-comment we posted ourselves
  const tokens = t.split(' ');
  return tokens.length > 0 && tokens.every((w) => w.startsWith('#')); // hashtag-only
}
