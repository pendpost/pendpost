...
import { activeRoot, connectFor } from './context.mjs';
import { loadState, saveState } from './state.mjs';
import { schedulerEnabledHere, appendActivity, metaUsage } from './scheduler.mjs';
...
// US-MC-10: the per-client scheduler flag (default-ON), NOT the process-global
// timer. account_status is resolved per active client, so this is now per-client-
// truthful: it reads THIS client's state.scheduler.enabled via the shared predicate.
export function schedulerRunning() {
  return schedulerEnabledHere(loadState());
}
...
export function accountStatus() {
  ...
  return {
    ...
    meta: {
      ...
      // second client's post published onto the pendpost account on 2026-07-25. Empty when
    ...
    linkedin: {
      ...
      // The connected member's personal-profile URN, captured at auth (empty until the owner
      // re-connects with the member scope). Its presence is what makes liAuthor:'member' postable.
      personUrn: readEnv('LINKEDIN_PERSON_URN') || '',
      hint: readEnv('LINKEDIN_ACCESS_TOKEN') ? null : `Not connected - ${connectFor('node scripts/linkedin-social.mjs auth')}`,
    ...
    x: {
      ...
      hint: (readEnv('X_REFRESH_TOKEN') || (readEnv('X_ACCESS_TOKEN') && readEnv('X_ACCESS_TOKEN_SECRET')))
        ? null
        : `Not connected - ${connectFor('node scripts/x-social.mjs auth')}`,
      live: live.x || null,
    },
    youtube: {
    ...
    telegram: {
      ...
      hint: readEnv('TELEGRAM_BOT_TOKEN')
        ...
        : `Not connected - get a bot token from @BotFather, then ${connectFor('node scripts/telegram-social.mjs auth')}`,
    ...
    discord: {
      ...
      hint: readEnv('DISCORD_WEBHOOK_URL') ? null : `Not connected - create a channel webhook, then ${connectFor('node scripts/discord-social.mjs auth')}`,
    ...
    reddit: {
      ...
      hint: (readEnv('REDDIT_CLIENT_ID') && readEnv('REDDIT_USERNAME'))
        ...
        : `Not connected - create a script app at reddit.com/prefs/apps, then ${connectFor('node scripts/reddit-social.mjs auth')}`,
    ...
    pinterest: {
      ...
      hint: (readEnv('PINTEREST_ACCESS_TOKEN') || readEnv('PINTEREST_REFRESH_TOKEN'))
        ...
        : `Not connected - create an app at developers.pinterest.com, then ${connectFor('node scripts/pinterest-social.mjs auth')}`,
    ...
    tiktok: {
      ...
      hint: (readEnv('TIKTOK_ACCESS_TOKEN') || readEnv('TIKTOK_REFRESH_TOKEN'))
        ? null
        : `Not connected - create an app at developers.tiktok.com, then ${connectFor('node scripts/tiktok-social.mjs auth')}`,
    ...
    mastodon: {
      ...
      hint: readEnv('MASTODON_ACCESS_TOKEN')
        ...
        : `Not connected - create an app under Preferences > Development on your instance, then ${connectFor('node scripts/mastodon-social.mjs auth')}`,
    ...
    wordpress: {
      ...
      hint: (readEnv('WORDPRESS_SITE_URL') && readEnv('WORDPRESS_USERNAME') && readEnv('WORDPRESS_APP_PASSWORD'))
        ? null
        : `Not connected - create an application password under Users > Profile, then ${connectFor('node scripts/wordpress-social.mjs auth')}`,
    ...
    ghost: {
      ...
      hint: (readEnv('GHOST_SITE_URL') && readEnv('GHOST_ADMIN_API_KEY'))
        ? null
        : `Not connected - create a custom integration under Settings > Integrations, then ${connectFor('node scripts/ghost-social.mjs auth')}`,
    ...
    nostr: {
      ...
      hint: readEnv('NOSTR_PRIVATE_KEY')
        ...
        : `Not connected - node scripts/nostr-social.mjs keygen --save, then ${connectFor('node scripts/nostr-social.mjs auth')}`,
    ...
    gbp: {
      ...
      hint: (readEnv('GBP_ACCESS_TOKEN') || readEnv('GBP_REFRESH_TOKEN'))
        ...
        : `Not connected - create OAuth credentials in the Google Cloud console, then ${connectFor('node scripts/gbp-social.mjs auth')}`,
      live: live.gbp || null,
    },
    scheduler: {
