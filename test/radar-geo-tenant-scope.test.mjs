...
//
// THE DEFECT (live, a second client's tenant, 2026-09-06): geo.comparisonBacklog leaked the WRONG
...
//      in the self-name filter, so a second client's scan emailed the operator "pendpost vs Buffer"
...
try {
  ...
  {
    // footprint bridge: a rival the AI names for a northwind buying question is a "northwind vs X" page.
    const fp = [miss('best coaching platform?', ['Rival']), miss('top coaching tools?', ['Rival'])];
    const out = comparisonBacklog([], [], fp, { brand: 'northwind', competitors: [] });
    ok(out.length === 1 && out[0].title === 'northwind vs Rival', 'footprint-bridge entry is titled with the ACTIVE brand ("northwind vs Rival")');
  ...
  }
  {
    ...
    const out = comparisonBacklog([sig('Buffer vs Hootsuite for a small team?')], [], [], { brand: 'northwind', competitors: ['Buffer', 'Hootsuite'] });
    ok(titles(out).sort().join() === 'northwind vs Buffer,northwind vs Hootsuite', 'a declared-rival vs-chain yields "northwind vs Buffer" + "northwind vs Hootsuite"');
  ...
  }
  {
    ...
    const out = comparisonBacklog([sig('northwind vs Buffer, which is better?')], [], [], { brand: 'northwind', competitors: ['Buffer'] });
    ok(titles(out).join() === 'northwind vs Buffer', 'the chain never mints "northwind vs northwind" (self-name filtered per-tenant)');
  }
  ...
  {
    // The exact live-observed noise, on a tenant with NO declared rivals (the real config of the tenant it was found on).
    const noise = [
    ...
    const out = comparisonBacklog(noise, [], [], { brand: 'northwind', competitors: [] });
  ...
  }
  {
    ...
    const out = comparisonBacklog([sig('Arduino vs Evil - a talk')], [], [], { brand: 'northwind', competitors: ['Buffer', 'Hootsuite'] });
  ...
  {
    const out = comparisonBacklog([sig('is there an alternative to Buffer that is cheaper?')], [], [], { brand: 'northwind', competitors: ['Buffer'] });
  ...
  }
  {
    ...
    const out = comparisonBacklog([sig('honestly northwind vs CoachAccountable is close')], [], [], { brand: 'northwind', competitors: [] });
    ok(titles(out).join() === 'northwind vs CoachAccountable', 'a direct "<brand> vs X" comparison discovers an undeclared rival (brand-in-chain exception)');
  }
  ...
  console.log('shareOfVoice: same per-tenant rival scoping');
  {
    ...
    const sov = shareOfVoice(sigs, [], { brand: 'northwind', competitors: ['Buffer'] });
