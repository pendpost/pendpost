...
try {
  ...
  ok(/63 routes, 43 tools/.test(parityOut), `parity is 63 routes / 43 tools: ${parityOut.trim()}`);

  console.log(`[clients-overview] OK - per-client roll-up metrics, 368=>metaBlocked+zero-writes, isolation (no nextDue bleed), corrupt-subtree fail-soft, parity 63/43 (${pass} assertions).`);
} finally {
