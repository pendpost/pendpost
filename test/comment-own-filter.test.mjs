#!/usr/bin/env node
// test/comment-own-filter.test.mjs - the inbox must show OTHERS' comments, not the
// owner's own. pendpost posts a first-comment on its own IG/FB media as itself, and
// Mastodon reads the whole thread (so our own replies come back). isOwnAuthor
// (lib/comments.mjs) drops those in the sweep. Also pins the row-like capability
...
const { isOwnAuthor } = await import('../lib/comments.mjs');
...
const READS = {
  [`${CAMP}:p-ig:instagram`]: { ok: true, items: [
    { commentId: 'ig-own', author: 'mybrand', authorId: 'IG_OWNER', text: 'our first comment #tags', ts: new Date(NOW - 3 * HOUR).toISOString(), kind: 'comment' },
...
try {
  ...
  // ---- sweep filters own, keeps strangers ----------------------------------
  ...
  ok(!igIds.includes('ig-own'), 'sweep: our own IG first-comment is filtered out');
  ...
  ok(!mIds.includes('m-own'), 'sweep: our own Mastodon reply is filtered out');
