...
- **Nothing phones home.** There is no analytics or usage reporting. Any future opt-in telemetry would be off by default, and it is not built today. This holds for feedback too: the dashboard's **Share feedback** action is a user-initiated browser navigation to a prefilled GitHub page, not a request from the engine, and it prefills only non-sensitive facts (version, run method, mode, Node, OS), never your `.env`, tokens, or account identifiers. The optional feedback form on pendpost.com is website infrastructure, separate from the local engine.
