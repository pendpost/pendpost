...
- **Always-on self-host.** Deploy templates (Fly.io, Railway, Render) plus a Docker image and a docs guide for running pendpost on a host that never sleeps, so Instagram, LinkedIn, and X publish on schedule even when your computer is off. The server stays loopback-only with no auth by default; an optional bearer-token gate activates only when you bind it to a non-loopback host and set a token. Facebook and YouTube already survive power-off via native scheduling.

## Near-term (aspirational)

- **More platforms.** Broaden coverage beyond Facebook, Instagram, LinkedIn, X, and YouTube, while keeping the same approval-gated, parity-checked contract. Bluesky is specced as the reference next lane.
